...
public class CT_RoundTripBeanMaps extends RoundTripTest {
	...
	public CT_RoundTripBeanMaps(String label, WriterSerializer s, ReaderParser p, int flags) throws Exception {
	...
	public void testSubTypesUsingAnnotation() throws Exception {
		JsonSerializer js = JsonSerializer.DEFAULT_LAX.clone().addFilters(XMLGregorianCalendarFilter.class);
	...
	public void testSubTypesUsingBeanFilter() throws Exception {
		...
		WriterSerializer s = getSerializer().clone().addFilters(CFilter.class, XMLGregorianCalendarFilter.class);
		ReaderParser p = getParser().clone().addFilters(CFilter.class, XMLGregorianCalendarFilter.class);
	...
	public void testSubTypePropertyWithRealPropertyUsingBeanFilter() throws Exception {
		...
		WriterSerializer s = getSerializer().clone().addFilters(CAFilter.class);
		ReaderParser p = getParser().clone().addFilters(CAFilter.class);
	...
	//====================================================================================================
	@Test
	public void testPropertiesUsingFilter() throws Exception {
		JsonSerializer js = JsonSerializer.DEFAULT_LAX.clone().addFilters(D2Filter.class);

		// Skip validation-only tests
		if (isValidationOnly())
			return;

		WriterSerializer s = getSerializer().clone().addFilters(D2Filter.class);
		ReaderParser p = getParser().clone().addFilters(D2Filter.class);
	...
	//====================================================================================================
	@Test
	public void testExcludePropertiesUsingFilter() throws Exception {
		// Skip validation-only tests
		if (isValidationOnly())
			return;

		WriterSerializer s = getSerializer().clone().addFilters(E2Filter.class);
		ReaderParser p = getParser().clone().addFilters(E2Filter.class);
	...
	//====================================================================================================
	@Test
	public void testInterfaceClassUsingFilter() throws Exception {
		WriterSerializer s;
		ReaderParser p;
		FB2 t;
		String r;
		...
		// --- Filter defined on parent class ---
		s = getSerializer().clone().addFilters(FB1Filter.class);
		p = getParser().clone().addFilters(FB1Filter.class);
		...
		// --- Filter defined on child class class ---
		s = getSerializer().clone().addFilters(FB2Filter.class);
		p = getParser().clone().addFilters(FB2Filter.class);
		...
		// --- Filter defined as plain class ---
		s = getSerializer().clone().addFilters(FB1.class);
		p = getParser().clone().addFilters(FB1.class);
	...
	public void testParsingIntoTopLevelNonStaticInnerClasses() throws Exception {
		...
		String r = s.serialize(n.n2);
		n = new N(2);
		ParserContext ctx = p.createContext(null, null, n);

		N.N2 n2 = p.parse(new StringReader(r), -1, BeanContext.DEFAULT.getClassMeta(N.N2.class), ctx);
