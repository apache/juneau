...
public abstract class RoundTripTest {
	...
	protected WriterSerializer s;
	protected ReaderParser p;
	...
	public RoundTripTest(String label, WriterSerializer s, ReaderParser p, int flags) throws Exception {
		this.s = s.clone().addFilters(getFilters());
		this.p = p == null ? null : p.clone().addFilters(getFilters());
		this.label = label;
	...
	public <T> T roundTrip(T object, ClassMeta<? extends T> t) throws Exception {
		String s = serialize(object, this.s);
		if (p == null)
			return object;
		T o = p.parse(s, t);
	...
	public <T> T roundTrip(T object, WriterSerializer serializer, ReaderParser parser) throws Exception {
		String s = serialize(object, serializer);
		if (parser == null)
			return object;
		T o = (T)parser.parse(s, object == null ? Object.class : object.getClass());
		return (returnOriginalObject ? object : o);
	}

	public WriterSerializer getSerializer() {
		return s;
	}

	public ReaderParser getParser() {
		return p;
	}

	protected void addFilters(Class<?>...c) {
		s.addFilters(c);
		if (p != null)
			p.addFilters(c);
	}
	...
	public <T> String serialize(T object, WriterSerializer s) throws Exception {

		String out = s.serialize(object);

		if (debug)
			System.err.println("Serialized contents from ["+label+"]...\n---START---\n" + out + "\n---END---\n");

		if (validateXmlWhitespace)
			TestUtils.checkXmlWhitespace(out);
