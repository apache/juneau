...
import static com.ibm.juno.server.RestServlet.ParamType.*;
...
import com.ibm.juno.server.annotation.Var;
...
public abstract class RestServlet extends HttpServlet {
	...
	private boolean renderResponseStackTraces, useStackTraceHashes = true;
	private Map<Integer,Integer> stackTraceHashes = new HashMap<Integer,Integer>();
	private String path;

	boolean allowHeaderParams, allowContentParam;
	...
	{
			this.varResolver = createVarResolver();
		...
		try {
			...
			for (Map.Entry<Class<?>,RestResource> e : restResourceAnnotationsChildFirst.entrySet()) {
				...
				if (! r.messages().isEmpty()) {
					if (msgs == null)
						msgs = new RestServletNls(c, r.messages());
					else
						msgs.addDef(c, r.messages());
				}
				if (label.isEmpty())
					label = r.label();
			...
			renderResponseStackTraces = p.getBoolean(REST_renderResponseStackTraces, false);
			useStackTraceHashes = p.getBoolean(REST_useStackTraceHashes, true);

			allowHeaderParams = p.getBoolean(REST_allowHeaderParams, true);

			allowMethodParams = new LinkedHashSet<String>();
			for (String m : StringUtils.split(p.getString(REST_allowMethodParam, ""), ','))
			...
			allowContentParam = p.getBoolean(REST_allowContentParam, true);
			defaultCharset = p.getString(REST_defaultCharset, "utf-8");
	 ...
	 * if {@link RestServletProperties#REST_renderResponseStackTraces} is enabled.
	 * </p>
	 * <p>
	...
	public String getDescription(RestRequest req) {
		if (! description.isEmpty())
			return req.getVarResolver().resolve(description);
		String description = msgs.findMessage(req.getLocale(), "description");
		return (description == null ? "" : req.getVarResolver().resolve(description));
	}
	...
	public String getLabel(RestRequest req) {
		if (! label.isEmpty())
			return req.getVarResolver().resolve(label);
		String label = msgs.findMessage(req.getLocale(), "label");
		return (label == null ? "" : req.getVarResolver().resolve(label));
	}
	...
	static enum ParamType {
		REQ, RES, ATTR, CONTENT, HEADER, METHOD, PARAM, PARAM_MP, QPARAM, QPARAM_MP, HASPARAM, HASQPARAM, PATHREMAINDER, PROPS, MESSAGES;
	...
	static class MethodParam {
		...
		MethodParam(Type type, Method method, Annotation[] annotations) throws ServletException {
			this.type = type;
			...
			if (isClass && isParentClass(HttpServletRequest.class, (Class)type))
				...
				if (a instanceof Attr) {
					...
					paramType = p.multipart() ? PARAM_MP : PARAM;
					name = p.value();
				} else if (a instanceof QParam) {
					QParam p = (QParam)a;
					if (p.multipart())
						assertCollection(type, method);
					paramType = p.multipart() ? QPARAM_MP : QPARAM;
					name = p.value();
		...
		private Object getValue(RestRequest req, RestResponse res) throws Exception {
			...
			switch(paramType) {
				...
				case PARAM:      return req.getParameter(name, type);
				case PARAM_MP:   return req.getParameters(name, type);
				case QPARAM:     return req.getQueryParameter(name, type);
				case QPARAM_MP:  return req.getQueryParameters(name, type);
	...
	private class MethodMeta extends ResourceMethod implements Comparable<MethodMeta>  {
		...
		private MethodMeta(java.lang.reflect.Method method) throws RestServletException {
			try {
				...
				if (m.filters().length > 0) {
					...
					mUrlEncodingParser.addFilters(m.filters());
				}
				...
				mDefaultEncoding = mProperties.getString(REST_defaultCharset, defaultCharset);
				...
				for (int i = 0; i < params.length; i++) {
					params[i] = new MethodParam(pt[i], method, pa[i]);
		...
		private String getDescription(RestRequest req) {
			if (! description.isEmpty())
				return req.getVarResolver().resolve(description);
			String description = msgs.findMessage(req.getLocale(), method.getName());
			return (description == null ? "" : req.getVarResolver().resolve(description));
		}
		...
		int invoke(String methodName, String pathInfo, RestServlet resource, RestRequest req, RestResponse res) throws RestException {
			...
			req.init(method, remainder, createRequestProperties(mProperties, req), mDefaultRequestHeaders, mDefaultEncoding, mParsers, mUrlEncodingParser);
			...
			} catch (RestServletException e1) {
				// This shouldn't happen.
				throw new RestException(SC_INTERNAL_SERVER_ERROR, e1);
			}
	...
	protected MethodDescription getMethodDescription(Method method, MethodMeta meta, RestRequest req) throws RestServletException {
		...
		StringVarResolver vr = req.getVarResolver();
		...
		if (rm != null) {
			...
			for (Response r : rm.responses()) {
				com.ibm.juno.server.labels.MethodDescription.Response r2 = d.addResponse(r.value());
			...
			SafeResourceBundle rb = getResourceBundle(req.getLocale());
	 ...
	 */
	protected StringVarResolver getVarResolver() {
		return varResolver;
	}
	...
	protected StringVarResolver createVarResolver() {
		...
		r.addVar("C", new StringVarWithDefault() {
			@Override /* StringVar */
			public String resolve(String varVal) {
				return getConfig().getString(varVal);
			}
		});

		// System properties.
		r.addVar("S", new StringVarWithDefault() {
			@Override /* StringVar */
			public String resolve(String varVal) {
		...
		r.addVar("E", new StringVarWithDefault() {
			@Override /* StringVar */
			public String resolve(String varVal) {
		...
		// Init parameters.
		r.addVar("I", new StringVarWithDefault() {
			@Override /* StringVar */
			public String resolve(String varVal) {
				return RestServlet.this.getServletConfig().getInitParameter(varVal);
			}
		});
	...
	/**
	 * Returns the variable resolver for the specified request.
	 * <p>
	 * Augments the variable resolver for the servlet {@link #getVarResolver()} with request-specific variables.
	 * Variable resolvers are used to replace variables in property values.
	 * </p>
	 * <p>
	 * In addition to the servlet-level variables available on {@link #getVarResolver()}, adds the following
	 * request-specific variables:
	 * </p>
	 * <ul>
	 * 	<li><code>$L{...}</code> - Localized strings pulled from resource bundle.<br>
	 * 		Resolves to the value returned by {@link RestRequest#getMessage(String, Object...)}.<br>
	 * 		Can include message arguments (e.g. <js>"$L{my.localized.string, foo, bar}"</js>
	 * 	<li><code>$A{...}</code> - Request attributes.<br>
	 * 		Resolves to the value returned by {@link RestRequest#getAttribute(String)}.<br>
	 * 		Can include a default value (e.g. <js>"$A{myAttribute,myDefaultValue}"</js>
	 * 	<li><code>$P{...}</code> - Request parameters.<br>
	 * 		Resolves to the value returned by {@link RestRequest#getParameter(String)}.<br>
	 * 		Can include a default value (e.g. <js>"$P{myAttribute,myDefaultValue}"</js>
	 * 	<li><code>$R{...}</code> - Request variables.<br>
	 * 		Possible values:
	 * <ul>
	 ...
	 * </ul>
	 * 	<li><code>$UE{...}</code> - URL-Encode the specified value.<br>
	 * 		Takes the contents inside the varible and replaces it with the value returned by calling {@link RestUtils#encode(String)}.
	 * </ul>
	 * <p>
	 * Subclasses can override this method to provide their own request-specific replacement variables.
	 * See {@link #createVarResolver()} for an example.
	 * </p>
	 *
	 * @param req The HTTP request.
	 * @return The string var resolver to use for this request.
	 */
	public StringVarResolver createRequestVarResolver(final RestRequest req) {
		StringVarResolver r = new StringVarResolver(getVarResolver());

		// Localized string (pulled from resource bundle).
		r.addVar("L", new StringVarMultipart() {
			...
			public String resolve(String[] args) {
				if (args.length > 0) {
					...
					return req.getMessage(key, (Object[])a);
				}
				return "";
			}
		});
		...
		r.addVar("R", new StringVar() {
			...
			public String resolve(String varVal) {
				if (varVal.length() > 0) {
					char c = varVal.charAt(0);
					if (c == 'c') {
						if (varVal.equals("contextPath"))
							return req.getContextPath();
					} else if (c == 'm') {
						if (varVal.equals("method"))
							return req.getMethod();
						if (varVal.equals("methodDescription"))
						...
						if (varVal.equals("relativeServletURI"))
						...
						if (varVal.equals("servletParentURI"))
							return req.getServletParentURI();
						if (varVal.equals("servletLabel"))
							return req.getServletLabel();
						if (varVal.equals("servletDescription"))
							return req.getServletDescription();
					} else if (c == 't') {
						if (varVal.equals("trimmedRequestURI"))
		...
		// Request attributes
		r.addVar("A", new StringVarWithDefault() {
			@Override /* StringVar */
			public String resolve(String varVal) {
				Object o = req.getAttribute(varVal);
				return (o == null ? "" : o.toString());
			}
		});

		// Request parameters
		r.addVar("P", new StringVarWithDefault() {
			@Override /* StringVar */
			public String resolve(String varVal) {
				return req.getParameter(varVal);
			}
		});

		r.addVar("UE", new StringVar() {
			@Override /* StringVar */
			public String resolve(String varVal) {
				return RestUtils.encode(varVal);
			}
		});
		...
		ObjectMap m = new ObjectMap() {
			...
			public Object get(Object key) {
				...
				if (o instanceof String)
					o = req.getVarResolver().resolve(o.toString());
				return o;
			}
		};
	 ...
	 * 	<li>{@link SerializerProperties#SERIALIZER_relativeUriBase} from {@link ServletConfig#getServletContext()}.
	 * </ul>
	 *
	 * @return The resource properties as an {@link ObjectMap}.
	 */
	 ...
	 * </p>
	 * <p>
	 * 	By default, returns the filters specified through the {@link RestResource#filters() @RestResource.filters()} annotation in child-to-parent order.
	 * (i.e. filters will be applied in child-to-parent order with child annotations overriding parent annotations when
	 ...
	 * </p>
	 * <p>
	 * Returns an ordered map with case-insentitive matching.
	 * </p>
	 *
	 * @return The class-level default request headers associated with this servlet.
	 * @throws RestServletException
	 */
	public TreeMap<String,String> getDefaultRequestHeaders() throws RestServletException {
		if (defaultRequestHeaders == null) {
			defaultRequestHeaders = new TreeMap<String,String>(String.CASE_INSENSITIVE_ORDER);
			defaultRequestHeaders.putAll(createDefaultRequestHeaders());
		}
	 ...
	 * @throws RestServletException
	 */
	protected Map<String,String> createDefaultRequestHeaders() throws RestServletException {
	 ...
	 * @throws RestServletException
	 */
	protected Map<String,Object> getDefaultResponseHeaders() throws RestServletException {
	 ...
	 * <p>
	 * On first execution, calls {@link #createResponseHandlers()} to create the set of handlers.
	 * Subsequent calls returns the same cached copy from the first call.
	 * </p>
	 * <p>
	 * Typically, subclasses override {@link #createResponseHandlers()} to implement or append their own set of handlers.
	 * </p>
	 *
	 * @return The class-level response handlers associated with this servlet.
	 * @throws RestException
	 */
	protected ResponseHandler[] getResponseHandlers() throws RestException {
		if (responseHandlers == null)
			responseHandlers = createResponseHandlers().toArray(new ResponseHandler[0]);
	 ...
	 * @throws RestException
	 */
	protected List<ResponseHandler> createResponseHandlers() throws RestException {
	 ...
	 * @throws RestServletException
	 */
	protected Map<String,Object> createDefaultResponseHeaders() throws RestServletException {
	...
	public SerializerGroup getSerializers() throws RestServletException {
		if (serializers == null)
			try {
				...
				throw new RestServletException("Exception occurred while trying to create serializers").initCause(e);
			}
		return serializers;
	}
	...
	public ParserGroup getParsers() throws RestServletException {
		if (parsers == null)
			try {
				...
				throw new RestServletException("Exception occurred while trying to create parsers").initCause(e);
			}
		return parsers;
	}
		...
		for (RestResource r : restResourceAnnotationsParentFirst.values())
			for (Class<? extends Parser<?>> c : reverse(r.parsers()))
				try {
					...
					throw new RestServletException("Exception occurred while trying to instantiate Parser ''{0}''", c.getSimpleName()).initCause(e);
				}
	...
	public UrlEncodingParser getUrlEncodingParser() throws RestServletException {
		if (urlEncodingParser == null)
			try {
				...
				throw new RestServletException("Exception occurred while trying to instantiate URL-Encoding parser.").initCause(e);
			}
	 ...
	 * <p>
	 * On first execution, calls {@link #createUrlEncodingSerializer()} to create a new serializer, then
	 * 	associates the class-level filters and properties with the serializer, then locks the serializer.
	 * Subsequent calls returns the same cached copy from the first call.
	 * </p>
	 * <p>
	 * Typically, subclasses override {@link #createUrlEncodingSerializer()} to implement their own serializer.
	 * However, they can instead override this method if they want to prevent the class-level filters
	 * 	and properties from being applied to the serializer.
	 * </p>
	 *
	 * @return The URL-Encoding serializer.
	 * @throws RestServletException If {@link #createUrlEncodingParser()} threw an exception.
	 */
	public UrlEncodingSerializer getUrlEncodingSerializer() throws RestServletException {
		if (urlEncodingSerializer == null) {
			try {
				urlEncodingSerializer = createUrlEncodingSerializer().addFilters(getFilters()).setProperties(getProperties()).lock();
			} catch (Exception e) {
				throw new RestServletException("Exception occurred while trying to instantiate URL-Encoding serializer.").initCause(e);
			}
		}
	 ...
	 * </p>
	 * <p>
	 * By default, returns a default URL-Encoding serializer from calling {@link UrlEncodingSerializer#UrlEncodingSerializer()}.
	 * </p>
	 * <p>
	 * This serializer must be unlocked so that the filters and properties defined on this servlet can be associated with it in the {@link #getUrlEncodingSerializer()} method.
	 * </p>
	 *
	 * @return The new URL-Encoding serializer.
	 * @throws Exception If the serializer could not be constructed for any reason.
	 */
	protected UrlEncodingSerializer createUrlEncodingSerializer() throws Exception {
		return new UrlEncodingSerializer();
	}
	 ...
	 * <p>
	 * On first execution, calls {@link #createBeanContextFactory()} to create a new bean context, then
	 * 	associates the class-level filters and properties with it, then locks it.
	 * Subsequent calls returns the same cached copy from the first call.
	 * </p>
	 * <p>
	 * Typically, subclasses override {@link #createBeanContextFactory()} to implement their own specialized bean context.
	 * </p>
	 *
	 * @return The bean context used for parsing path variables and header values.
	 * @throws RestException If {@link #createBeanContextFactory()} threw an exception.
	 */
	public BeanContext getBeanContext() {
		if (beanContext == null)
			try {
				beanContext = createBeanContextFactory().addFilters(getFilters()).setProperties(getProperties()).setDefaultParser(getUrlEncodingParser()).getBeanContext();
			} catch (Exception e) {
				throw new RestException(SC_INTERNAL_SERVER_ERROR, "Exception occurred while trying to instantiate Bean context.").initCause(e);
			}
		return beanContext;
	}
	 ...
	 * They can do this by either creating a new {@link BeanContextFactory} from scratch, or appending to the
	 * 	group returned by <code><jk>super</jk>.createBeanContext()</code>.
	 * </p>
	 * <p>
	 * By default, returns a new bean context from calling {@link BeanContextFactory#BeanContextFactory()}.
	 * <p>
	 ...
	 */
	protected BeanContextFactory createBeanContextFactory() throws Exception {
		return new BeanContextFactory();
	}

	/**
	 * Subclasses can override this method to provide a way to resolve interfaces
	 * 	in {@link com.ibm.juno.server.annotation.RestResource#children() @RestResource.children()} annotations.
	 *
	...
	public <T> T getResource(Class<T> c, String mediaType, String name) throws IOException, ServletException {
		...
		try {
			...
			if (p != null) {
				try {
					if (p.isReaderParser())
						return (T)p.parse(new InputStreamReader(is, IOUtils.UTF8), -1, c);
					return (T)p.parse(is, -1, c);
