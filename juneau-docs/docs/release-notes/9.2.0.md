...
- **@Beanp(format) Not Working**: Fixed a bug where the `@Beanp(format="...")` annotation was not applying `String.format()` patterns during bean serialization. The issue was caused by missing calls to `getPropertySwap()` when processing `@Beanp` annotations on fields and getter methods. This feature now correctly formats numeric fields using Java format strings (e.g., `@Beanp(format="%.2f")` for floating-point values, `@Beanp(format="%05d")` for zero-padded integers, `@Beanp(format="$%.2f")` for currency formatting).
