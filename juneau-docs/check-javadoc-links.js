...
class JavadocAwareLinkChecker {
  constructor() {
    this.brokenLinks = [];
    this.checkedUrls = new Set();
    this.staticFiles = new Set();
    this.javadocFiles = new Set();
  ...
  buildStaticFileIndex() {
    ...
    console.log(`📋 Indexed ${this.javadocFiles.size} javadoc files\n`);
  }
    ...
    const walkDir = (currentDir, basePath = '') => {
      ...
      for (const item of items) {
        ...
        if (stat.isDirectory()) {
          ...
          // Special handling for javadoc files
          if (relativePath.includes('/apidocs/') && relativePath.endsWith('.html')) {
            this.javadocFiles.add(relativePath);
            this.javadocFiles.add('/' + relativePath);
            this.javadocFiles.add('./' + relativePath);
          }
  ...
  async checkAll() {
    console.log('🔍 Starting Javadoc-aware link check for Juneau documentation...\n');
  ...
  checkRelativeLink(url, filePath) {
    try {
      ...
      // Special handling for Javadoc links
      if (url.includes('/apidocs/') && url.includes('.html')) {
        const javadocAlternative = this.findJavadocAlternative(url);
        if (javadocAlternative && fs.existsSync(javadocAlternative)) {
          return false; // Found alternative Javadoc file
        }
      }
  ...
  findJavadocAlternative(url) {
    // Convert slash-based inner class references to dot-based ones
    // e.g., RestContext/Builder.html -> RestContext.Builder.html
    ...
    if (url.includes('/') && url.includes('.html')) {
      ...
      if (lastPart.includes('.html')) {
        // Check if this looks like an inner class reference
        const className = lastPart.replace('.html', '');
        
        // Look for files with dots instead of slashes
        for (const javadocFile of this.javadocFiles) {
          if (javadocFile.endsWith('.html')) {
            const fileName = path.basename(javadocFile, '.html');
            
            // Check if this is the same class but with dot notation
            if (fileName.includes('.') && className.includes('/')) {
              const dotVersion = className.replace('/', '.');
              if (fileName === dotVersion) {
                return path.join(process.cwd(), javadocFile);
              }
            }
          }
        }
  ...
  generateReport() {
    console.log('\n' + '='.repeat(80));
    console.log('📊 JAVADOC-AWARE LINK CHECK REPORT');
...
if (require.main === module) {
  const checker = new JavadocAwareLinkChecker();
...
module.exports = JavadocAwareLinkChecker;
