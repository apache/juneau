...
- **SuppressWarnings Format**: All `@SuppressWarnings` annotations must use multi-line format with curly braces and include a brief comment explaining why each suppression is needed. Never use single-line `@SuppressWarnings("xxx")` without the multi-line format and comment.
  ```java
  @SuppressWarnings({
  ...
  ```
  **Exception**: Parameter-level annotations (e.g. `catch (@SuppressWarnings("unused") Exception e)`) may remain single-line when multi-line format is impractical.
