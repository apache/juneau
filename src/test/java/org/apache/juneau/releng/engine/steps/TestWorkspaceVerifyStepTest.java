...
class TestWorkspaceVerifyStepTest {
	...
	private StepContext ctx() {
		...
		c.run = RunState.create("9.2.1", "b", List.of("test-workspace-verify"));
	...
	void a01_unzipsAllFourReactorWorkspaceTemplatesFromTheirPreReleaseSnapshotPaths() {
		...
		assertUnzip(base + "juneau-examples/juneau-examples-core/target/juneau-examples-core-9.2.1-SNAPSHOT-bin.zip",
				base + "target/workspace/juneau-examples-core");
		assertUnzip(base + "juneau-examples/juneau-examples-mcp/target/juneau-examples-mcp-9.2.1-SNAPSHOT-bin.zip",
				base + "target/workspace/juneau-examples-mcp");
		assertUnzip(base + "juneau-petstore/juneau-petstore-jetty/target/juneau-petstore-jetty-9.2.1-SNAPSHOT-bin.zip",
				base + "target/workspace/juneau-petstore-jetty");
		assertUnzip(base + "juneau-sc/juneau-sc-server/target/juneau-sc-server-9.2.1-SNAPSHOT-bin.zip",
				base + "target/workspace/juneau-sc-server");
	}

	private void assertUnzip(String zip, String dest) {
		assertTrue(calls.stream().anyMatch(c -> c.equals(List.of("unzip", "-o", zip, "-d", dest))),
				"expected unzip of " + zip + " into " + dest + "; actual calls: " + calls);
	}
}
