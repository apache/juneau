...
/**
 * Serverless example test: drives {@link HelloWorldResource} in-process via {@link MockRestClient},
 * and exercises the {@link GreetingApi} typed {@code @Remote} client.
 */
class HelloWorldResourceTest {
	...
	void contentNegotiation_servesJsonXmlHtml_andReadsYamlConfig() throws Exception {
		try (var client = MockRestClient.create(HelloWorldResource.class)) {
			...
			// JSON — the message text proves the YAML config (my-app.yaml) was read.
