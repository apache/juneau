...
/**
 * Minimal Juneau starter resource.
 *
 * <ul>
 * 	<li>Returns a {@link Greeting} POJO so the same endpoint serves JSON / XML / HTML (content negotiation).
 * 	<li>Carries an {@link HtmlDocConfig} so the HTML view gets nav links + an aside (the built-in "view").
 * 	<li>Exposes an auto-generated Swagger / OPTIONS page (the <c>api</c> nav link).
 * 	<li>{@link #greet(String, boolean)} demonstrates {@code @Path} / {@code @Query} binding.
 * 	<li>Reads one value from a YAML config file (<c>my-app.yaml</c>) to show config wiring.
 * </ul>
 */
@Rest(
	path="/helloWorld",
