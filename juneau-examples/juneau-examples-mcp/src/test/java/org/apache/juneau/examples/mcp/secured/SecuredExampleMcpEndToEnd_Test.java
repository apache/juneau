 ...
 */
@SuppressWarnings({
	"resource" // server is opened in setUp()/closed in tearDown(); getAuthServer() aliases are owned by server (or by an enclosing try-with-resources standalone instance) and are not independent resources.
})
