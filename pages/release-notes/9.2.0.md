...
- **Hjson (Human JSON) Support**: New native serializer and parser for Hjson format with quoteless strings, multiline strings, comments, and optional commas — ideal for configuration files and hand-edited data
...
### Hjson (Human JSON) Support

- **New Hjson Serializer and Parser**: Added native support for Hjson (Human JSON), a syntax extension of JSON designed for human-friendly configuration files and hand-edited data.
  - **Media types**: Produces `application/hjson`; accepts `application/hjson`, `application/hjson+json`
  - **Hjson features**: Quoteless strings, multiline strings (`'''...'''`), comments (`#`, `//`, `/* */`), optional commas, trailing commas, unquoted keys, root-braceless objects (parse only)
  - **New classes**: `HjsonSerializer`, `HjsonParser`, `Hjson` (marshaller), `HjsonConfig`, `HjsonWriter`, `HjsonTokenizer`
  - **REST integration**: Hjson included in `BasicUniversalConfig`
  - **Readable and compact modes**: `Hjson.DEFAULT` (newlines between members) and `Hjson.DEFAULT_COMPACT` (single-line)
  - See [Hjson Basics](/docs/topics/Hjson) for full documentation.
...
- **[Shaded Artifacts Overview](/docs/topics/JuneauShaded)** - Complete guide with Bazel examples
