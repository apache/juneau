...
- **juneau-rest-client** - REST client API (transport-agnostic, no Apache HttpClient dependency)
...
<DependencyInfo artifact="juneau-shaded-all" bazel />
...
### For REST Client
- **Apache HttpClient 4.5.x** — bundled via `juneau-rest-client-classic` for the legacy client. The canonical NG `juneau-rest-client` artifact composes any `HttpTransport` (HC 4.5, HC 5, JDK `HttpClient`, OkHttp, Jetty client) — add the appropriate transport module separately if you target a non-HC-4.5 backend.
...
- **[REST Server Basics](/docs/topics/JuneauRestServer)** - Build REST APIs
- **[REST Client Basics](/docs/topics/JuneauRestClient)** - Consume REST APIs
- **[Marshalling Basics](/docs/topics/JuneauMarshall)** - Serialize/deserialize objects
- **[Microservice Basics](/docs/topics/JuneauMicroservice)** - Build microservices
- **[Spring Boot Integration](/docs/topics/SpringBootOverview)** - Use with Spring Boot
