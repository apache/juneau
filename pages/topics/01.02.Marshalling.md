...
JsonSerializer serializer = JsonSerializer
    ...
    .swaps(
        ...
        TemporalCalendarSwap.Rfc1123DateTime.class   // Override default ISO 8601 format for Calendars
    )
    .build();
```
...
Since 9.5.0, parsers and serializers can also enforce JSON-Schema constraints declared via
...
The Marshalling API also supports YAML serialization and parsing, implemented natively without any external
library dependencies.  The YAML serializer produces clean, human-readable block-style output using
