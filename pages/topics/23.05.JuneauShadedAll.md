...
- **juneau-rest-common-classic** - Common REST APIs for the classic (Apache HttpClient 4.5) client and server
...
- **juneau-rest-server-mcp** - Model Context Protocol (MCP) REST server endpoint (revision-neutral core)
...
- **juneau-bean-mcp-v20250618** - Model Context Protocol (MCP) `2025-06-18` beans
...
// Client-side consumption (NG client — no .json() shortcut, so wire the JSON serializer/parser explicitly)
...
## Example: Testing with the Mock Framework

The mock and assertion test utilities (`juneau-rest-mock`, `juneau-test`) are **not** bundled in this shaded artifact — add them as separate test-scoped dependencies to use `MockRestClient` and the fluent assertions:
...
### Serialization
- JSON, XML, HTML, MessagePack, CSV, SOAP, UON, OpenAPI, PlainText
...
- Fluent API
- Remote proxies
- Pluggable HTTP transports (JDK `HttpClient`, Apache HttpClient, OkHttp, Jetty client)
...
- **Size constraints** - Use `juneau-shaded-core` (2.0 MB) for core-only
