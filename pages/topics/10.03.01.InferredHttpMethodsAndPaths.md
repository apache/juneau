...
When the `method` and/or `path` values are not specified, their values are inferred from the Java method name.
...
:::note
If `method` and `path` are both specified, the Java method name can be anything.
:::