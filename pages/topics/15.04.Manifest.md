...
<node-1><java-class><a href="/site/apidocs/org/apache/juneau/commons/runtime/ManifestFile.html" target="_blank">ManifestFile</a></java-class> - A pre-parsed manifest file</node-1>
...
This method returns an instance of <a href="/site/apidocs/org/apache/juneau/commons/runtime/ManifestFile.html" target="_blank">ManifestFile</a> — a lean, immutable wrapper around a `java.util.jar.Manifest` that returns `Optional<String>` from its `get(...)` accessors.

:::tip Example
```java
ManifestFile manifestFile = Microservice.getInstance().getManifest();

String mainClass = manifestFile.get("Main-Class").orElse(null);
int myInt = manifestFile.get("My-Int").map(Integer::parseInt).orElse(123);
boolean myBoolean = manifestFile.get("My-Boolean").map(Boolean::parseBoolean).orElse(false);
```
:::

The manifest is also used for the <a href="/site/apidocs/org/apache/juneau/commons/svl/vars/ManifestFileVar.html" target="_blank">$MF</a> SVL variable.
