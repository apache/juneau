...
### Required
- **Apache HttpClient 4.5.x** - bundled via `juneau-rest-client-classic` for the legacy client. The canonical `juneau-rest-client` artifact itself has **no Apache HttpClient dependency** — it ships with a built-in `JavaHttpTransport` backed by the JDK's `java.net.http.HttpClient` and works out of the box on Java 11+.
