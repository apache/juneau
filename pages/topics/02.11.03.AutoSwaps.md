...
If you want to force a bean-like class to be serialized as a string, you can use the <a href="/site/apidocs/org/apache/juneau/annotation/Marshalled.html#as()" target="_blank">@Marshalled(as=STRING)</a> annotation on the class to force it to be serialized to a string using the `toString()` method.
...
For example, you could provide customized results based on the media type being produced (<a href="/site/apidocs/org/apache/juneau/MarshallingSession.html#getMediaType()" target="_blank">MarshallingSession.getMediaType()</a>).
