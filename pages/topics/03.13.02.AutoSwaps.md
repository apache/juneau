...
If your static factory method doesn't match any of those conventional names (e.g. `of(String)` or `create(String)`), you can use the <a href="/site/apidocs/org/apache/juneau/marshall/StringCtor.html" target="_blank">@StringCtor</a> annotation on the class to name the exact method to use, short-circuiting the name-based lookup above:
...
Classes without `@StringCtor` continue to fall back to the conventional-name lookup (and then the single-`String`-arg constructor) described above.

If you want to force a bean-like class to be serialized as a string, you can use the <a href="/site/apidocs/org/apache/juneau/marshall/Marshalled.html#as()" target="_blank">@Marshalled(as=STRING)</a> annotation on the class to force it to be serialized to a string using the `toString()` method.
...
- `public X swap(MarshallingSession)` method, where `X` is any serializable object.
- `public static MyPojo unswap(X)` method, where `X` is any serializable object.
- `public static MyPojo unswap(X,MarshallingSession)` method, where `X` is any serializable object.
...
- `public Map toMap(MarshallingSession)` method.
  
  Can be any type of map with string keys and object vals.

- `public JsonMap toMap(MarshallingSession)` method.
- `public static MyPojo fromMap(Map)` method.
  
  Can be any type of map with string keys and object vals.

- `public static MyPojo fromMap(JsonMap)` method.
- `public static MyPojo fromMap(Map,MarshallingSession)` method.
  
  Can be any type of map with string keys and object vals.

- `public static MyPojo fromMap(JsonMap,MarshallingSession)` method.

The `MarshallingSession` parameter allows you access to various information about the current serialization session.
For example, you could provide customized results based on the media type being produced (<a href="/site/apidocs/org/apache/juneau/marshall/MarshallingSession.html#getMediaType()" target="_blank">MarshallingSession.getMediaType()</a>).
...
- `public static T unswap(MarshallingSession, X)` method where `X` is the swap class type.
