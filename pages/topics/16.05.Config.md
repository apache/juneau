...
- Resolve a file first in the working directory, then in the classpath, using the following names:
  ...
  - Any `*.cfg` file that can be found in the working directory.
...
If no configuration file is found, an empty in-memory configuration is used.
...
`SYSTEM_DEFAULT` such as those extending <a href="/site/apidocs/org/apache/juneau/rest/server/servlet/BasicRestServlet.html" target="_blank">BasicRestServlet</a> or <a href="/site/apidocs/org/apache/juneau/rest/server/servlet/BasicRestResource.html" target="_blank">BasicRestResource</a>.
