...
public class XmlSerializerContext extends SerializerContext {
	...
	public XmlSerializerContext(BeanContext beanContext, SerializerProperties sp, XmlSerializerProperties xsp, ObjectMap op, Method javaMethod) {
		...
		if (op == null || op.isEmpty()) {
			...
			defaultNamespace = findDefaultNamespace(xsp.defaultNamespace);
			...
			defaultNamespace = findDefaultNamespace(op.containsKey(XML_defaultNamespaceUri) ? op.getString(XML_defaultNamespaceUri) : xsp.defaultNamespace);
	...
	public XmlSerializerWriter getWriter(Writer out) {
		...
		return new XmlSerializerWriter(out, isUseIndentation(), isTrimStrings(), getQuoteChar(), getRelativeUriBase(), getAbsolutePathUriBase(), isEnableNamespaces(), getDefaultNamespace());
	}
}
