...
public final class UonSerializerContext extends SerializerContext {
	...
	private final boolean simpleMode, useWhitespace, encodeChars, expandedParams;
	...
	final UonSerializerWriter getWriter(Writer out) {
		if (out instanceof UonSerializerWriter)
			return (UonSerializerWriter)out;
		return new UonSerializerWriter(out, useWhitespace, isSimpleMode(), isEncodeChars(), isTrimStrings(), getRelativeUriBase(), getAbsolutePathUriBase());
	}
	...
	final boolean shouldUseExpandedParams(Object value) {
		if (value == null || ! expandedParams)
			return false;
		ClassMeta<?> cm = getBeanContext().getClassMetaForObject(value).getFilteredClassMeta();
		if (cm.isArray() || cm.isCollection()) {
			if (expandedParams)
				return true;
		}
