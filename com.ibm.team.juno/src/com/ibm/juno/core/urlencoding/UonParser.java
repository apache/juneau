...
public class UonParser extends ReaderParser {
		...
		while (c != -1 && c != AMP) {
			...
			if (! isInEscape) {
				if (state == S1) {
					if (c == ')')
						return m;
					if ((c == '\n' || c == '\r') && ctx.isWhitespaceAware())
						skipSpace(r);
					else {
						r.unread();
						Object attr = parseAttr(r, ctx.isDecodeChars(), ctx);
						currAttr = ctx.trim((attr == null ? null : ctx.getBeanContext().convertToType(attr, keyType)));
						state = S2;
			...
			if (! isInEscape) {
				if (state == S1) {
					...
					if ((c == '\n' || c == '\r') && ctx.isWhitespaceAware())
						...
						currAttr = parseAttrName(ctx, r, ctx.isDecodeChars());
	...
	Object parseAttr(ParserReader r, boolean encoded, UonParserContext ctx) throws IOException, ParseException {
		...
		if (c == '$') {
			...
			if (f == 'b')
				...
				attr = parseAttrName(ctx, r, encoded);
		} else {
			attr = parseAttrName(ctx, r, encoded);
		}
		return attr;
	...
	String parseAttrName(UonParserContext ctx, ParserReader r, boolean encoded) throws IOException, ParseException {
				...
				if (! isInEscape) {
					if (c == '=' || c == -1) {
						if (c != -1)
							r.unread();
						String s = r.getMarked();
						return (s.equals("\u0000") ? null : ctx.trim(s));
					}
				}
				isInEscape = isInEscape(c, r, isInEscape);
			}
	...
	String parseString(ParserReader r, boolean isUrlParamValue, UonParserContext ctx) throws IOException, ParseException {
		...
		return (s == null || s.equals("\u0000") ? null : ctx.trim(s));
	}
	 ...
	 */
	static String parsePString(UonParserContext ctx, ParserReader r) throws IOException, ParseException {
		...
		while (c != -1) {
			...
			if (! isInEscape) {
				if (c == ')')
					return ctx.trim(r.getMarked(0, -1));
			}
			if (c == EQ)
				r.replace('=');
			isInEscape = isInEscape(c, r, isInEscape);
	...
	UonParserContext createParameterContext() {
