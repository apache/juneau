...
public class UrlEncodingParser extends UonParser {
		...
		while (c != -1) {
			...
			if (! isInEscape) {
				if (state == S1) {
					...
					currAttr = ctx.trim(ctx.getBeanContext().convertToType(attr, keyType));
					state = S2;
		...
		while (c != -1) {
			...
			if (! isInEscape) {
				if (state == S1) {
					...
					currAttr = parseAttrName(ctx, r, true);
					...
					if (c == -1 || c == '\u0001') {
						...
						if (! currAttr.equals("_class")) {
							...
							if (pMeta == null) {
								...
								if (ctx.shouldUseExpandedParams(pMeta)) {
									ClassMeta cm = pMeta.getClassMeta();
	...
	public <T> T parseParameter(CharSequence in, ClassMeta<T> type) throws ParseException {
		if (in == null)
			return null;
		UonParserContext uctx = createParameterContext();
