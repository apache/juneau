...
"""
Start Juneau Petstore (Spring Boot) Application

This script starts the Juneau petstore sample application using Spring Boot.
It runs the org.apache.juneau.petstore.springboot.App main class via the Spring Boot Maven plugin.
...
def main():
    """Start the petstore Spring Boot application."""
    print(f"{BLUE}🚀 Starting Juneau Petstore (Spring Boot)...{RESET}")
    ...
    petstore_dir = project_root / 'juneau-petstore' / 'juneau-petstore-springboot'

    if not petstore_dir.exists():
        print(f"{RED}[ERROR] Petstore Spring Boot directory not found: {petstore_dir}{RESET}")
        sys.exit(1)
    ...
    print(f"{BLUE}Main class: org.apache.juneau.petstore.springboot.App{RESET}")
