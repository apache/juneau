...
"""
Start Juneau REST Examples (Jetty) Application

This script starts the Juneau REST examples application using the embedded Jetty server.
It runs the org.apache.juneau.examples.rest.jetty.App main class using Maven's exec plugin.
...
def main():
    """Start the Jetty examples application."""
    print(f"{BLUE}🚀 Starting Juneau REST Examples (Jetty)...{RESET}")
    ...
    # Set working directory to the examples-rest-jetty module
    examples_dir = project_root / 'juneau-examples' / 'juneau-examples-rest-jetty'
    ...
    print(f"{BLUE}Main class: org.apache.juneau.examples.rest.jetty.App{RESET}")
    print(f"{BLUE}Default URL: http://localhost:10000{RESET}")
    print(f"\n{YELLOW}Press Ctrl+C to stop the server{RESET}\n")
    print("=" * 80)
    
    try:
        # Use Maven exec plugin to run the main class with proper classpath
        cmd = [
            'mvn',
            'exec:java',
            '-Dexec.mainClass=org.apache.juneau.examples.rest.jetty.App'
        ]
