...
`HashMap`, `TreeMap`) but using [JsonMap]({{API_DOCS}}/org/apache/juneau/collections/JsonMap.html) is recommended since it has many convenience methods for converting values to various types.
The same is true when parsing collections.
You can use any Collection (e.g.
`HashSet`, `LinkedList`) or array (e.g.
`Object[]`, `String[]`, `String[][]`) but using [JsonList]({{API_DOCS}}/org/apache/juneau/collections/JsonList.html) is recommended.
