...
By default, `Configs` use the [VarResolver.DEFAULT]({{API_DOCS}}/org/apache/juneau/svl/VarResolver.html#DEFAULT) variable resolver which provides support for the following variables and constructs:

<tree>
<node-0><java-class>[SystemPropertiesVar]({{API_DOCS}}/org/apache/juneau/svl/vars/SystemPropertiesVar.html) - `$S{key[,default]}`</java-class></node-0>
<node-0><java-class>[EnvVariablesVar]({{API_DOCS}}/org/apache/juneau/svl/vars/EnvVariablesVar.html) - `$E{key[,default]}`</java-class></node-0>
<node-0><java-class>[ConfigVar]({{API_DOCS}}/org/apache/juneau/config/vars/ConfigVar.html) - `$C{key[,default]}`</java-class></node-0>
</tree>
