...
What makes this possible is that method-level annotations such as `@RestOp` and parameter-level annotations such as
`@Query` are inherited from parent classes.
...
Note how we didn't need to use the `@Header` and `@Path` annotations in our implementation since the annotations were
