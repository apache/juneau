...
/**
 * Validates that {@link JettyServerComponent}'s auto-discovery loop honors the runtime-override
...
class RestPathsRuntimeOverride_JettyMount_Test extends TestBase {
	...
	//-----------------------------------------------------------------------------------------------------------------
	// a — getPaths() getter override beats @Rest(paths) at Jetty mount time
	//-----------------------------------------------------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------------------------------------------------
	// d — Empty getPaths() (clear) → no top-level mounts; falling back to annotation never fires
	//-----------------------------------------------------------------------------------------------------------------
	...
	void d01_emptyGetter_clearsAllMounts() throws Exception {
		...
		try {
			...
			// The default servlet may still claim "/" — assert specifically that the annotation path is absent.
		} finally {
			ms.stop();
		}
	}
