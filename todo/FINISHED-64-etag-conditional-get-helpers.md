# FINISHED-64: Conditional-GET / ETag / `If-Modified-Since` helpers on `RestResponse`

**Completed 2026-05-22.** Shipped the RFC 7232 conditional-request layer end-to-end: six fluent setters on `RestResponse` (`eTag(String)` / `eTag(EntityTag)`, `lastModified(Instant)` / `lastModified(ZonedDateTime)`, `cacheControl(String)` / `cacheControl(CacheControlBuilder)`), a new typed `CacheControlBuilder` in `juneau-rest-common` covering every standard directive (`public` / `private`, `no-cache` / `no-store` / `no-transform`, `must-revalidate` / `proxy-revalidate`, `immutable`, `max-age` / `s-maxage`, `stale-while-revalidate` / `stale-if-error`, plus arbitrary extensions) with a stable emission order, and `RestRequest.checkPreconditions(RestResponse)` returning `Optional<BasicHttpException>` per RFC 7232 §6 ordering (`If-Match` → `If-Unmodified-Since` → `If-None-Match` → `If-Modified-Since`) with strong/weak ETag matching per §2.3.2 and `GET`/`HEAD`-only honoring of `If-Modified-Since`. Coverage landed via `RestResponse_EtagHelpers_Test` (34 tests), `RestRequest_CheckPreconditions_Test` (33 tests covering the 16-cell precedence matrix plus weak/strong rules), and `RestEtag_RoundTrip_Test` (5 end-to-end MockRestClient round-trip tests). Docs landed via a new release-notes block under `### juneau-rest-server` in `juneau-docs/pages/release-notes/9.5.0.md`, a new topic page `juneau-docs/pages/topics/10.20b.RestServerConditionalGet.md`, and a sidebar entry at the 10.20b slot.
