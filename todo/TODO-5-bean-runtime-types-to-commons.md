...
The "incomplete-but-documented over broken-build" rule from Phase 5a still applies. When picking up the next slice of this work, **Step 5 is the recommended next checkpoint** — Step 4 dropped the constructor coupling but the `BeanMap.session` field is still read by `BeanPropertyMeta.add` / `BeanPropertyMeta.set` / `applyChildPropertiesFilter` (for `convertToType` / `JsonList(session)` / `JsonMap(session)` construction) and a transitional `BeanMap.getMarshallingSession()` accessor still survives. The next blocker is the `BeanRegistry` field on `BeanPropertyMeta` (which `findDictionaryName` reads for polymorphic dispatch) and the `bc` (`MarshallingContext`) field on `BeanPropertyMeta.Builder` (which is read by ~30 call sites inside `BeanPropertyMeta` for type narrowing, conversion, and bean-store lookups). Pulling those out is more invasive than Step 4 because the call-site count is high. Surprises for Step 5 surveyed during Step 4: (a) `BeanRegistry` is constructed inside `BeanMeta.builder.beanRegistry(...)` and stored on each `BeanPropertyMeta`; the `findDictionaryName(ClassMeta<?>)` path uses it for polymorphic dispatch when serializing nested beans — extracting this into a `MarshalledPropertyMeta` companion is the cleanest path, but lots of cross-references. (b) `BeanPropertyMeta.bc` (a `MarshallingContext`) is read by `applyChildPropertiesFilter`, `setPropertyValue`, `set`, `Builder.validate`, etc. — the obvious replacement is the new `BeanConfigContext` from Step 1, but a few call sites read marshalling-only settings (`isBeanMapPutReturnsOldValue`, `isIgnoreInvocationExceptionsOnGetters`, `getBeanRegistry`, `convertToType`/`convertToMemberType`) that aren't on `BeanConfigContext` — those need to stay marshalling-side or move to the side companion. (c) The `MarshallingSession` field on `BeanMap` itself — the `getBean()` constructor-arg conversion path uses `session.convertToType(rawVal, cm)` for read-only beans, which is the only direct dependency `BeanMap` has on the session for non-property-meta operations. Once `convertToType` is moved to a callback or BeanMap is refactored to defer that conversion to the marshalling layer, the session field on `BeanMap` can be deleted entirely.
