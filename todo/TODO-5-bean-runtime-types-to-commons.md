...
The "incomplete-but-documented over broken-build" rule from Phase 5a still applies. When picking up the next slice of this work, **Step 4 is the recommended next checkpoint** — Step 3 made `BeanPropertyMeta.get`/`set` raw (modulo installed callback), so the next blocker is the `MarshallingSession` back-pointer carried by `BeanMap` and the parallel swap-application sites currently inside `BeanMap` (none today on the read path — `BeanMap.get` just delegates to `BeanPropertyMeta.get` — but `BeanMap.put` will need to participate in the same callback story once the session back-pointer goes). Surprises for Step 4 surveyed during Step 3: (a) `BeanMap` holds a `MarshallingSession session` field that flows through `convertToType` / `convertToMemberType` paths inside `BeanPropertyMeta.add` and `setPropertyValue`; pulling the session pointer out of `BeanMap` means those conversion paths also need a session source (probably a `BiFunction` argument to `get`/`put`, or a transitional `BeanMap.getMarshallingSession()` that the marshalling layer still provides at `toBeanMap` time). (b) `BeanMap.of(T)` currently calls `MarshallingContext.DEFAULT_SESSION.toBeanMap(...)` — Step 4 needs to decide whether `BeanMap.of` survives in `commons.bean` or moves marshalling-side. (c) `BeanMapEntry`, `DelegateBeanMap`, and `BeanProxyInvocationHandler` all read `bm.getMarshallingSession()` or `meta.getMarshallingContext()` — Step 4 should sketch how those access marshalling state once the back-pointer is gone.
