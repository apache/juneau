...
- [TODO-89] `RateLimitGuard.Storage` snapshot SPI — surfaced during FINISHED-77's `BasicAdminResource` work. The `/admin/ratelimit` endpoint currently emits the rate-limit guard's static configuration only (window size, limit per principal, key extractor identity) because `RateLimitGuard.Storage` has no read-side / snapshot operation; operators can see "what the rate-limit policy is" but not "which buckets are currently throttled and at what fill level". Add a snapshot SPI to `RateLimitGuard.Storage` (something like `Map<String, BucketState> snapshot()` or a streaming variant for large stores), then enrich `BasicAdminResource#getRateLimit(...)` to include live bucket state alongside configuration. Keep the SPI optional — default in-memory storage implements it; custom Redis-backed / external storages can opt in. Acceptance: `/admin/ratelimit` returns both config + live bucket state on the default storage; backwards-compatible default `snapshot()` returns empty for storages that don't implement it. Plan file TBD.
