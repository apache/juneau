...
1. ~~**YAML serializer registration.**~~ **Resolved by TODO-81.** `BasicOpenApiResource` declares `@Rest(serializers={YamlSerializer.class})` on the mixin class. Under TODO-81's sub-`RestContext` model, this APPENDS to the host's inherited serializer set for mixin endpoints only — `/openapi.yaml` works without polluting the host's content-negotiation surface. Phase 0 still needs to verify which module ships `YamlSerializer` (could create a hard dependency from `juneau-rest-server` onto a YAML-bearing module). See `todo/TODO-81-mixin-sub-context-inheritance.md`.
...
- `todo/TODO-81-mixin-sub-context-inheritance.md` (hard dependency — must land first) — sub-`RestContext` per mixin with parent-linked inheritance for serializers/parsers/etc. `BasicOpenApiResource` uses the new model to declare YAML scoped to mixin endpoints only.
