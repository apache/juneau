...
- **TODO-105** — Container-test perf-signal hardening landed: `juneau-utest` now has explicit container-tag meta-annotations (`@SpringbootTest`, `@JettyMicroserviceTest`) on the 22 container-booting classes, Surefire executes core and container buckets separately, and `scripts/check-container-tags.py` is wired as a pre-push guard so new container tests cannot silently drift into the core timing bucket. `/push` now records per-execution timing history in `~/.cache/juneau-push-timings/<branch>.jsonl`, prints rolling-median delta reports via `scripts/push-timings.py`, and includes `scripts/surefire-rollup.py` for `core` / `container.springboot` / `container.jetty` bucketing from Surefire XML. `scripts/test.py --profile <module>` support also landed for one-shot JFR capture under `target/profile-results/`, with profiling findings intentionally deferred to TODO-999 per scope guard. See [`FINISHED-105-container-tag-perf-tracking.md`](FINISHED-105-container-tag-perf-tracking.md).
