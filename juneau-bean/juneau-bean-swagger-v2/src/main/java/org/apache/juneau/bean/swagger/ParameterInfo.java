...
public class ParameterInfo extends SwaggerElement {
	...
	public ParameterInfo set(String property, Object value) {
		...
		return switch (property) {
			...
			case PROP_examples -> setExamples(toType(value, Map.class));
	...
	public ParameterInfo setCollectionFormat(String value) {
		if (isStrict() && ! contains(value, VALID_COLLECTION_FORMATS))
			throw rex("Invalid value passed in to setCollectionFormat(String).  Value='%s', valid values=%s", value, Json5.of(VALID_COLLECTION_FORMATS));
	...
	public ParameterInfo setIn(String value) {
		if (isStrict() && ! contains(value, VALID_IN))
			throw rex("Invalid value passed in to setIn(String).  Value='%s', valid values=%s", value, Json5.of(VALID_IN));
		in = value;
	...
	public ParameterInfo setType(String value) {
		if (isStrict() && ! contains(value, VALID_TYPES))
			throw rex("Invalid value passed in to setType(String).  Value='%s', valid values=%s", value, Json5.of(VALID_TYPES));
		type = value;
		return this;
	}
