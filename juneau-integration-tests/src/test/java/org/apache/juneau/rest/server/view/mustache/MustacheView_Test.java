 ...
 *
 * <p>
 * Covers the immutable-value-class contract: {@link MustacheView#of(String) of(...)} validation,
...
class MustacheView_Test extends TestBase {
	...
	@Test void a02_ofRejectsNullTemplate() {
		assertThrows(IllegalArgumentException.class, () -> MustacheView.of(null));
	}

	@Test void a03_ofRejectsBlankTemplate() {
		assertThrows(IllegalArgumentException.class, () -> MustacheView.of(""));
		assertThrows(IllegalArgumentException.class, () -> MustacheView.of("   "));
	}
	...
	@Test void b04_attrRejectsNullValue() {
		// mustache.java renders null scope bindings as empty strings; reject at build-time so
		// the mistake surfaces at the call site rather than as a silently empty template
	...
	@Test void e01_toStringIncludesTemplateName() {
		var v = MustacheView.of("hello").attr("k", "v").header("H", "1");
		var s = v.toString();
