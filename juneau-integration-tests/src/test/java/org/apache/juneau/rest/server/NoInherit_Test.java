...
class NoInherit_Test extends TestBase {
	...
	private static RestContext restContext(Class<? extends BasicRestResource> c) throws Exception {
		...
		return new RestContext(new RestContext.Args(c, parentContext, null, () -> o, "", null, null, null, RestContext.ContextKind.ROOT)).postInit().postInitChildFirst();
	}
	...
	void a06_restOp_noInherit_defaultCharset_skipsContextCharset() throws Exception {
		...
		var ctx = new RestContext(new RestContext.Args(CharsetNoInheritPlain.class, null, null, () -> o, "", null, null, null, RestContext.ContextKind.ROOT)).postInit().postInitChildFirst();
	...
	void a07_restOp_withoutNoInherit_defaultCharset_inheritsContext() throws Exception {
		...
		var ctx = new RestContext(new RestContext.Args(CharsetInheritPlain.class, null, null, () -> o, "", null, null, null, RestContext.ContextKind.ROOT)).postInit().postInitChildFirst();
		var op = ctx.getRestOperations().getOpContexts().get(0);
	...
	void a09_restOp_withoutNoInherit_maxInput_inheritsContext() throws Exception {
		...
		var ctx = new RestContext(new RestContext.Args(MaxInheritPlain.class, null, null, () -> o, "", null, null, null, RestContext.ContextKind.ROOT)).postInit().postInitChildFirst();
		var op = ctx.getRestOperations().getOpContexts().get(0);
