...
/**
 * Validates {@link EchoMixin} mounted as a mixin via {@code @Rest(mixins=...)} on a vanilla
 * {@link RestServlet}.
 *
 * <p>
 * Cases covered:
...
class EchoMixin_AsMixin_Test extends TestBase {
	...
	@Test void b03_authorizationHeaderRedacted() throws Exception {
		...
		Assertions.assertTrue(body.contains(EchoMixin.REDACTED),
	...
	@Test void f01_builderRejectsNegativeBodyLimit() {
		Assertions.assertThrows(IllegalArgumentException.class,
			() -> EchoMixin.create().bodyLimit(-1L));
	}

	@Test void f02_builderRejectsBlankRedactHeader() {
		Assertions.assertThrows(IllegalArgumentException.class,
			() -> EchoMixin.create().redactHeader(""));
		Assertions.assertThrows(IllegalArgumentException.class,
			() -> EchoMixin.create().redactHeader(null));
	}

	@Test void f03_redactedHeadersLowerIsImmutable() {
		var r = EchoMixin.create().build();
		Assertions.assertThrows(UnsupportedOperationException.class,
			() -> r.getRedactedHeadersLower().add("X-Test"));
	}

	@Test void f04_defaultsExposedAsConstants() {
		Assertions.assertEquals(1_048_576L, EchoMixin.DEFAULT_BODY_LIMIT);
		Assertions.assertTrue(EchoMixin.DEFAULT_REDACTED_HEADERS.contains("Authorization"));
		Assertions.assertTrue(EchoMixin.DEFAULT_REDACTED_HEADERS.contains("Cookie"));
	}

	@Test void f05_noArgConstructorMatchesDefaultBuilder() {
		var r = new EchoMixin();
		Assertions.assertEquals(EchoMixin.DEFAULT_BODY_LIMIT, r.getBodyLimit());
		Assertions.assertTrue(r.getRedactedHeadersLower().contains("authorization"));
	}

	@Test void f06_redactedHeadersFiltersNullAndBlank() {
		var r = EchoMixin.create()
	...
	@Test void f07_redactedHeadersNullVarargsClears() {
		var r = EchoMixin.create().redactedHeaders((String[]) null).build();
