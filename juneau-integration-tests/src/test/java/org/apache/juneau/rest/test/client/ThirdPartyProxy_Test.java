...
class ThirdPartyProxy_Test extends TestBase {
	...
	void ea02_setWrongInt(Input input) {
		assertThrowsWithMessage(AssertionFailedError.class, "expected: <1> but was: <2>", ()->input.proxy.setInt(2));
	}
	...
	void ea09_setNullStringBad(Input input) {
		assertThrowsWithMessage(AssertionFailedError.class, "expected: <null> but was: <foo>", ()->input.proxy.setNullString("foo"));
	}

	@ParameterizedTest
	@MethodSource("input")
	void ea10_setInt3dArray(Input input) {
		assertDoesNotThrow(()->input.proxy.setInt3dArray(new int[][][]{{{1},null},null}, 1));
	}
	...
	public interface ThirdPartyProxy {
		...
		void setInt(@Content int x) throws AssertionFailedError;
