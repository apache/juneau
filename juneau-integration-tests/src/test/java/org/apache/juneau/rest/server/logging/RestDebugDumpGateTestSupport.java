 ...
 * {@code juneau-integration-tests} test sources) purely so the Spring Boot end-to-end debug tests can force both
 * gate states without mutating the process environment and without a system-property fallback. It exists only in
 * test sources, so application code cannot reach the seam through it.
 *
 * <p>
 * Duplicated (rather than reused) from the equivalent {@code juneau-rest-mock} test helper because
 * {@code juneau-integration-tests} depends on {@code juneau-rest-mock}'s main artifact only, not its test-jar, so
