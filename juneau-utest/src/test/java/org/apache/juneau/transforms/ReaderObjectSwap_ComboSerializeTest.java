...
public class ReaderObjectSwap_ComboSerializeTest extends ComboSerializeTest_Base {
	...
	private static final ComboSerialize_Tester<?>[] TESTERS = {
		tester(1, "PojoToSimpleReader", PojoToSimpleReader::new)
			...
			.msgPackT("666F6F")
			.build(),
		tester(2, "PojoToDynamicReader", ()->new PojoToDynamicReader("foo"))
			.json("foo-json")
			.jsonT("foo-json")
			.jsonR("foo-json")
			.json5("foo-json5")
			...
			.build(),
		tester(3, "SometimesSwappedBean1", ()->new SometimesSwappedBean1("foo"))
			.json("{\"f\":\"foo\"}")
			.jsonT("{\"f\":\"foo\"}")
			.jsonR("{\n\t\"f\": \"foo\"\n}")
			.json5("foo-application/json5")
			.json5T("foo-application/json5")
			.json5R("foo-application/json5")
			.xml("foo-text/xml")
			.xmlT("foo-text/xml")
			.xmlR("foo-text/xml\n")
			.xmlNs("foo-text/xml")
			...
			.build(),
		tester(4, "SometimesSwappedBean2", ()->new SometimesSwappedBean2("foo"))
		...
		tester(5, "BeanWithSwappedField", ()->new BeanWithSwappedField("x"))
			...
			.build(),
		tester(6, "BeanWithSwapped1dField", ()->new BeanWithSwapped1dField("x"))
			.json("{\"f\":[x1-json,x2-json,null]}")
			.jsonT("{\"f\":[x1-json,x2-json,null]}")
			.jsonR("{\n\t\"f\": [\n\t\tx1-json,\n\t\tx2-json,\n\t\tnull\n\t]\n}")
			.json5("{f:[x1-json5,x2-json5,null]}")
			.json5T("{f:[x1-json5,x2-json5,null]}")
			.json5R("{\n\tf: [\n\t\tx1-json5,\n\t\tx2-json5,\n\t\tnull\n\t]\n}")
		...
		tester(7, "BeanWithSwappedNullField", BeanWithSwappedNullField::new)
			...
			.msgPackT("80")
			.build(),
		tester(8, "BeanWithSwappedListField", ()->new BeanWithSwappedListField("x"))
			.json("{\"f\":[x1-json,x2-json,null]}")
			.jsonT("{\"f\":[x1-json,x2-json,null]}")
			.jsonR("{\n\t\"f\": [\n\t\tx1-json,\n\t\tx2-json,\n\t\tnull\n\t]\n}")
			.json5("{f:[x1-json5,x2-json5,null]}")
		...
		tester(9, "BeanWithSwappedMapField", ()->new BeanWithSwappedMapField("x"))
			...
			.jsonR("{\n\t\"f\": {\n\t\t\"foo\": x1-json,\n\t\t\"bar\": null,\n\t\tnull: x2-json\n\t}\n}")
			.json5("{f:{foo:x1-json5,bar:null,null:x2-json5}}")
			.json5T("{f:{foo:x1-json5,bar:null,null:x2-json5}}")
			.json5R("{\n\tf: {\n\t\tfoo: x1-json5,\n\t\tbar: null,\n\t\tnull: x2-json5\n\t}\n}")
		...
		tester(10, "BeanWithListBeanSwappedField", ()->new BeanWithListBeanSwappedField("x"))
			...
			.jsonR("{\n\t\"f\": [\n\t\t{\n\t\t\t\"f1\": x1a-json,\n\t\t\t\"f2\": [\n\t\t\t\tx2a-json,\n\t\t\t\tx2b-json,\n\t\t\t\tnull\n\t\t\t],\n\t\t\t\"f4\": [\n\t\t\t\tx4a-json,\n\t\t\t\tx4b-json,\n\t\t\t\tnull\n\t\t\t],\n\t\t\t\"f5\": {\n\t\t\t\t\"foo\": x5a-json,\n\t\t\t\t\"bar\": null,\n\t\t\t\tnull: x5c-json\n\t\t\t}\n\t\t},\n\t\tnull\n\t]\n}")
			.json5("{f:[{f1:x1a-json5,f2:[x2a-json5,x2b-json5,null],f4:[x4a-json5,x4b-json5,null],f5:{foo:x5a-json5,bar:null,null:x5c-json5}},null]}")
			.json5T("{f:[{f1:x1a-json5,f2:[x2a-json5,x2b-json5,null],f4:[x4a-json5,x4b-json5,null],f5:{foo:x5a-json5,bar:null,null:x5c-json5}},null]}")
			.json5R("{\n\tf: [\n\t\t{\n\t\t\tf1: x1a-json5,\n\t\t\tf2: [\n\t\t\t\tx2a-json5,\n\t\t\t\tx2b-json5,\n\t\t\t\tnull\n\t\t\t],\n\t\t\tf4: [\n\t\t\t\tx4a-json5,\n\t\t\t\tx4b-json5,\n\t\t\t\tnull\n\t\t\t],\n\t\t\tf5: {\n\t\t\t\tfoo: x5a-json5,\n\t\t\t\tbar: null,\n\t\t\t\tnull: x5c-json5\n\t\t\t}\n\t\t},\n\t\tnull\n\t]\n}")
		...
		tester(11, "BeanWithMapBeanSwappedField", ()->new BeanWithMapBeanSwappedField("x"))
			...
			.jsonR("{\n\t\"f\": {\n\t\t\"foo\": {\n\t\t\t\"f1\": x1a-json,\n\t\t\t\"f2\": [\n\t\t\t\tx2a-json,\n\t\t\t\tx2b-json,\n\t\t\t\tnull\n\t\t\t],\n\t\t\t\"f4\": [\n\t\t\t\tx4a-json,\n\t\t\t\tx4b-json,\n\t\t\t\tnull\n\t\t\t],\n\t\t\t\"f5\": {\n\t\t\t\t\"foo\": x5a-json,\n\t\t\t\t\"bar\": null,\n\t\t\t\tnull: x5c-json\n\t\t\t}\n\t\t},\n\t\t\"bar\": null,\n\t\tnull: {\n\t\t\t\"f1\": x1a-json,\n\t\t\t\"f2\": [\n\t\t\t\tx2a-json,\n\t\t\t\tx2b-json,\n\t\t\t\tnull\n\t\t\t],\n\t\t\t\"f4\": [\n\t\t\t\tx4a-json,\n\t\t\t\tx4b-json,\n\t\t\t\tnull\n\t\t\t],\n\t\t\t\"f5\": {\n\t\t\t\t\"foo\": x5a-json,\n\t\t\t\t\"bar\": null,\n\t\t\t\tnull: x5c-json\n\t\t\t}\n\t\t}\n\t}\n}")
			.json5("{f:{foo:{f1:x1a-json5,f2:[x2a-json5,x2b-json5,null],f4:[x4a-json5,x4b-json5,null],f5:{foo:x5a-json5,bar:null,null:x5c-json5}},bar:null,null:{f1:x1a-json5,f2:[x2a-json5,x2b-json5,null],f4:[x4a-json5,x4b-json5,null],f5:{foo:x5a-json5,bar:null,null:x5c-json5}}}}")
			.json5T("{f:{foo:{f1:x1a-json5,f2:[x2a-json5,x2b-json5,null],f4:[x4a-json5,x4b-json5,null],f5:{foo:x5a-json5,bar:null,null:x5c-json5}},bar:null,null:{f1:x1a-json5,f2:[x2a-json5,x2b-json5,null],f4:[x4a-json5,x4b-json5,null],f5:{foo:x5a-json5,bar:null,null:x5c-json5}}}}")
			.json5R("{\n\tf: {\n\t\tfoo: {\n\t\t\tf1: x1a-json5,\n\t\t\tf2: [\n\t\t\t\tx2a-json5,\n\t\t\t\tx2b-json5,\n\t\t\t\tnull\n\t\t\t],\n\t\t\tf4: [\n\t\t\t\tx4a-json5,\n\t\t\t\tx4b-json5,\n\t\t\t\tnull\n\t\t\t],\n\t\t\tf5: {\n\t\t\t\tfoo: x5a-json5,\n\t\t\t\tbar: null,\n\t\t\t\tnull: x5c-json5\n\t\t\t}\n\t\t},\n\t\tbar: null,\n\t\tnull: {\n\t\t\tf1: x1a-json5,\n\t\t\tf2: [\n\t\t\t\tx2a-json5,\n\t\t\t\tx2b-json5,\n\t\t\t\tnull\n\t\t\t],\n\t\t\tf4: [\n\t\t\t\tx4a-json5,\n\t\t\t\tx4b-json5,\n\t\t\t\tnull\n\t\t\t],\n\t\t\tf5: {\n\t\t\t\tfoo: x5a-json5,\n\t\t\t\tbar: null,\n\t\t\t\tnull: x5c-json5\n\t\t\t}\n\t\t}\n\t}\n}")
