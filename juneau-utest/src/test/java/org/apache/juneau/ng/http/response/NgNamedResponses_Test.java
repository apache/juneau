...
/**
 * Parametric tests over every concrete response class in {@code org.apache.juneau.ng.http.response}.
 *
 * <p>
...
class NgNamedResponses_Test extends TestBase {
	...
	static Stream<Class<?>> responseClasses() throws Exception {
		return NgPackageScanner.enumerateConcreteClasses("org.apache.juneau.ng.http.response", BasicHttpResponse.class)
			.stream()
	...
	void coverAllConstructorsAndAccessors(Class<?> cls) throws Exception {
		...
		var bodyCtor = findCtor(cls, org.apache.juneau.ng.http.HttpBody.class);
		if (bodyCtor != null) {
