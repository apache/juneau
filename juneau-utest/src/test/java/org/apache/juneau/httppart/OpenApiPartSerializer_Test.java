...
class OpenApiPartSerializer_Test extends TestBase {
	...
	@Test void c06_stringType_dateFormat() throws Exception {
		var ps = T_DATE;
		var in = opt("2012-12-21").filter(x1 -> ! isBlank(x1)).map(x -> GranularZonedDateTime.parse("2012-12-21").getZonedDateTime()).map(GregorianCalendar::from).orElse(null);
	...
	@Test void c07_stringType_dateTimeFormat() throws Exception {
		var ps = T_DATETIME;
		var in = opt("2012-12-21T12:34:56.789").filter(x1 -> ! isBlank(x1)).map(x -> GranularZonedDateTime.parse("2012-12-21T12:34:56.789").getZonedDateTime()).map(GregorianCalendar::from).orElse(null);
		assertTrue(serialize(ps, in).contains("2012"));
		assertEquals("null", serialize(ps, null));
	}
...
//f1=1,f2=2,f3=true|f1=x,f2=3,f3=false null null
	...
	@Test void h04_objectType_simpleProperties() throws Exception {
		...
		assertEquals(
			...
			serialize(ps, new H2("foo",foob,opt("2012-12-21T12:34:56Z").filter(x1 -> ! isBlank(x1)).map(x -> GranularZonedDateTime.parse("2012-12-21T12:34:56Z").getZonedDateTime()).map(GregorianCalendar::from).orElse(null),foob,foob,"foo",1,2,1.0,1.0,true,1))
		);
		assertEquals("", serialize(ps, new H2(null,null,null,null,null,null,null,null,null,null,null,null)));
		assertEquals("null", serialize(ps, null));
		assertEquals(
			"f01=foo,f02=Zm9v,f04=2012-12-21T12:34:56Z,f05=666F6F,f06=66 6F 6F,f07=foo,f08=1,f09=2,f10=1.0,f11=1.0,f12=true,f99=1",
			serialize(ps, JsonMap.of("f01","foo","f02",foob,"f04",opt("2012-12-21T12:34:56Z").filter(x11 -> ! isBlank(x11)).map(x2 -> GranularZonedDateTime.parse("2012-12-21T12:34:56Z").getZonedDateTime()).map(GregorianCalendar::from).orElse(null),"f05",foob,"f06",foob,"f07","foo","f08",1,"f09",2,"f10",1.0,"f11",1.0,"f12",true,"f99",1))
		);
	}
