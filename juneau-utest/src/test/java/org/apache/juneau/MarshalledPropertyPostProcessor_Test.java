...
class MarshalledPropertyPostProcessor_Test {
	...
	// Exercises the `if (b.swap == null)` short-circuit inside the @MarshalledProp foreach
	// (lines 120 / 132 / 144) — the @Swap-installed swap sets b.swap before the foreach finishes,
	// so the second-pass marshalledPropSwap call short-circuits.
	//------------------------------------------------------------------------------------------------------------------
	...
	@Test void h02_floatSwap_blankStringParsesToZero() throws Exception {
		// Boxed Float field with @MarshalledProp(floatFormat=NaN_AS_NULL) gets the swap factory;
		// passing a blank-string wire exercises the s.isBlank() true branch inside floatSwap.unswap.
	...
	// The xmlGregorianCalendarSwap is installed at line 113 BEFORE the @MarshalledProp foreach runs,
	// so the {@code if (b.swap == null)} short-circuits at lines 120 / 132 / 144 for the innerField,
	// getter, and setter scans respectively.
	//------------------------------------------------------------------------------------------------------------------
