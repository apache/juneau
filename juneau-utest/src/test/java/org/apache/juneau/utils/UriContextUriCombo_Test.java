...
class UriContextUriCombo_Test extends TestBase {
	...
	void a01_testAbsoluteAuthority(Data d) {
		assertEquals(d.r.eAbsoluteAuthority, d.in.uriContext.getAbsoluteAuthority(), mfs("{0}: testAbsoluteAuthority() failed", d.label));
	}

	@ParameterizedTest
	@MethodSource("data")
	void a02_testAbsoluteContext(Data d) {
		assertEquals(d.r.eAbsoluteContext, d.in.uriContext.getAbsoluteContextRoot(), mfs("{0}: testAbsoluteContext() failed", d.label));
	}

	@ParameterizedTest
	@MethodSource("data")
	void a03_testAbsoluteResource(Data d) {
		assertEquals(d.r.eAbsoluteResource, d.in.uriContext.getAbsoluteServletPath(), mfs("{0}: testAbsoluteResource() failed", d.label));
	}

	@ParameterizedTest
	@MethodSource("data")
	void a04_testAbsolutePath(Data d) {
		assertEquals(d.r.eAbsolutePath, d.in.uriContext.getAbsolutePathInfo(), mfs("{0}: testAbsolutePath() failed", d.label));
	}

	@ParameterizedTest
	@MethodSource("data")
	void a05_testRootRelativeContext(Data d) {
		assertEquals(d.r.eRootRelativeContext, d.in.uriContext.getRootRelativeContextRoot(), mfs("{0}: testRootRelativeContext() failed", d.label));
	}

	@ParameterizedTest
	@MethodSource("data")
	void a06_testRootRelativeResource(Data d) {
		assertEquals(d.r.eRootRelativeResource, d.in.uriContext.getRootRelativeServletPath(), mfs("{0}: testRootRelativeResource() failed", d.label));
	}

	@ParameterizedTest
	@MethodSource("data")
	void a07_testRootRelativePath(Data d) {
		assertEquals(d.r.eRootRelativePath, d.in.uriContext.getRootRelativePathInfo(), mfs("{0}: testRootRelativePath() failed", d.label));
	}
}