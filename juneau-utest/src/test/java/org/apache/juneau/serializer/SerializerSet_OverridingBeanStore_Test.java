...
/**
 * Verifies that {@link SerializerSet.Builder#overridingBeanStore(BeanStore)} threads the overlay
 * through to the builder's reflective bean lookups so test-time overrides win over the builder's
 * regular bean store entries.
 */
class SerializerSet_OverridingBeanStore_Test extends TestBase {
	...
	void a01_overlay_winsOverLocalRegistration() {
		...
		var b = SerializerSet.create(defaultStore).overridingBeanStore(overlay);
	...
	void a02_overlay_fallsThroughForUnsupportedTypes() {
		...
		// Overlay doesn't contain Marker, so we fall through to the builder's bean store entries.
		var resolved = b.beanStore().getBean(Marker.class).orElseThrow();
		assertEquals("production", resolved.tag(),
			"Empty overlay must fall through to the builder's regular bean store");
	}

	@Test
	void a03_build_succeedsWithOverlayInstalled() {
		var overlay = new TestBeanStore();
		var set = SerializerSet.create()
			.overridingBeanStore(overlay)
			.add(org.apache.juneau.json.JsonSerializer.class)
			.build();
		assertNotNull(set);
	}
}
