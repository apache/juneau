...
class SerializerProperties_ComboRoundTripTest extends ComboRoundTripTest_Base {
	...
	private static final ComboRoundTrip_Tester<?>[] TESTERS = {
		...
		tester(5, "SERIALIZER_sortMaps", Map.class, u(map("c","3","a","1","b","2")))
			...
			.build(),
		tester(6, "SERIALIZER_trimEmptyCollections", T5.class, new T5())
			.json5("{}")
			.json5T("{}")
			.json5R("{\n}")
			.json("{}")
			...
			.build(),
		tester(7, "SERIALIZER_trimEmptyMaps", T6.class, new T6())
			.json5("{}")
			.json5T("{}")
			.json5R("{\n}")
			.json("{}")
			...
			.build(),
		tester(8, "SERIALIZER_keepNullProperties", T7.class, new T7())
			.json5("{f:null}")
			.json5T("{f:null}")
			.json5R("{\n\tf: null\n}")
			.json("{\"f\":null}")
			...
			.build(),
		tester(9, "SERIALIZER_trimStrings", T8.class, new T8())
			.json5("{f:'foo'}")
			.json5T("{f:'foo'}")
			.json5R("{\n\tf: 'foo'\n}")
			.json("{\"f\":\"foo\"}")
			...
			.build(),
		tester(10, "SERIALIZER_uriContext/uriResolution/uriRelativity", T9.class, new T9())
			.json5("{f:'https://localhost:80/context/resource/foo'}")
			.json5T("{f:'https://localhost:80/context/resource/foo'}")
			...
			.postConvert(x -> new T9())
			.skipTest(x -> x.contains("parseRdf") || x.contains("verifyRdf"))
			.build(),
		tester(11, "WSERIALIZER_maxIndent/WSERIALIZER_useWhitespace", T10.class, new T10().init())
			.json5("{\n\tf1: 1,\n\tf2: {\n\t\tf1: 2,\n\t\tf2: {f1:3}\n\t}\n}")
			.json5T("{\n\tf1: 1,\n\tf2: {\n\t\tf1: 2,\n\t\tf2: {f1:3}\n\t}\n}")
			.json5R("{\n\tf1: 1,\n\tf2: {\n\t\tf1: 2,\n\t\tf2: {f1:3}\n\t}\n}")
			.json("{\n\t\"f1\": 1,\n\t\"f2\": {\n\t\t\"f1\": 2,\n\t\t\"f2\": {\"f1\":3}\n\t}\n}")
			...
			.build(),
		tester(12, "WSERIALIZER_quoteChar", T11.class, new T11())
			.json5("{_type:|T11|,f:[{f:[| foo |]}]}")
			.json5T("{t:|T11|,f:[{f:[| foo |]}]}")
			.json5R("{\n\t_type: |T11|,\n\tf: [\n\t\t{\n\t\t\tf: [\n\t\t\t\t| foo |\n\t\t\t]\n\t\t}\n\t]\n}")
