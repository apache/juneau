 ...
 *
 * <p>
 * The pre-fix implementation did {@code new File(rootDir.getAbsolutePath() + '/' + path)} and only
 * verified {@link File#exists()} — so {@code ..} segments, absolute paths, and symlinks pointing
 * outside the configured root could all read files the JVM had access to. Each test below asserts
 * the post-fix behavior: paths that escape the configured root are rejected with 403 (Forbidden);
 * paths that stay inside the root continue to work.
 *
 * <p>
 * The funnel for every public operation on this resource (view, download, delete, upload, info) is
 * the private {@code getFile(String)} method, so the fix is applied once and verified across each
...
class DirectoryResource_PathTraversal_Test extends TestBase {
	...
	static void setup() throws Exception {
		...
		// Best-effort symlinks. Skip via assumption on filesystems that don't support them.
		try {
	...
	/**
	 * Test resource with a no-arg constructor so {@link MockRestClient} can instantiate it via
	 * reflection. Points at the test's {@link #rootDir} and enables every operation so the boundary
	 * check is exercised on each surface (view, download, delete, upload, info).
	 */
	@Rest(
		allowedMethodParams="*"
	)
	...
	@Test void t08_absolutePathInRequest_isNotTreatedAsFilesystemAbsolute() throws Exception {
		// "/etc/passwd" arrives as a relative segment under root → 404 (no such file), or 403 on
		// platforms where it resolves absolute. Either is a non-leak outcome.
		try (var c = buildClient()) {
			@SuppressWarnings({
				"resource"  // Closeable resources in tests are intentionally unassigned; closing is handled by test infrastructure.
			})
	...
	@Test void t09_urlEncodedTraversal_doesNotLeak() throws Exception {
		// Container/HttpClient may URL-decode before our handler, or reject "%2e%2e" outright. Either
		// is acceptable as long as the outside-root secret is NOT returned.
		try (var c = buildClient()) {
			@SuppressWarnings({
				"resource"  // Closeable resources in tests are intentionally unassigned; closing is handled by test infrastructure.
			})
	...
	//-----------------------------------------------------------------------------------------------------------------
	// Upload / delete traversal (uploads + deletes enabled in TestDirResource)
	//-----------------------------------------------------------------------------------------------------------------
	...
	@Test void t10_uploadTraversal_does_not_create_outside_root() throws Exception {
		...
		try (var c = buildClient()) {
			...
			// Post-fix: boundary check fires first → 403 (or 404 legacy "must exist before PUT").
