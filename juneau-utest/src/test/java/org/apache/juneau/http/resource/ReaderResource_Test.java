...
class ReaderResource_Test extends TestBase {
	...
	@Test void a01_basic() throws Exception {
		var x = new ReaderResource(contentType("text/plain"), reader("foo"));
	...
	@Test void a02_caching() throws Exception {
		var x = new ReaderResource(null, reader("foo")).setCached();
		assertEquals("foo", toUtf8(x.getContent()));
		assertEquals("foo", toUtf8(x.getContent()));
	...
	@Test void a03_headers() {
		var x = new ReaderResource(null, reader("foo"))
			.addHeader("Foo", "bar")
			.addHeader("Foo", "baz")
			.setHeader("Qux", "quux");
	...
	@Test void a04_contentType() {
		...
		var x2 = new ReaderResource(null, reader("foo"))
	...
	@Test void a06_chunked() {
		...
		var x3 = new ReaderResource(null, reader("foo"))
			.setChunked(false);
	...
	@Test void a09_contentLength() {
		...
		var x2 = new ReaderResource(null, reader("foo")).setContentLength(3);
