...
public class Remote_HeaderAnnotation_Test {
	...
	public void b01_default_allowEmptyValue() throws Exception {
		...
		assertThrown(()->x.getX1("")).messages().contains("Empty value not allowed.");
		assertEquals("{x:'foo'}",x.getX2(null));
		assertEquals("{x:''}",x.getX2(""));
		assertEquals("{x:''}",x.getX3(null));
		assertThrown(()->x.getX3("")).messages().contains("Empty value not allowed.");
	...
	public void h01_required() throws Exception {
		...
		assertThrown(()->x.getX3(null)).messages().contains("Required value not provided.");
	}
	...
	public static interface J1 {
		@RemoteOp(path="/") String getX1(@Header(n="x",serializer=XPartSerializer.class) String b);
	}
	...
	public static interface K1 {
		...
		@RemoteOp(path="/") String getX2(@Request(serializer=XSerializer.class) K1a rb);
	}
	...
	public static interface K4 {
		...
		@RemoteOp(path="/") String getX2(@Request(serializer=XSerializer.class) K4a rb);
	}
