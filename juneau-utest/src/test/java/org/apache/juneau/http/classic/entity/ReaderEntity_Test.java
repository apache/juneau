...
class ReaderEntity_Test extends TestBase {
	...
	@Test void a04_asBytes_cached() throws Exception {
		var x = new ReaderEntity().setContent(reader()).setCached();
		assertTrue(x.isCached());
		// Call twice to verify cached path
		var bytes = x.asBytes();
		assertArrayEquals("hello world".getBytes(StandardCharsets.UTF_8), bytes);
		assertArrayEquals("hello world".getBytes(StandardCharsets.UTF_8), x.asBytes());
	}
	...
	@Test void a17_copy() throws Exception {
		var x = new ReaderEntity().setContent(reader()).setContentType("text/plain").setCached();
		var copy = x.copy();
	...
	@Test void a18_setChunked() {
		var x = new ReaderEntity().setContent(reader()).setChunked();
		assertTrue(x.isChunked());
	}

	@Test void a19_setChunked_bool() {
		var x = new ReaderEntity().setContent(reader()).setChunked(true);
		assertTrue(x.isChunked());
		x.setChunked(false);
	...
	@Test void a21_setCharset() throws Exception {
		var x = new ReaderEntity().setContent(reader()).setCharset(StandardCharsets.UTF_8);
		assertEquals("hello world", x.asString());
	}

	@Test void a22_setMaxLength() throws Exception {
		var x = new ReaderEntity().setContent(reader()).setMaxLength(5);
	...
	@Test void a25_setContentLength() {
		var x = new ReaderEntity().setContent(reader()).setContentLength(99L);
	...
	@Test void a29_asString_cached_maxLength() throws Exception {
		var x = new ReaderEntity().setContent(reader()).setMaxLength(5).setCached();
