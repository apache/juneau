...
public class RestClient_Config_OpenApi_Test {
	...
	public void a01_oapiFormat() throws Exception {
		client().oapiFormat(HttpPartFormat.UON).build().get("/checkQuery").queryData("Foo","bar baz").run().assertBody().asString().asUrlDecode().is("Foo='bar baz'");
	}
	...
	public void a02_oapiCollectionFormat() throws Exception {
		...
		x.get("/checkQuery").queryData("Foo",a).run().assertBody().asString().asUrlDecode().is("Foo=bar|baz");
		x.post("/checkFormData").formData("Foo",a).run().assertBody().asString().asUrlDecode().is("Foo=bar|baz");
