...
public class RestClient_Config_RestClient_Test {
	...
	public void a12_partSerializer_partParser() throws Exception {
		...
		ABean b = x.get("/").header("Foo",bean).run().assertHeader("Foo").is("x{f:1}").getResponseHeader("Foo").as(ABean.class).get();
		assertEquals("{f:1}",b.toString());
		b = x.get().header("Foo",bean).run().assertHeader("Foo").is("x{f:1}").getResponseHeader("Foo").as(ABean.class).get();
		assertEquals("{f:1}",b.toString());
