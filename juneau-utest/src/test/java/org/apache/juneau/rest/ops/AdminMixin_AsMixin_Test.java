...
/**
 * Validates {@link AdminMixin} mounted as a mixin via {@code @Rest(mixins=...)} on a
...
class AdminMixin_AsMixin_Test extends TestBase {
	...
	public static class B extends RestServlet {
		...
		@Bean public AdminMixin admin() {
			return AdminMixin.create()
	...
	@Test void d01_builderRejectsBlankCacheFlushName() {
		Assertions.assertThrows(IllegalArgumentException.class,
			() -> AdminMixin.create().cacheFlush("", () -> {}));
		Assertions.assertThrows(IllegalArgumentException.class,
			() -> AdminMixin.create().cacheFlush(null, () -> {}));
	}

	@Test void d02_builderRejectsNullHook() {
		Assertions.assertThrows(IllegalArgumentException.class,
			() -> AdminMixin.create().cacheFlush("foo", null));
	}
	...
	@Test void d03_cacheFlushAllRoundTrips() {
		...
		hooks.put("a", r1);
		hooks.put("b", r2);
		var admin = AdminMixin.create().cacheFlushAll(hooks).build();
	...
	@Test void d04_threadNameExcludeReplaceList() {
		var admin = AdminMixin.create().threadNamePrefixExclude("foo-", "bar-").build();
	...
	@Test void d05_defaultExcludeListContainsKnownNoise() {
		Assertions.assertTrue(AdminMixin.DEFAULT_THREAD_NAME_PREFIX_EXCLUDE.contains("Reference Handler"));
		Assertions.assertTrue(AdminMixin.DEFAULT_THREAD_NAME_PREFIX_EXCLUDE.contains("jetty-"));
	}

	@Test void d06_noArgConstructorMatchesEmptyHooks() {
		var r = new AdminMixin();
		Assertions.assertTrue(r.getCacheFlushHooks().isEmpty());
		Assertions.assertEquals(AdminMixin.DEFAULT_THREAD_NAME_PREFIX_EXCLUDE,
	...
	@Test void d08_cacheFlushAllNullIsNoOp() {
		var admin = AdminMixin.create().cacheFlushAll(null).build();
		Assertions.assertTrue(admin.getCacheFlushHooks().isEmpty());
	}

	@Test void d09_threadNamePrefixExcludeNullClearsList() {
		var admin = AdminMixin.create().threadNamePrefixExclude((String[]) null).build();
	...
	@Test void d10_threadNamePrefixExcludeFiltersNullAndEmpty() {
		var admin = AdminMixin.create().threadNamePrefixExclude("good-", null, "", "also-").build();
