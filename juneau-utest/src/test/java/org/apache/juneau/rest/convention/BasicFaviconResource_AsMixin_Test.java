...
/**
 * Validates {@link BasicFaviconResource} mounted as a mixin via {@code @Rest(mixins=...)} on a
 ...
 * 	<li>Importer's {@code @Bean BasicFaviconResource} factory overrides the default bytes.
 * 	<li>{@link BasicFaviconResource.Builder#classpath(String) classpath(...)} loads icon bytes
...
class BasicFaviconResource_AsMixin_Test extends TestBase {
	...
	/** Host with an importer-supplied {@code @Bean BasicFaviconResource} carrying custom bytes. */
	@Rest(mixins=BasicFaviconResource.class)
