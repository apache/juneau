 ...
 * {@link MustacheMixin#render render(...)}.
 *
 * <p>
 * Mirrors {@code JspMixin_PathTraversal_Test} and
 * {@code ThymeleafMixin_PathTraversal_Test}: the handler must reject any user-supplied
 ...
 * Mustache analog of TODO-97 / TODO-107 that lights up Jetty/Spring Boot end-to-end alongside
...
class MustacheMixin_PathTraversal_Test extends TestBase {
	...
	//-----------------------------------------------------------------------------------------------------------------
	// Baseline — well-formed requests reach the engine (real template renders since mustache.java
	...
	@Test void t02_directTraversal_returns403() throws Exception {
		...
		assertEquals(403, status, "GET /mustache/../secret must be rejected (escapes /mustache-templates/)");
	}

	@Test void t03_nestedTraversal_returns403() throws Exception {
		var status = c.get("/mustache/a/b/../../../secret").run().getStatusCode();
		assertEquals(403, status, "GET /mustache/a/b/../../../secret must be rejected");
	}

	@Test void t04_traversalToSibling_returns403() throws Exception {
		// /mustache/../views2/foo → after join: /mustache-templates/../views2/foo →
		// normalized: /views2/foo → does NOT start with /mustache-templates/ → 403.
		var status = c.get("/mustache/../views2/foo").run().getStatusCode();
	...
	@Test void t05_urlEncodedTraversal_doesNotEscape() throws Exception {
		...
		var status = c.get("/mustache/%2e%2e/secret").run().getStatusCode();
