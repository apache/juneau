...
/**
 * Validates {@link VersionMixin} mounted as a mixin via {@code @Rest(mixins=...)} on a
 ...
 * 	<li>Importer's {@code @Bean VersionMixin} factory drives the entries map (manifest
...
class VersionMixin_AsMixin_Test extends TestBase {
	...
	public static class C extends RestServlet {
		...
		@Bean public VersionMixin version() {
			...
			return VersionMixin.create().fromManifest(manifest).build();
		}
	}
	...
	@Test void d01_missingManifestGracefulFallback() {
		var v = VersionMixin.create()
			...
			.build();
		Assertions.assertEquals(VersionMixin.UNKNOWN, v.getInfoMap().get("name"),
			"missing manifest must yield (unknown) name");
		Assertions.assertEquals(VersionMixin.UNKNOWN, v.getInfoMap().get("version"),
