...
class RestClient_Body_Test extends TestBase {
	...
	@Test void a01_BasicHttpResource() throws Exception {
		...
		var x2 = stringResource("foo").setContentType("text/plain").setContentEncoding("identity");
		...
		var x3 = stringResource("foo").setContentType(contentType("text/plain")).setContentEncoding(contentEncoding("identity")).setChunked();
		...
		var x4 = stringResource("foo", contentType("text/plain")).setContentEncoding("identity");
		...
		var x5 = stringResource("foo").setHeader("Foo","bar").setHeaders(header("Baz","qux"));
		client().build().post("/",x5).run()
			.assertHeader("X-Foo").is("bar")
			.assertHeader("X-Baz").is("qux")
		;

		var x6 = stringResource("foo").addHeaders(header("Foo","bar"),header("Baz","qux"));
		client().build().post("/",x6).run()
			.assertHeader("X-Foo").is("bar")
			.assertHeader("X-Baz").is("qux")
		;

		var x7 = readerResource(reader("foo"));
	...
	@Test void a02_StringEntity() throws Exception {
		...
		var x2 = stringEntity("foo").setContentType("text/plain").setContentEncoding("identity");
		client().build().post("/",x2).run()
			.assertHeader("X-Content-Length").asInteger().is(3)
			.assertHeader("X-Content-Encoding").is("identity")
			.assertHeader("X-Content-Type").is("text/plain")
		...
		var x3 = stringEntity("foo").setContentType(contentType("text/plain")).setContentEncoding(contentEncoding("identity")).setChunked();
		client().build().post("/",x3).run()
			.assertHeader("X-Content-Length").isNull()  // Missing when chunked.
			.assertHeader("X-Content-Encoding").is("identity")
			.assertHeader("X-Content-Type").is("text/plain")
		...
		var x4 = stringEntity("foo", contentType("text/plain")).setContentEncoding("identity");
		client().build().post("/",x4).run()
			.assertHeader("X-Content-Length").asInteger().is(3)
			.assertHeader("X-Content-Encoding").is("identity")
			.assertHeader("X-Content-Type").is("text/plain")
		...
		var x7 = readerEntity(reader("foo"));
		client().build().post("/",x7).run().assertContent("foo");

		var x8 = readerEntity(reader("foo")).setCached();
		client().build().post("/",x8).run().assertContent("foo");
		client().build().post("/",x8).run().assertContent("foo");

		var x12 = stringEntity("foo");
		x12.assertString().is("foo");
		x12.assertBytes().asString().is("foo");
	}

	@Test void a03_SerializedHttpEntity() throws Exception {
		var js = JsonSerializer.DEFAULT;

		var x1 = serializedEntity(ABean.get(),null,null);
		client().build().post("/",x1).run()
		...
		var x2 = serializedEntity(ABean.get(),js,null);
		client().build().post("/",x2).run()
			.assertHeader("X-Content-Length").isNull()
			.assertHeader("X-Content-Encoding").isNull()
			.assertHeader("X-Content-Type").is("application/json")
