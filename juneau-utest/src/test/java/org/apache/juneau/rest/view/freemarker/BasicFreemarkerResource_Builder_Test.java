 ...
 * {@link BasicFreemarkerResource#applyTemplateSuffix applyTemplateSuffix} helper, the
 * {@link BasicFreemarkerResource#toResourceRoot toResourceRoot} helper, and the
 * {@link BasicFreemarkerResource#stripBasePath stripBasePath} helper.
 *
 * <p>
 * Mirrors the {@code BasicMustacheResource_Builder_Test} /
 * {@code BasicThymeleafResource_Builder_Test} shape so sibling view modules can follow the same
 * template.
 *
 * @since 9.5.0
 */
class BasicFreemarkerResource_Builder_Test extends TestBase {
	...
	@Test void a01_defaultBasePathIsRoot() {
		...
		assertEquals(BasicFreemarkerResource.DEFAULT_BASE_PATH, r.getBasePath());
		assertEquals("/", r.getBasePath());
	}

	@Test void a02_basePathSetterRoundTrips() {
		var r = BasicFreemarkerResource.create().basePath("/templates/").build();
	...
	@Test void a05_builderGetBasePathReadsBeforeBuild() {
		...
		assertEquals(BasicFreemarkerResource.DEFAULT_BASE_PATH, b.getBasePath());
	...
	@Test void a06_noArgConstructorUsesDefaults() {
		...
		assertEquals(BasicFreemarkerResource.DEFAULT_BASE_PATH, r.getBasePath());
		assertEquals(BasicFreemarkerResource.DEFAULT_TEMPLATE_SUFFIX, r.getTemplateSuffix());
		assertEquals(BasicFreemarkerResource.DEFAULT_CACHE_TEMPLATES, r.isCacheTemplates());
	}
	...
	@Test void a08_templateSuffixSetterRoundTrips() {
		var r = BasicFreemarkerResource.create().templateSuffix(".ftlh").build();
	...
	@Test void a11_cacheTemplatesSetterRoundTrips() {
		var r = BasicFreemarkerResource.create().cacheTemplates(false).build();
	...
	@Test void a13_defaultConfigurationIsLazyButFreshlyBuilt() {
		...
		// configured).
		var r = BasicFreemarkerResource.create().basePath("/freemarker-templates/").build();
	...
	@Test void c01_toResourceRootKeepsLeadingSlashTrimsTrailing() {
		assertEquals("/templates", BasicFreemarkerResource.toResourceRoot("/templates/"));
	}
	...
	@Test void c07_toResourceRootHandlesMultiSegmentBase() {
		assertEquals("/a/b/c", BasicFreemarkerResource.toResourceRoot("/a/b/c/"));
	}
	...
	@Test void d01_stripBasePathRemovesPrefix() {
		assertEquals("hello",
			BasicFreemarkerResource.stripBasePath("/templates/", "/templates/hello"));
	}
	...
	@Test void d06_stripBasePathHandlesMultiSegment() {
		assertEquals("admin/dashboard",
			BasicFreemarkerResource.stripBasePath("/templates/", "/templates/admin/dashboard"));
	}
