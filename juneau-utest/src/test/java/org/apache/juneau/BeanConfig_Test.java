...
class BeanConfig_Test extends TestBase {
	...
	@Test void a01_basic() {
		...
		assertEquals(pm1.size(), m1.size(), mfs("Bean Map size failed for: {0} / {1} / {2}", p1, pm1.size(), m1.size()));
		assertEquals(pm1.keySet(), m1.keySet(), mfs("Bean Map key set equality failed for: {0} / {1} / {2}", p1, pm1.keySet() , m1.keySet()));
		assertEquals(m1.keySet(), pm1.keySet(), mfs("Bean Map key set reverse equality failed for: {0} / {1} / {2}", p1, pm1.keySet(), m1.keySet()));
		assertEquals(pm1, m1, mfs("Bean Map equality failed for: {0} / {1} / {2}", p1, pm1, m1));  // NOSONAR
		...
		assertEquals(bm1.size(), m2.size(), mfs("Bean Adapter map's key set has wrong size: {0} / {1} / {2}", a, bm1.size(), m2.size()));
		...
		assertEquals(count, m2.size(), mfs("Iteration count over bean adpater key set failed: {0} / {1} / {2}", a, count, m2.size()));
		assertEquals(m2.keySet(), temp, mfs("Iteration over bean adpater key set failed: {0} / {1} / {2}", a, bm1.keySet(), m2.keySet()));
		assertNotNull(bc.toBeanMap(p2), mfs("Failed to identify class as bean type: {0}", p2.getClass()));

		var m5 = bc.toBeanMap(p2);
		@SuppressWarnings("cast")
		var es1 = (Set)m5.entrySet();

		assertEquals(es1, m3.entrySet(), mfs("Entry set equality failed: {0} / {1} / {2}", p2, es1, m3.entrySet()));
		assertEquals(m3.entrySet(), es1, mfs("Entry set reverse equality failed: {0} / {1} / {2}", p2, es1, m3.entrySet()));
		...
		assertEquals(count, m3.size(), mfs("Iteration count over bean adpater entry set failed: {0} / {1} / {2}", a, count, m3.size()));
		assertEquals(m3.entrySet(), temp, mfs("Iteration over bean adpater entry set failed: {0} / {1} / {2}", a, es1, m3.entrySet()));
	}
	...
	public static class Address {
		...
		@Override /* Overridden from Object */
		public String toString() {
			return mf("Address(street: {0}, city: {1}, state: {2}, zip: {3})", street, city, state, zip);
		}
	}
	...
	@Test void a05_proxyHandler() {
		...
		assertNotNull(bm1, mfs("Failed to obtain bean adapter for proxy: {0}", f1));

		var bm2 = session.newBeanMap(A.class);
		assertNotNull(bm2, mfs("Failed to create dynamic proxy bean for interface: {0}", A.class.getName()));
		...
		assertEquals(bm1, bm2, mfs("Failed equality test of dynamic proxies beans: {0} / {1}", bm1, bm2));
		assertEquals(bm2, bm1, mfs("Failed reverse equality test of dynamic proxies beans: {0} / {1}", bm1, bm2));
	}
