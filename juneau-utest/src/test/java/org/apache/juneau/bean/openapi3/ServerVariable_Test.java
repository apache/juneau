...
/**
 * Testcase for {@link ServerVariable}.
 */
class ServerVariable_Test extends TestBase {

	@Nested class A_basicTests extends TestBase {

		private static final BeanTester<ServerVariable> TESTER =
			testBean(
				bean()
					.setDefault("a")
					.setDescription("b")
			...
			.vals("a,b,[c]")
			.json("{'default':'a',description:'b','enum':['c']}")
			.string("{'default':'a','description':'b','enum':['c']}".replace('\'','"'))
		;
		...
		@Test void a07_keySet() {
			assertList(TESTER.bean().keySet(), "default", "description", "enum");
		}
	...
	@Nested class B_emptyTests extends TestBase {
		...
		private static final BeanTester<ServerVariable> TESTER =
			testBean(bean())
			.props("enum,default,description")
			.vals("<null>,<null>,<null>")
			.json("{}")
			.string("{}")
		;
	...
	@Nested class C_extraProperties extends TestBase {
		private static final BeanTester<ServerVariable> TESTER =
			...
			.props("default,description,enum{#{toString}},x1,x2")
			.vals("a,b,{[{c1},{c2}]},x1a,<null>")
			.json("{'default':'a',description:'b','enum':['c1','c2'],x1:'x1a'}")
			.string("{'default':'a','description':'b','enum':['c1','c2'],'x1':'x1a'}".replace('\'', '"'))
		;
		...
		@Test void c07_keySet() {
			assertList(TESTER.bean().keySet(), "default", "description", "enum", "x1", "x2");
		}

		@Test void c08_get() {
			assertMapped(
				TESTER.bean(), (obj,prop) -> obj.get(prop, Object.class),
				"default,description,enum{#{toString}},x1,x2",
				"a,b,{[{c1},{c2}]},x1a,<null>"
			);
		}
		...
		@Test void c10_nullPropertyValue() {
			var serverVariable = bean();
			assertThrows(IllegalArgumentException.class, ()->serverVariable.get(null));
			assertThrows(IllegalArgumentException.class, ()->serverVariable.get(null, String.class));
			assertThrows(IllegalArgumentException.class, ()->serverVariable.set(null, "a"));
		}
	}
	...
	private static ServerVariable bean() {
