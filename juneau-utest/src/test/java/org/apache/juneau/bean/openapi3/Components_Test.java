		...
		private static final BeanTester<Components> TESTER =
			testBean(
				bean()
					...
					.set("examples", m("b1", example().setSummary("b2")))
					.set("headers", m("c1", headerInfo(schemaInfo("c2"))))
					.set("links", m("d1", link().setOperationId("d2")))
					.set("parameters", m("e1", parameter("e2", "e3")))
					.set("requestBodies", m("f1", requestBodyInfo().setDescription("f2")))
					.set("responses", m("g1", response("g2")))
					.set("schemas", m("h1", schemaInfo().setType("h2")))
					.set("securitySchemes", m("i1", securitySchemeInfo("i2")))
					.set("x1", "x1a")
					.set("x2", null)
			)
