...
public final class RrpcInterfaceMethodMeta {
	...
	private static int findUrlParamIndex(Method method) {
		...
		for (var i = 0; i < params.length; i++) {
			if (params[i].getAnnotation(Url.class) != null) {
				if (index != -1)
					throw illegalArg("Method {0}.{1} declares more than one @Url parameter; at most one is allowed",
	...
	private static void validateMultipart(Method method, boolean multipart) {
		...
		if (multipart) {
			if (hasContentParam)
				throw illegalArg("Method {0}.{1} declares both @Multipart and @Content; a method is either multipart or single-@Content, not both", cn, mn);
			if (partCount == 0)
				throw illegalArg("Method {0}.{1} is annotated @Multipart but declares no @Part parameters", cn, mn);
		} else if (partCount > 0) {
			throw illegalArg("Method {0}.{1} declares @Part parameter(s) but is not annotated @Multipart", cn, mn);
		}
	}
