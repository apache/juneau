...
public class Remote_CommonInterfaces_Test {
	...
	public void e01_predefinedExceptions() {
		...
		assertThrown(() -> {x.badRequest(); return null;}).isType(BadRequest.class).contains(BadRequest.MESSAGE);
		assertThrown(() -> {x.conflict(); return null;}).contains(Conflict.MESSAGE);
		assertThrown(() -> {x.expectationFailed(); return null;}).contains(ExpectationFailed.MESSAGE);
		assertThrown(() -> {x.failedDependency(); return null;}).contains(FailedDependency.MESSAGE);
		assertThrown(() -> {x.forbidden(); return null;}).contains(Forbidden.MESSAGE);
		assertThrown(() -> {x.gone(); return null;}).contains(Gone.MESSAGE);
		assertThrown(() -> {x.httpVersionNotSupported(); return null;}).contains(HttpVersionNotSupported.MESSAGE);
		assertThrown(() -> {x.insufficientStorage(); return null;}).contains(InsufficientStorage.MESSAGE);
		assertThrown(() -> {x.internalServerError(); return null;}).contains(InternalServerError.MESSAGE);
		assertThrown(() -> {x.lengthRequired(); return null;}).contains(LengthRequired.MESSAGE);
		assertThrown(() -> {x.locked(); return null;}).contains(Locked.MESSAGE);
		assertThrown(() -> {x.loopDetected(); return null;}).contains(LoopDetected.MESSAGE);
		assertThrown(() -> {x.methodNotAllowed(); return null;}).contains(MethodNotAllowed.MESSAGE);
		assertThrown(() -> {x.misdirectedRequest(); return null;}).contains(MisdirectedRequest.MESSAGE);
		assertThrown(() -> {x.networkAuthenticationRequired(); return null;}).contains(NetworkAuthenticationRequired.MESSAGE);
		assertThrown(() -> {x.notAcceptable(); return null;}).contains(NotAcceptable.MESSAGE);
		assertThrown(() -> {x.notExtended(); return null;}).contains(NotExtended.MESSAGE);
		assertThrown(() -> {x.notFound(); return null;}).contains(NotFound.MESSAGE);
		assertThrown(() -> {x.notImplemented(); return null;}).contains(NotImplemented.MESSAGE);
		assertThrown(() -> {x.payloadTooLarge(); return null;}).contains(PayloadTooLarge.MESSAGE);
		assertThrown(() -> {x.preconditionFailed(); return null;}).contains(PreconditionFailed.MESSAGE);
		assertThrown(() -> {x.preconditionRequired(); return null;}).contains(PreconditionRequired.MESSAGE);
		assertThrown(() -> {x.rangeNotSatisfiable(); return null;}).contains(RangeNotSatisfiable.MESSAGE);
		assertThrown(() -> {x.requestHeaderFieldsTooLarge(); return null;}).contains(RequestHeaderFieldsTooLarge.MESSAGE);
		assertThrown(() -> {x.serviceUnavailable(); return null;}).contains(ServiceUnavailable.MESSAGE);
		assertThrown(() -> {x.tooManyRequests(); return null;}).contains(TooManyRequests.MESSAGE);
		assertThrown(() -> {x.unauthorized(); return null;}).contains(Unauthorized.MESSAGE);
		assertThrown(() -> {x.unavailableForLegalReasons(); return null;}).contains(UnavailableForLegalReasons.MESSAGE);
		assertThrown(() -> {x.unprocessableEntity(); return null;}).contains(UnprocessableEntity.MESSAGE);
		assertThrown(() -> {x.unsupportedMediaType(); return null;}).contains(UnsupportedMediaType.MESSAGE);
		assertThrown(() -> {x.upgradeRequired(); return null;}).contains(UpgradeRequired.MESSAGE);
		assertThrown(() -> {x.uriTooLong(); return null;}).contains(UriTooLong.MESSAGE);
		assertThrown(() -> {x.variantAlsoNegotiates(); return null;}).contains(VariantAlsoNegotiates.MESSAGE);
	}
	...
	public void e02_predefinedExceptions_customMessages() {
		...
		assertThrown(() -> {x.badRequest(); return null;}).contains("foo");
		assertThrown(() -> {x.conflict(); return null;}).contains("foo");
		assertThrown(() -> {x.expectationFailed(); return null;}).contains("foo");
		assertThrown(() -> {x.failedDependency(); return null;}).contains("foo");
		assertThrown(() -> {x.forbidden(); return null;}).contains("foo");
		assertThrown(() -> {x.gone(); return null;}).contains("foo");
		assertThrown(() -> {x.httpVersionNotSupported(); return null;}).contains("foo");
		assertThrown(() -> {x.insufficientStorage(); return null;}).contains("foo");
		assertThrown(() -> {x.internalServerError(); return null;}).contains("foo");
		assertThrown(() -> {x.lengthRequired(); return null;}).contains("foo");
		assertThrown(() -> {x.locked(); return null;}).contains("foo");
		assertThrown(() -> {x.loopDetected(); return null;}).contains("foo");
		assertThrown(() -> {x.methodNotAllowed(); return null;}).contains("foo");
		assertThrown(() -> {x.misdirectedRequest(); return null;}).contains("foo");
		assertThrown(() -> {x.networkAuthenticationRequired(); return null;}).contains("foo");
		assertThrown(() -> {x.notAcceptable(); return null;}).contains("foo");
		assertThrown(() -> {x.notExtended(); return null;}).contains("foo");
		assertThrown(() -> {x.notFound(); return null;}).contains("foo");
		assertThrown(() -> {x.notImplemented(); return null;}).contains("foo");
		assertThrown(() -> {x.payloadTooLarge(); return null;}).contains("foo");
		assertThrown(() -> {x.preconditionFailed(); return null;}).contains("foo");
		assertThrown(() -> {x.preconditionRequired(); return null;}).contains("foo");
		assertThrown(() -> {x.rangeNotSatisfiable(); return null;}).contains("foo");
		assertThrown(() -> {x.requestHeaderFieldsTooLarge(); return null;}).contains("foo");
		assertThrown(() -> {x.serviceUnavailable(); return null;}).contains("foo");
		assertThrown(() -> {x.tooManyRequests(); return null;}).contains("foo");
		assertThrown(() -> {x.unauthorized(); return null;}).contains("foo");
		assertThrown(() -> {x.unavailableForLegalReasons(); return null;}).contains("foo");
		assertThrown(() -> {x.unprocessableEntity(); return null;}).contains("foo");
		assertThrown(() -> {x.unsupportedMediaType(); return null;}).contains("foo");
		assertThrown(() -> {x.upgradeRequired(); return null;}).contains("foo");
		assertThrown(() -> {x.uriTooLong(); return null;}).contains("foo");
		assertThrown(() -> {x.variantAlsoNegotiates(); return null;}).contains("foo");
	}
