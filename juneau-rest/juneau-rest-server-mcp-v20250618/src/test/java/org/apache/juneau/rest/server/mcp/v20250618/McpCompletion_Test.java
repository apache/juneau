...
/**
 * Coverage for {@code completion/complete} dispatch on the {@code 2025-06-18} adapter: strict params
 * validation, neutral ref/lookup reuse, per-target completer invocation, empty-miss behavior, internal-error
 * mapping, and result cap/order/total/hasMore mapping.
 */
@SuppressWarnings({
...
class McpCompletion_Test {
	...
	private static JsonRpcRequest req(Object id, Object params) {
		return new JsonRpcRequest().setJsonrpc(McpProtocol.JSON_RPC_2_0).setId(id).setMethod(McpMethods.COMPLETION_COMPLETE).setParams(params);
	}
