 ...
 *
 * <p>
 * Covers the immutable-value-class contract: {@link ThymeleafView#of(String) of(...)} validation,
 * fluent {@link ThymeleafView#attr(String, Object) attr(...)} /
 * {@link ThymeleafView#attrs(Map) attrs(...)} /
 * {@link ThymeleafView#header(String, String) header(...)} chaining, and the {@link View}-interface
...
class ThymeleafView_Test extends TestBase {
	...
	@Test void a02_ofRejectsNullTemplate() {
		assertThrows(IllegalArgumentException.class, () -> ThymeleafView.of(null));
	}

	@Test void a03_ofRejectsBlankTemplate() {
		assertThrows(IllegalArgumentException.class, () -> ThymeleafView.of(""));
		assertThrows(IllegalArgumentException.class, () -> ThymeleafView.of("   "));
	}
	...
	@Test void b04_attrRejectsNullValue() {
		// Thymeleaf Context silently drops null bindings; reject at build-time so the mistake
		// surfaces at the call site rather than as a missing template variable downstream.
	...
	@Test void e01_toStringIncludesTemplateName() {
		var v = ThymeleafView.of("hello").attr("k", "v").header("H", "1");
		var s = v.toString();
