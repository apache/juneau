...
class BasicResource_Test extends TestBase {
	...
	@Test void a01_constructor_entity() throws Exception {
		var x = new BasicResource(new StringEntity(contentType("text/plain"), "foo"));
	...
	@Test void a02_constructor_copy() throws Exception {
		var src = new BasicResource(new StringEntity(contentType("text/plain"), "foo"))
			.setHeader("Foo", "bar");
		var x = new BasicResource(src);
		assertNotSame(src, x);
		assertEquals("foo", toUtf8(x.getContent()));
		assertEquals("text/plain", x.getContentType().getValue());
	...
	@Test void c01_setContent_value() throws Exception {
		var x = new BasicResource(new StringEntity(null, "old"))
			.setContent("new");
		assertEquals("new", toUtf8(x.getContent()));
	}

	@Test void c02_setContent_supplier() throws Exception {
		var x = new BasicResource(new StringEntity(null, null))
			.setContent(() -> "newish");
	...
	@Test void c03_setContentType_string_andContentType() {
		...
		var x2 = new BasicResource(new StringEntity(null, "foo"))
	...
	@Test void c06_chunked() {
		var x = new BasicResource(new StringEntity(null, "foo")).setChunked();
		assertTrue(x.isChunked());
		var x2 = new BasicResource(new StringEntity(null, "foo")).setChunked(false);
	...
	@Test void d01_setCached_andRepeatable() throws Exception {
		var x = new BasicResource(new StringEntity(null, "foo")).setCached();
		assertEquals("foo", toUtf8(x.getContent()));
		assertEquals("foo", toUtf8(x.getContent()));
	...
	@Test void d02_asBytes_asString() throws Exception {
		var x = new BasicResource(new StringEntity(null, "hello"));
	...
	@Test void d03_assertBytes_assertString() throws Exception {
		var x = new BasicResource(new StringEntity(null, "abc"));
	...
	@Test void d05_writeTo() throws Exception {
		var x = new BasicResource(new StringEntity(null, "writeMe"));
