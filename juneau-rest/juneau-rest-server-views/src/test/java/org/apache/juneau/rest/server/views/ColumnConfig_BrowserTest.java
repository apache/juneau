 ...
 */
@EnabledIfSystemProperty(named=ColumnConfig_BrowserTest.GATE, matches="true",
	disabledReason="JS-execution harness is opt-in; run with `mvn -Pjs-tests -f juneau-rest/juneau-rest-server-views/pom.xml test`")
class ColumnConfig_BrowserTest extends TestBase {

	/** System property the {@code js-tests} profile sets to enable this class. */
	static final String GATE = "juneau.jsTests";

	private static final String XSS_NAME = "<img src=x onerror=\"window.__juneauChooserXss=1\">";
	private static final String XSS_LABEL = "<img src=x onerror=\"window.__juneauChooserLabelXss=1\">";
	...
	static void probe() throws Exception {
		...
		var harness = Path.of(requiredProperty("juneau.jsTests.harness")).getParent().resolve("column-config.cjs");
		...
		var fixture = "<!DOCTYPE html><html><head><meta charset=\"utf-8\"></head><body>\n<script>\n"
			...
			+ "\n</script></body></html>";
		var fixtureFile = Files.createDirectories(dir.resolve("fixtures")).resolve("column-config.html");
	...
	private static String run(Path dir, Path harness, Path fixture) throws Exception {
		...
		var stdout = dir.resolve("column-config-stdout.json");
		var stderr = dir.resolve("column-config-stderr.txt");
	...
	@Test void a01_runtimeLoadedWithNoScriptErrors() {
		assertEquals(Boolean.TRUE, report.get("hasConfig"), () -> "juneau-config.js did not load: " + report);
		assertEquals(List.of(), report.get("jsFailures"), () -> "the runtime logged errors: " + report.get("jsFailures"));
	}

	@Test void a02_hostileViewName_isLiteralText_doesNotExecute() {
		assertEquals(XSS_NAME, report.get("nameText"), () -> report.toString());
		assertEquals(0, ((Number) report.get("nameImgCount")).intValue(), () -> report.toString());
		assertEquals(Boolean.FALSE, report.get("xssNameFired"), () -> report.toString());
	}

	@Test void a03_hostileColumnLabel_isLiteralText_doesNotExecute() {
		assertEquals(XSS_LABEL, report.get("labelValue"), () -> report.toString());
		assertEquals(XSS_LABEL, report.get("headerText"), () -> report.toString());
		assertEquals(0, ((Number) report.get("headerImgCount")).intValue(), () -> report.toString());
		assertEquals(Boolean.FALSE, report.get("xssLabelFired"), () -> report.toString());
		assertEquals(Boolean.TRUE, report.get("sanitizedTitleBlank"), () -> report.toString());
	}
}
