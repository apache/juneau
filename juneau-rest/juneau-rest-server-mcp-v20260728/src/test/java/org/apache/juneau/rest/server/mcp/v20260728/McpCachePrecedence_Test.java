...
class McpCachePrecedence_Test {
	...
	private static McpServerConfig configFor() {
	...
	void a01_endpointOverridesDefault(String method, int ttl) {
		...
		var result = (CacheableResult<?>)send(configFor(), cache, req(1, method, null), hdrs(method, "")).getResult();
	...
	void a02_defaultThenNone(String method) {
		var withDefault = (CacheableResult<?>)send(configFor(),
		...
		var none = (CacheableResult<?>)send(configFor(), new McpCacheConfig(),
	...
	@Test void a03_emptyEndpointSuppressesDefaultWithoutMerge() {
		...
		var result = (ListToolsResult)send(configFor(), cache,
	...
	// -------- read precedence: template-backed outcomes ---------
	...
	private static McpResourceTemplateHandler template(String uriTemplate) {
		return new McpResourceTemplateHandler() {
			@Override public McpResourceTemplateSpec descriptor() {
				return new McpResourceTemplateSpec().setUriTemplate(uriTemplate).setName("t");
			}
			@Override public McpResourceOutcome read(String uri, Map<String,String> variables, BeanStore ctx) { return new McpResourceOutcome(); }
		};
	}

	private ReadResourceResult readTemplateBacked(McpCacheConfig cacheConfig, String uriTemplate, String concreteUri) {
		var config = new McpServerConfig().addResourceTemplate(template(uriTemplate));
		return (ReadResourceResult)send(config, cacheConfig,
			req(1, "resources/read", JsonMap.of("uri", concreteUri)), hdrs("resources/read", concreteUri)).getResult();
	}

	@Test void b04_readHintAppliedToTemplateBackedOutcomeByOriginalConcreteUri() {
		// The exact-per-resource override is keyed by the ORIGINAL concrete request URI, not the template
		// pattern, so it applies to a template-backed read exactly like an exact-resource read.
		var cache = cache().addResourceReadOverride("file:///a/one", hint(1)).setResourcesRead(hint(2)).setDefaultHint(hint(3));
		assertEquals(1, readTemplateBacked(cache, "file:///a/{x}", "file:///a/one").getTtlMs());
		assertEquals(2, readTemplateBacked(cache().setResourcesRead(hint(2)).setDefaultHint(hint(3)), "file:///a/{x}", "file:///a/one").getTtlMs());
		assertEquals(3, readTemplateBacked(cache().setDefaultHint(hint(3)), "file:///a/{x}", "file:///a/one").getTtlMs());
		assertNull(readTemplateBacked(cache(), "file:///a/{x}", "file:///a/one").getTtlMs());
	}
}
