...
@SuppressWarnings({
	...
	"unused"     // Allocation intentionally exercises constructor validation/side-effects.
})
class RestClient_Config_Context_Test extends TestBase {
	...
	@Test void a06_applyAnnotations() throws Exception {
		...
		var al = AnnotationWorkList.of(rstream(ClassInfo.of(A6c.class).getAnnotations()).filter(CONTEXT_APPLY_FILTER));
