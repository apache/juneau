 ...
 *
 * <p>
 * Per-operation overrides are available via {@link org.apache.juneau.rest.server.RestOp#metricName()}.
 * When a non-empty {@code metricName} is passed to {@code record(...)}, it takes precedence over the
 * recorder-level timer name. Per-op tags added via {@link org.apache.juneau.rest.server.RestOp#metricTags()}
...
public class MicrometerMetricsRecorder implements MetricsRecorder {
	...
	private static String exceptionTag(Throwable error) {
		return error == null ? NO_EXCEPTION_TAG : cns(error);
	}
