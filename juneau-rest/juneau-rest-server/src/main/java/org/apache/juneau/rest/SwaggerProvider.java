...
public class SwaggerProvider {
	...
	public SwaggerProvider(SwaggerProviderBuilder builder) {
		...
		this.vr = firstNonNull(builder.varResolver, bf.getBean(VarResolver.class).orElse(VarResolver.DEFAULT)).createSession();
		this.js = firstNonNull(builder.jsonSchemaGenerator, bf.getBean(JsonSchemaGenerator.class).orElse(JsonSchemaGenerator.DEFAULT)).createSession();
	...
	public Swagger getSwagger(RestContext context, Locale locale) throws Exception {
		...
		Messages mb = messages != null ? messages : Messages.create(c).build();
