...
public class ApiKeyGuard extends RestGuard {
	...
	private String readKey(RestRequest req) {
		return switch (source) {
			...
			case COOKIE -> readCookie(req, name);
		};
	}

	private static String readCookie(RestRequest req, String name) {
		...
		// Fallback: parse the raw Cookie header. Real Servlet containers populate getCookies()
		// from the Cookie header automatically, but mock containers (juneau-rest-mock) only return
		// what was explicitly set via .cookies(Cookie[]) — they don't parse the Cookie header.
		// Falling back to header parsing makes the guard work uniformly under both real and mock
		// containers without forcing callers to set cookies twice.
