...
/**
 * Validates {@link FluentRequestFormParamAssertion} tolerates a null {@code value} (TODO-286 #6 null-tolerance fix).
 *
 * <p>
 * A null {@link RequestFormParam} used to NPE in the constructor ({@code value.asString()}) and in every transform
 * method ({@code value.asBoolean()}, etc.).
 */
class FluentRequestFormParamAssertion_Test extends TestBase {

	@Test void a01_nullValue_chainedConstructor_doesNotThrow() {
		var a = new FluentRequestFormParamAssertion<>(null, (RequestFormParam)null, null);
		assertDoesNotThrow(a::isNull);
	}

	@Test void a02_nullValue_directConstructor_doesNotThrow() {
		var a = new FluentRequestFormParamAssertion<>((RequestFormParam)null, null);
		assertDoesNotThrow(a::isNull);
	}
