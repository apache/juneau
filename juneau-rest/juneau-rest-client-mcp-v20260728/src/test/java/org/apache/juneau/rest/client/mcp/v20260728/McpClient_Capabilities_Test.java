...
class McpClient_Capabilities_Test extends TestBase {

	@Test
	void a01_builderCapabilities_areStampedIntoRequestMeta() throws Exception {
		...
		HttpTransport transport = tReq -> {
			...
			var json = "{\"jsonrpc\":\"2.0\",\"id\":\"1\",\"result\":{\"resultType\":\"complete\",\"_meta\":{\"io.modelcontextprotocol/serverInfo\":{\"name\":\"s\",\"version\":\"1\"}}}}";
		...
		assertTrue(seenBody.get().contains("\"roots\""));
		assertTrue(seenBody.get().contains("\"sampling\""));
		assertTrue(seenBody.get().contains("\"experimental\""));
		assertTrue(seenBody.get().contains("\"extensions\""));
	}
}
