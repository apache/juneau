 ...
 */
final class Mcp20250618Wire {

	private Mcp20250618Wire() {}
	...
	static CallToolResult toWire(McpToolOutcome x) {
		...
		if (x.getContent() != null)
			r.setContent(x.getContent().stream().map(Mcp20250618Wire::toWire).toList());
		return r;
	}
	...
	static Prompt toWire(McpPromptSpec x) {
		...
		if (x.getArguments() != null)
			r.setArguments(x.getArguments().stream().map(Mcp20250618Wire::toWire).toList());
		return r;
	}
	...
	static GetPromptResult toWire(McpPromptOutcome x) {
		...
		if (x.getMessages() != null)
			r.setMessages(x.getMessages().stream().map(Mcp20250618Wire::toWire).toList());
		return r;
	}
	...
	static ReadResourceResult toWire(McpResourceOutcome x) {
		...
		if (x.getContents() != null)
			r.setContents(x.getContents().stream().map(Mcp20250618Wire::toWire).toList());
		return r;
	}
	...
	static Implementation serverInfo(McpServerConfig config) {
		if (config.getName() == null && config.getVersion() == null)
			return new Implementation()
				.setName(Mcp20250618Revision.DEFAULT_SERVER_NAME)
				.setVersion("unknown");
	...
	static McpToolOutcome toNeutral(CallToolResult x) {
		...
		if (x.getContent() != null)
			r.setContent(x.getContent().stream().map(Mcp20250618Wire::toNeutral).toList());
		return r;
	}
	...
	static McpPromptSpec toNeutral(Prompt x) {
		...
		if (x.getArguments() != null)
			r.setArguments(x.getArguments().stream().map(Mcp20250618Wire::toNeutral).toList());
		return r;
	}
	...
	static McpPromptOutcome toNeutral(GetPromptResult x) {
		...
		if (x.getMessages() != null)
			r.setMessages(x.getMessages().stream().map(Mcp20250618Wire::toNeutral).toList());
		return r;
	}
