...
/**
 * Direct unit tests for {@link Hc45RedirectCredentialGuard}'s edge cases that end-to-end redirect tests
 * ({@code ApacheHc45Transport_RedirectCredentials_Test}) never reach &mdash; a real HttpClient exchange
 * always supplies a resolvable {@link HttpHost} target, so a missing/unparsable target host is only
 * reachable by driving {@link Hc45RedirectCredentialGuard#process(HttpRequest, HttpContext)} directly
 * against a hand-built {@link HttpContext}. In the same package as the guard since it (and its members)
 * are package-private.
 */
class Hc45RedirectCredentialGuard_Test {

	private final Hc45RedirectCredentialGuard guard = new Hc45RedirectCredentialGuard();
