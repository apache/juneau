 ...
 */
@SuppressWarnings({
	"resource" // Transport instances are short-lived test fixtures; closing is irrelevant to these assertions.
})
class ApacheHc45Transport_Test {
	...
	void c03_post_nullContentTypeBody() throws Exception {
		// Exercises the ct == null branch in TransportBodyEntity constructor.
		var transport = ApacheHc45Transport.create();
		try (var client = RestClient.builder().transport(transport).rootUrl(rootUrl()).build()) {
