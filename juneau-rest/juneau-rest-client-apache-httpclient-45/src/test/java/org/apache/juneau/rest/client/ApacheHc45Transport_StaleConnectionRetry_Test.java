...
/**
 * Verifies that {@link ApacheHc45Transport} transparently retries a provably-idempotent request once when a pooled
...
class ApacheHc45Transport_StaleConnectionRetry_Test {
	...
	void a01_put_retriedOnceAfterStaleConnection() throws Exception {
		try (var server = new A01_StaleServer(1)) {
			...
			try (var client = RestClient.builder().transport(transport).rootUrl(server.rootUrl()).build()) {
				try (var response = client.put("/x").body(StringBody.of("data", "text/plain")).run()) {
	...
	private static final class A01_StaleServer implements AutoCloseable {
		...
		A01_StaleServer(int staleConnections) throws IOException {
			...
			var acceptThread = new Thread(this::acceptLoop, "hc45-stale-server");
		...
		// client has fully written its request before the connection is closed, guaranteeing a pre-response failure
		...
		private static void consumeRequest(InputStream in) throws IOException {
			...
			var contentLength = parseContentLength(headers.toString());
			for (var i = 0; i < contentLength && in.read() != -1; i++) {
				// Drain the request body so the exchange is complete before the connection is closed.
			}
		}
