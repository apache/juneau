...
/**
 * Verifies that {@link JavaHttpTransport} transparently retries a provably-idempotent request once when a pooled
 ...
 *
 * <p>
 * The positive case uses a body-less {@code DELETE} deliberately: the JDK {@link java.net.http.HttpClient} retries
 * body-less {@code GET} requests itself, but does <em>not</em> retry other idempotent methods such as {@code DELETE},
 * so a successful retry here proves the transport-level retry &mdash; not the underlying client's built-in handler
 * &mdash; is doing the work.
 */
@SuppressWarnings({
	"resource" // Transport/client instances are short-lived test fixtures.
})
class JavaHttpTransport_StaleConnectionRetry_Test {
	...
	@Test
	void a01_delete_retriedOnceAfterStaleConnection() throws Exception {
		try (var server = new A01_StaleServer(1)) {
			...
			try (var client = RestClient.builder().transport(transport).rootUrl(server.rootUrl()).build()) {
				try (var response = client.delete("/x").run()) {
	...
	private static final class A01_StaleServer implements AutoCloseable {
		...
		A01_StaleServer(int staleConnections) throws IOException {
			...
			var acceptThread = new Thread(this::acceptLoop, "java-stale-server");
