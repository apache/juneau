...
public class ClassInfo extends ElementInfo implements Annotatable {
	 ...
	 * 		<jv>ci</jv>.findAnnotations(MyAnnotation.<jk>class</jk>, SELF, PARENTS, PACKAGE);
	 * </p>
	 *
	 * <p>
	 * This does NOT include runtime annotations. For runtime annotation support, use
	 ...
	 * @param traversals The traversal settings defining what to search (e.g., SELF, PARENTS, PACKAGE).
			...
			.flatMap(traversal -> {
				if (traversal == AnnotationTraversal.SELF) {
					return Arrays.stream(inner.getDeclaredAnnotations())
						...
						.filter(a -> a.isType(type))
						.map(a -> (AnnotationInfo<A>)a);
				} else if (traversal == AnnotationTraversal.PARENTS) {
					return getParentsAndInterfaces().stream()
						.flatMap(x -> Arrays.stream(x.inner().getDeclaredAnnotations())
							.flatMap(a -> Arrays.stream(splitRepeated(a)))
							.map(a -> AnnotationInfo.of(x, a))
							.filter(a -> a.isType(type))
							.map(a -> (AnnotationInfo<A>)a));
				} else if (traversal == AnnotationTraversal.PACKAGE) {
					A packageAnn = getPackageAnnotation(type);
					return nn(packageAnn) ? Stream.of(AnnotationInfo.of(this, packageAnn)) : Stream.empty();
				}
	...
	/**
	 * Finds annotations on this class using the specified traversal settings in parent-first order.
	 *
	 * <p>
