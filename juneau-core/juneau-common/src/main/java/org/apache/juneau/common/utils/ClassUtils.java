...
public class ClassUtils {
	...
	public static Class<?> getProxyFor(Object o) {
		...
		if (s.contains("$$EnhancerBySpringCGLIB$$")) {
			...
			ClassInfo.of(c).getPublicMethods().stream().filter(m -> m.hasName("getTargetClass") && m.hasNoParameters() && m.hasReturnType(Class.class)).forEach(m -> safe(() -> v.set(m.invoke(o))));
