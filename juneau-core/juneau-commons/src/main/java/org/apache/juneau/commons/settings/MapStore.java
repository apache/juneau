 ...
 * will return a {@link PropertyLookupResult#present(Optional)} result with an empty optional.
 * This allows you to explicitly override system properties with null values.
 * Use {@link #unset(String)} if you want to remove a key entirely (so {@link #get(String)} returns {@link PropertyLookupResult#missing()}).
 *
 ...
 * 	<jc>// get() will now return present(Optional.empty()) for "system.property"</jc>
 *
...
public class MapStore implements PropertyStore {
	 ...
	 *
	 * <p>
	 * Returns {@link PropertyLookupResult#missing()} if the key doesn't exist in the map, or a present result
	 * (which may contain {@link Optional#empty()} if the value was explicitly set to <c>null</c>).
	 *
	 ...
	 * stores <c>Optional.empty()</c> in the map, which means {@link #get(String)} will return a present result
	 * with an empty optional. This allows you to explicitly override system properties with null values.
	 *
	 * @param key The property name.
	 ...
	 *
	 * <p>
	 * After calling this method, {@link #get(String)} will return {@link PropertyLookupResult#missing()} for
	 * the specified key, indicating that the key doesn't exist in this store.
	 *
