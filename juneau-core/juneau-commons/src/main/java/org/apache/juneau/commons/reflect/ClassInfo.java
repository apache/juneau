...
public class ClassInfo extends ElementInfo implements Annotatable, Type, Comparable<ClassInfo> {
	...
	protected ClassInfo(Class<?> inner, Type innerType) {
		...
		this.parentsAndInterfaces = mem(this::findParentsAndInterfaces);
		this.annotationInfos = mem(this::findAnnotations);
		this.recordComponents = mem(() -> opt(inner).filter(Class::isRecord).map(x -> u(l(x.getRecordComponents()))).orElse(liste()));
		this.genericInterfaces = mem(() -> opt(inner).map(x -> u(l(x.getGenericInterfaces()))).orElse(liste()));
		this.typeParameters = mem(() -> opt(inner).map(x -> u(l((TypeVariable<?>[])x.getTypeParameters()))).orElse(liste()));
		this.annotatedInterfaces = mem(() -> opt(inner).map(x -> u(l(x.getAnnotatedInterfaces()))).orElse(liste()));
		this.signers = mem(() -> opt(inner).map(Class::getSigners).map(x -> u(l(x))).orElse(liste()));
		this.publicMethods = mem(() -> opt(inner).map(x -> stream(x.getMethods()).filter(m -> neq(m.getDeclaringClass(), Object.class)).map(this::getMethod).sorted().toList()).orElse(liste()));
		...
		this.declaredConstructors = mem(() -> opt(inner).map(x -> stream(x.getDeclaredConstructors()).map(this::getConstructor).sorted().toList()).orElse(liste()));
		this.repeatedAnnotationMethod = mem(this::findRepeatedAnnotationMethod);
