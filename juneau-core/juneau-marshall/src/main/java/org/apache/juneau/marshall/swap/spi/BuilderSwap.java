...
public class BuilderSwap<T,B> {
	...
	public static BuilderSwap<?,?> findSwapFromObjectClass(MarshallingContext bc, Class<?> objectClass, Visibility cVis, Visibility mVis) {
		...
		bc.getAnnotationProvider().find(org.apache.juneau.commons.Builder.class, pci).stream().map(x -> x.inner().value()).filter(ClassUtils::isNotVoid).forEach(builderClass::set);
