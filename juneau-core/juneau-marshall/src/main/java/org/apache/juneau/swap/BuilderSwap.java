...
public class BuilderSwap<T,B> {
	...
	public static BuilderSwap<?,?> findSwapFromObjectClass(BeanContext bc, Class<?> objectClass, Visibility cVis, Visibility mVis) {
		...
		bc.getAnnotationProvider().find(org.apache.juneau.commons.annotation.Builder.class, pci).stream().map(x -> x.inner().value()).filter(ClassUtils::isNotVoid).forEach(builderClass::set);
