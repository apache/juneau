...
/**
 * Dynamically applies a {@link Uri @Uri} annotation to specified classes, methods, or fields.
 *
 * <p>
 * This annotation separates the <b>targeting</b> concern ({@link #on()}/{@link #onClass()}) from the
 * <b>content</b> concern ({@link #value()}), enabling {@link Uri @Uri} to be a pure data annotation
 * without marshall-specific application machinery.
 *
 * <h5 class='section'>Example:</h5>
 * <p class='bjava'>
 * 	<ja>@UriApply</ja>(on=<js>"com.example.Foo"</js>, value=<ja>@Uri</ja>)
...
@Target({ TYPE, METHOD })
@Retention(RUNTIME)
@Repeatable(UriApply.Array.class)
@ContextApply(UriApplyAnnotation.Applier.class)
public @interface UriApply {

	/**
	 * The {@link Uri @Uri} annotation to apply.
	 *
	...
	/**
	 * A collection of {@link UriApply @UriApply annotations}.
	 */
	@Documented
	@Target({ TYPE, METHOD })
	@Retention(RUNTIME)
