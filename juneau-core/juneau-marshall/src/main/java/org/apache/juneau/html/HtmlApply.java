...
/**
 * Dynamically applies a {@link Html @Html} annotation to specified classes, methods, or fields.
 *
 * <p>
 * This annotation separates the <b>targeting</b> concern ({@link #on()}/{@link #onClass()}) from the
 * <b>content</b> concern ({@link #value()}), enabling {@link Html @Html} to be a pure data annotation
 * without marshall-specific application machinery.
 *
 * <h5 class='section'>Example:</h5>
 * <p class='bjava'>
 * 	<ja>@HtmlApply</ja>(on=<js>"com.example.Foo"</js>, value=<ja>@Html</ja>(format=HtmlFormat.XML))
...
@Target({ TYPE, METHOD })
@Retention(RUNTIME)
@Repeatable(HtmlApply.Array.class)
@ContextApply(HtmlApplyAnnotation.Applier.class)
public @interface HtmlApply {

	/**
	 * The {@link Html @Html} annotation to apply.
	 *
	...
	/**
	 * A collection of {@link HtmlApply @HtmlApply annotations}.
	 */
	@Documented
	@Target({ TYPE, METHOD })
	@Retention(RUNTIME)
