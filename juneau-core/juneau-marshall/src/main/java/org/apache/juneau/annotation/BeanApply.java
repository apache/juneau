...
/**
 * Dynamically applies a {@link Bean @Bean} annotation to specified classes, methods, or fields.
 *
 * <p>
 * This annotation separates the <b>targeting</b> concern ({@link #on()}/{@link #onClass()}) from the
 * <b>content</b> concern ({@link #value()}), enabling {@link Bean @Bean} to be a pure data annotation
...
@Target({ TYPE, METHOD })
@Retention(RUNTIME)
@Repeatable(BeanApply.Array.class)
@ContextApply(BeanApplyAnnotation.Applier.class)
public @interface BeanApply {

	/**
	 * The {@link Bean @Bean} annotation to apply.
	 *
	...
	/**
	 * A collection of {@link BeanApply @BeanApply annotations}.
	 */
	@Documented
	@Target({ TYPE, METHOD })
	@Retention(RUNTIME)
