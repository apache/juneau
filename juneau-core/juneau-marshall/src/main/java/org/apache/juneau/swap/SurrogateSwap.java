...
public class SurrogateSwap<T,F> extends ObjectSwap<T,F> {
	...
	public static List<SurrogateSwap<?,?>> findObjectSwaps(Class<?> c, BeanContext bc) {
		...
		ci.getPublicConstructors().stream().filter(x -> bc.getAnnotationProvider().xfind(BeanIgnore.class, x.inner()).findAny().isEmpty() && x.hasNumParameters(1) && x.isPublic()).forEach(x -> {
