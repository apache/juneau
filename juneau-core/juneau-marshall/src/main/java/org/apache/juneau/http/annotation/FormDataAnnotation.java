...
public class FormDataAnnotation {
	...
	public static Value<String> findDef(ParameterInfo pi) {
		...
		rstream(pi.getAllAnnotationInfos(FormData.class)).map(AnnotationInfo::inner).filter(x -> isNotEmpty(x.def())).forEach(x -> n.set(x.def()));
		return n;
	}
	...
	public static Value<String> findName(ParameterInfo pi) {
		Value<String> n = Value.empty();
		rstream(pi.getAllAnnotationInfos(FormData.class)).map(AnnotationInfo::inner).filter(x -> isNotEmpty(x.value())).forEach(x -> n.set(x.value()));
		rstream(pi.getAllAnnotationInfos(FormData.class)).map(AnnotationInfo::inner).filter(x -> isNotEmpty(x.name())).forEach(x -> n.set(x.name()));
		return n;
	}
}