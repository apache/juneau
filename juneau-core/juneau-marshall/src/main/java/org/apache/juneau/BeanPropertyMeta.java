...
public class BeanPropertyMeta implements Comparable<BeanPropertyMeta> {
	...
	public Map<String,Object> getDynaMap(Object bean) throws IllegalArgumentException, IllegalAccessException, InvocationTargetException {
		if (isDyna) {
			...
			throw bex(beanMeta.c, "Getter or public field not defined on property ''{0}''", name);
		}
	...
	public Object getRaw(BeanMap<?> m, String pName) {
		try {
			...
			throw bex(e, beanMeta.c, "Exception occurred while getting property ''{0}''", name);
		}
	}
	...
	private Object setInner(BeanMap<?> m, String pName, Object value) throws BeanRuntimeException {
		try {
			...
			try {
				...
				if (isMap && (setter == null || ! pcInfo.isParentOf(vc))) {
					...
					if (! (value instanceof Map)) {
						if (value instanceof CharSequence)
							...
							throw bex(beanMeta.c, "Cannot set property ''{0}'' of type ''{1}'' to object of type ''{2}''", name, propertyClass.getName(), findClassName(value));
					}
				...
				throw bex(e, beanMeta.c, "Error occurred trying to set property ''{0}''", name);
			}
