...
public class BeanPropertyMeta implements Comparable<BeanPropertyMeta> {
	...
	public static class Builder {
		...
		boolean validate(BeanContext bc, BeanRegistry parentBeanRegistry, Map<Class<?>,Class<?>[]> typeVarImpls, Set<String> bpro, Set<String> bpwo) throws Exception {
			...
			if (nn(getter)) {
				List<Beanp> lp = list();
				bc.getAnnotationProvider().find(Beanp.class, getter).map(x -> x.inner()).filter(x -> true).forEach(x -> lp.add(x));
				if (rawTypeMeta == null)
				...
				bc.getAnnotationProvider().find(Swap.class, getter).map(x -> x.inner()).filter(x -> true).forEach(x -> swap = getPropertySwap(x));
			}

			if (nn(setter)) {
				List<Beanp> lp = list();
				bc.getAnnotationProvider().find(Beanp.class, setter).map(x -> x.inner()).filter(x -> true).forEach(x -> lp.add(x));
				if (rawTypeMeta == null)
				...
				bc.getAnnotationProvider().find(Swap.class, setter).map(x -> x.inner()).filter(x -> true).forEach(x -> swap = getPropertySwap(x));
			}
		...
		if (isDyna) {
			...
			if (nn(field))
				...
				throw new BeanRuntimeException(beanMeta.c,
					"Cannot set property ''{0}'' of type ''{1}'' to object of type ''{2}'' because no setter is defined on this property, and the existing property value is null", name,
