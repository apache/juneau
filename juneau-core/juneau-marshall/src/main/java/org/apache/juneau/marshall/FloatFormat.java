 ...
 * {@link org.apache.juneau.marshall.Marshalled#floatFormat()},
 * {@link org.apache.juneau.marshall.MarshalledProp#floatFormat()}, and
 * {@link org.apache.juneau.marshall.MarshalledConfig#floatFormat()} to control how non-finite
 ...
 * <ol>
 * 	<li>{@link org.apache.juneau.marshall.MarshalledProp#floatFormat() @MarshalledProp(floatFormat=…)} on the bean property.
 * 	<li>{@link org.apache.juneau.marshall.Marshalled#floatFormat() @Marshalled(floatFormat=…)} on the bean class.
 * 	<li>{@link org.apache.juneau.marshall.MarshalledConfig#floatFormat() @MarshalledConfig(floatFormat=…)} on
...
public enum FloatFormat {
	...
	public static Object format(float value, FloatFormat format) {
		...
		return switch (fmt) {
			...
			case NaN_AS_ERROR -> throw illegalArg("Non-finite float value {0} rejected by FloatFormat.NaN_AS_ERROR", value);
		};
	}
	...
	private static Object formatNonFinite(double value, FloatFormat format) {
		...
		return switch (fmt) {
			...
			case NaN_AS_ERROR -> throw illegalArg("Non-finite double value {0} rejected by FloatFormat.NaN_AS_ERROR", value);
		};
	}
	...
	public static <T extends Number> T parse(String value, FloatFormat format, Class<T> targetType) {
		...
		try {
			...
			throw illegalArg("Unsupported FloatFormat target type: {0}", targetType.getName());
		} catch (NumberFormatException e) {
			throw illegalArg("Invalid float value ''{0}'' for format {1}: {2}", value, format, e.getMessage());
		}
	}
