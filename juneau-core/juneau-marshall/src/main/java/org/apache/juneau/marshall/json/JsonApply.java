...
/**
 * Dynamically applies a {@link Json @Json} annotation to specified classes, methods, or fields.
 *
 * <p>
 * This annotation separates the <b>targeting</b> concern ({@link #on()}/{@link #onClass()}) from the
 * <b>content</b> concern ({@link #value()}), enabling {@link Json @Json} to be a pure data annotation
 * without marshall-specific application machinery.
 *
 * <h5 class='section'>Example:</h5>
 * <p class='bjava'>
 * 	<ja>@JsonApply</ja>(on=<js>"com.example.Foo"</js>, value=<ja>@Json</ja>(wrapperAttr=<js>"foo"</js>))
...
@Target({ TYPE, METHOD })
@Retention(RUNTIME)
@Repeatable(JsonApply.Array.class)
@ContextApply(JsonApplyAnnotation.Applier.class)
public @interface JsonApply {

	/**
	 * The {@link Json @Json} annotation to apply.
	 *
	...
	/**
	 * A collection of {@link JsonApply @JsonApply annotations}.
	 */
	@Documented
	@Target({ TYPE, METHOD })
	@Retention(RUNTIME)
