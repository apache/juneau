 ...
 * {@link Marshalled#currencyFormat()},
 * {@link MarshalledProp#currencyFormat()}, and
 * {@link MarshalledConfig#currencyFormat()} to control how {@link Currency}
 ...
 * <ol>
 * 	<li>{@link MarshalledProp#currencyFormat() @MarshalledProp(currencyFormat=…)} on the bean property.
 * 	<li>{@link Marshalled#currencyFormat() @Marshalled(currencyFormat=…)} on the bean class.
 * 	<li>{@link MarshalledConfig#currencyFormat() @MarshalledConfig(currencyFormat=…)} on
...
public enum CurrencyFormat {
	...
	public static Currency parse(String value, CurrencyFormat format, Locale locale) {
		...
		return switch (fmt) {
			case NOT_SET, ISO_CODE -> throw iaex("Invalid currency code '%s'", value);
	...
	private static Currency resolveAmbiguousMatches(List<Currency> matches, Locale loc, String original, String kind) {
		if (matches.isEmpty())
			throw iaex("Could not resolve currency %s '%s' in locale %s", kind, original, loc);
		if (matches.size() == 1)
		...
		throw iaex("Ambiguous currency %s '%s' in locale %s: matches %s", kind, original, loc, matches);
	}
