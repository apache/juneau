...
/**
 * Tests for {@link PrototextTokenizer} behavior, exercised through {@link PrototextParser}.
 */
class PrototextTokenizer_Test {
	...
	private static PrototextTokenizer tok(String s) {
		return new PrototextTokenizer(new java.io.StringReader(s));
	}

	@Test void b01_nullReaderInit() throws Exception {
		// Hits the null-reader branch in constructor (line 58).
		var t = new PrototextTokenizer(null);
		assertEquals(PrototextToken.TokenType.EOF, t.peek().type());
	}
	...
	@Test void b52_structuralPunctuation() throws Exception {
		...
		var t = tok("{}<>[]:,;");
		assertEquals(PrototextToken.TokenType.LBRACE, t.read().type());
		assertEquals(PrototextToken.TokenType.RBRACE, t.read().type());
		assertEquals(PrototextToken.TokenType.LANGLE, t.read().type());
		assertEquals(PrototextToken.TokenType.RANGLE, t.read().type());
		assertEquals(PrototextToken.TokenType.LBRACKET, t.read().type());
		assertEquals(PrototextToken.TokenType.RBRACKET, t.read().type());
		assertEquals(PrototextToken.TokenType.COLON, t.read().type());
		assertEquals(PrototextToken.TokenType.COMMA, t.read().type());
		assertEquals(PrototextToken.TokenType.SEMICOLON, t.read().type());
	}
