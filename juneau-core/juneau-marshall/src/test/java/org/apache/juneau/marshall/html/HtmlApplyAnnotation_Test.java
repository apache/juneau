...
class HtmlApplyAnnotation_Test extends TestBase {

	@Test void a01_empty_null() {
		assertTrue(HtmlApplyAnnotation.empty(null));
	}

	@Test void a02_empty_default() {
		assertTrue(HtmlApplyAnnotation.empty(HtmlApplyAnnotation.DEFAULT));
	}

	@Test void a03_empty_nonDefault() {
		var a = HtmlApplyAnnotation.create(String.class).build();
		assertFalse(HtmlApplyAnnotation.empty(a));
	}

	@Test void a04_create_and_build() {
		var a = HtmlApplyAnnotation.create("MyClass").build();
	...
	@Test void b05_applier_emptyOn_returnsEarly() {
		...
		var applier = new HtmlApplyAnnotation.Applier(vr);
		var b = MarshallingContext.DEFAULT.copy();
		var a = HtmlApplyAnnotation.DEFAULT;
