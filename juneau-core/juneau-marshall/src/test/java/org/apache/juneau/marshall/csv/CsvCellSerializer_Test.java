...
class CsvCellSerializer_Test extends TestBase {
	...
	@Test void c01_serializeMapEmpty() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.BASE64, "null");
		assertEquals("{}", cs.serialize(new LinkedHashMap<>(), session()));
	}

	@Test void c02_serializeMapSingleEntry() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.BASE64, "null");
		var m = new LinkedHashMap<String,Object>();
		m.put("k", "v");
		assertEquals("{k:v}", cs.serialize(m, session()));
	}

	@Test void c03_serializeMapMultipleEntries() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.BASE64, "null");
		var m = new LinkedHashMap<String,Object>();
		m.put("a", 1);
		m.put("b", 2);
		assertEquals("{a:1;b:2}", cs.serialize(m, session()));
	}

	@Test void c04_serializeMapNullKey() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.BASE64, "NIL");
		var m = new LinkedHashMap<String,Object>();
		m.put(null, "v");
		assertEquals("{NIL:v}", cs.serialize(m, session()));
	}

	@Test void c05_serializeMapNullValue() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.BASE64, "NIL");
		var m = new LinkedHashMap<String,Object>();
		m.put("k", null);
		assertEquals("{k:NIL}", cs.serialize(m, session()));
	}
	...
	@Test void d01_serializeCollectionEmpty() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.BASE64, "null");
		assertEquals("[]", cs.serialize(new ArrayList<>(), session()));
	}

	@Test void d02_serializeCollectionSingle() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.BASE64, "null");
		assertEquals("[a]", cs.serialize(List.of("a"), session()));
	}

	@Test void d03_serializeCollectionMultiple() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.BASE64, "null");
		assertEquals("[a;b;c]", cs.serialize(List.of("a", "b", "c"), session()));
	}

	@Test void d04_serializeCollectionNested() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.BASE64, "null");
		assertEquals("[[a;b];[c;d]]", cs.serialize(List.of(List.of("a", "b"), List.of("c", "d")), session()));
	}
	...
	@Test void e01_serializeObjectArrayEmpty() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.BASE64, "null");
		assertEquals("[]", cs.serialize(new Object[0], session()));
	}

	@Test void e02_serializeObjectArrayMultiple() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.BASE64, "null");
		assertEquals("[1;2;3]", cs.serialize(new Object[] { 1, 2, 3 }, session()));
	}
	...
	@Test void f01_intArray() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.BASE64, "null");
		assertEquals("[1;2;3]", cs.serialize(new int[] { 1, 2, 3 }, session()));
	}

	@Test void f02_intArrayEmpty() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.BASE64, "null");
		assertEquals("[]", cs.serialize(new int[0], session()));
	}

	@Test void f03_longArray() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.BASE64, "null");
		assertEquals("[1;2;3]", cs.serialize(new long[] { 1L, 2L, 3L }, session()));
	}

	@Test void f04_doubleArray() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.BASE64, "null");
		assertEquals("[1.5;2.5]", cs.serialize(new double[] { 1.5, 2.5 }, session()));
	}

	@Test void f05_floatArray() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.BASE64, "null");
		assertEquals("[1.5;2.5]", cs.serialize(new float[] { 1.5f, 2.5f }, session()));
	}

	@Test void f06_shortArray() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.BASE64, "null");
		assertEquals("[1;2;3]", cs.serialize(new short[] { (short) 1, (short) 2, (short) 3 }, session()));
	}

	@Test void f07_booleanArray() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.BASE64, "null");
		assertEquals("[true;false;true]", cs.serialize(new boolean[] { true, false, true }, session()));
	}

	@Test void f08_charArray() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.BASE64, "null");
		// chars are emitted as int code points
		assertEquals("[65;66;67]", cs.serialize(new char[] { 'A', 'B', 'C' }, session()));
	}

	@Test void f09_charArrayEmpty() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.BASE64, "null");
		assertEquals("[]", cs.serialize(new char[0], session()));
	}
	...
	@Test void g01_byteArrayBase64() {
		...
		var result = cs.serialize(bytes, session());
		// base64-encoded
	...
	@Test void g02_byteArraySemicolon() {
		...
		assertEquals("1;2;3", cs.serialize(bytes, sessionSemicolonBytes()));
	}

	@Test void g03_byteArraySemicolonNegative() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.SEMICOLON_DELIMITED, "null");
		var bytes = new byte[] { (byte) 0xFF, (byte) 0x80 };
		assertEquals("255;128", cs.serialize(bytes, sessionSemicolonBytes()));
	}

	@Test void g04_byteArrayEmpty() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.SEMICOLON_DELIMITED, "null");
		assertEquals("", cs.serialize(new byte[0], sessionSemicolonBytes()));
	}
	...
	@Test void h01_escapeNoSpecialChars() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.BASE64, "null");
		assertEquals("[hello]", cs.serialize(List.of("hello"), session()));
	}

	@Test void h02_escapeContainsSemicolon() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.BASE64, "null");
		assertEquals("[\"a;b\"]", cs.serialize(List.of("a;b"), session()));
	}
	...
	@Test void h08_escapeMapValueWithSpecialChar() {
		...
		var m = new LinkedHashMap<String,Object>();
		m.put("k", "a;b");
		assertEquals("{k:\"a;b\"}", cs.serialize(m, session()));
	}
	...
	@Test void h09_escapeMapKeyWithSpecialChar() {
		...
		var m = new LinkedHashMap<String,Object>();
		m.put("a:b", "v");
		assertEquals("{\"a:b\":v}", cs.serialize(m, session()));
	}
	...
	@Test void i01_beanSerialization() {
		var cs = new CsvCellSerializer(CsvByteArrayCellFormat.BASE64, "null");
		var s = cs.serialize(new Bean1("hi", 42), session());
