...
class UriApplyAnnotation_Test extends TestBase {

	@Test void a01_empty_null() {
		assertTrue(UriApplyAnnotation.empty(null));
	}

	@Test void a02_empty_default() {
		assertTrue(UriApplyAnnotation.empty(UriApplyAnnotation.DEFAULT));
	}

	@Test void a03_empty_nonDefault() {
		var a = UriApplyAnnotation.create(String.class).build();
		assertFalse(UriApplyAnnotation.empty(a));
	}

	@Test void a04_create_and_build() {
		var a = UriApplyAnnotation.create("MyClass").build();
	...
	@Test void b05_applier_emptyOn_returnsEarly() {
		...
		var applier = new UriApplyAnnotation.Applier(vr);
		var b = MarshallingContext.DEFAULT.copy();
		var a = UriApplyAnnotation.DEFAULT;
