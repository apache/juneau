...
class Common_UonTest extends TestBase {
	...
	@Test void a02_trimEmptyMaps() throws Exception {
		...
		var t2 = pe.read(r, B.class);
		assertEquals(json(t2), json(t1));

		s.trimEmptyMaps();
		r = s.build().write(t1);
		assertEquals("(f2=(f2a=null,f2b=(s2=s2)))", r);
		t2 = pe.read(r, B.class);
		assertNull(t2.f1);
	}
	...
	@Test void a04_trimEmptyArrays() throws Exception {
		...
		assertEquals("(f1=@(),f2=@(null,(s2=s2)))", r);
		var t2 = pe.read(r, D.class);
		assertEquals(json(t2), json(t1));

		s.trimEmptyCollections();
		r = s.build().write(t1);
		assertEquals("(f2=@(null,(s2=s2)))", r);
		t2 = pe.read(r, D.class);
		assertNull(t2.f1);
	}
	...
	@Test void a08_recursion() throws Exception {
		...
		s.detectRecursions();
		assertThrowsWithMessage(Exception.class, l("$R1", "$R2", "$R3"), ()->s.build().write(r1));
