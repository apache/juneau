...
class XmlValidatingDtd_Test extends TestBase {
	...
	@Test void a01_internalDtdRejectedWhenValidatingEnabled() {
		...
		// The DTD is not processed, so parsing must fail rather than expanding the nested entities.
		var e = assertThrows(Exception.class, () -> p.read(xml, Json5Map.class));

		// The expanded entity payload must never appear anywhere in the failure.
		assertFalse(e.toString().contains("aaaaaaaaaa"), "Internal DTD entity was expanded");
	}
	...
	@Test void a02_doctypeRejectedWhenValidatingEnabled() {
		...
		assertThrows(Exception.class, () -> p.read(xml, Json5Map.class));
	}
}
