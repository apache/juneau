...
class Jsonl_Test extends TestBase {
	...
	@Test void a01_of() throws Exception {
		...
		{ var sw = stringWriter(); Jsonl.DEFAULT.write(a, sw); assertEquals("\"foo\"", sw.toString().trim()); }
		assertTrue(Jsonl.of(b).contains("\"foo\":\"bar\""));
		var sw = new StringWriter();
		Jsonl.DEFAULT.write(b, sw);
	...
	@Test void a02_to() throws Exception {
		...
		assertEquals("foo", Jsonl.DEFAULT.read(stringReader(a), String.class));
		var m = (Map<String,String>) Jsonl.to(b, Map.class, String.class, String.class);
		assertEquals("bar", m.get("foo"));
		m = (Map<String,String>) Jsonl.DEFAULT.read(stringReader(b), Map.class, String.class, String.class);
		assertEquals("bar", m.get("foo"));
