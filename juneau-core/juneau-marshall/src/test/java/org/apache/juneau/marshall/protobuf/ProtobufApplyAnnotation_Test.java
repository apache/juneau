...
class ProtobufApplyAnnotation_Test extends TestBase {
	...
	@Test void a03_empty_nonDefault() {
		var a = ProtobufApplyAnnotation.create(String.class).build();
		assertFalse(ProtobufApplyAnnotation.empty(a));
	}

	@Test void a04_create_and_build() {
		var a = ProtobufApplyAnnotation.create("MyClass").build();
	...
	@Test void b05_applier_emptyOn_returnsEarly() {
		...
		var applier = new ProtobufApplyAnnotation.Applier(vr);
		var b = MarshallingContext.DEFAULT.copy();
		var a = ProtobufApplyAnnotation.DEFAULT;
