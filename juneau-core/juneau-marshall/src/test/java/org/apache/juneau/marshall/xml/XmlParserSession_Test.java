...
class XmlParserSession_Test extends TestBase {
	...
	@Test void c03_unknown_cdataAndPiAndComment() {
		...
		assertEquals("hello", P.read("<A><![CDATA[hello]]></A>", Object.class));
		assertEquals("hello", P.read("<A><?pi data?><!-- comment -->hello</A>", Object.class));
	}

	@Test void c05_unknown_duplicateKeysAccumulateIntoList() {
		// First duplicate → JsonList; third occurrence → MarshalledList.add() branch.
		var m = (Map) P.read("<A><x>1</x><x>2</x><x>3</x></A>", Object.class);
		var v = m.get("x");
	...
	@Test void c07_unknown_namedNestedElements() {
		...
		var m = (Map) P.read("<A><x _name='custom'>v</x></A>", Object.class);
	...
	@Test void c08_unknown_namespacedChildren() {
		...
		var m = (Map) P.read(xml, Object.class);
	...
	@Test void d01_malformed_unclosedTag() {
		assertThrows(ParseException.class, () -> P.read("<A><B>", Object.class));
	}
	...
	@Test void d03_malformed_notXml() {
		assertThrows(ParseException.class, () -> P.read("not-xml", Object.class));
	}

	@Test void d04_malformed_emptyInput() {
		assertThrows(ParseException.class, () -> P.read("", Object.class));
	}

	// -----------------------------------------------------------------------------------------------------------------
	// e01 - readIntoMap / Map parsing with typed keys/values + duplicate handling
	// -----------------------------------------------------------------------------------------------------------------
	...
	@Test void e02_genericMap_attributeAndDuplicates() {
		...
		var m1 = (Map) P.read("<object a='1'><b>2</b></object>", Map.class);
		assertEquals("1", m1.get("a"));
		assertEquals("2", m1.get("b"));
		var m2 = (Map) P.read("<object><x>1</x><x>2</x><x>3</x></object>", Map.class);
		assertEquals(3, ((List) m2.get("x")).size());
	}

	@Test void e03_emptyMap() {
		var m = P.read("<object/>", JsonMap.class);
		assertNotNull(m);
		assertTrue(m.isEmpty());
	}

	// -----------------------------------------------------------------------------------------------------------------
	// f01 - readIntoCollection / Array branches
	// -----------------------------------------------------------------------------------------------------------------

	@Test void f01_collection_strings() {
		var xml = "<array><string>a</string><string>b</string><string>c</string></array>";
		var l = P.read(xml, List.class);
	...
	@Test void f02_collection_typedList() {
		...
		var l = (LinkedList<Integer>) P.read(xml, LinkedList.class, Integer.class);
		assertEquals(2, l.size());
		assertEquals(Integer.valueOf(1), l.get(0));
	}

	@Test void f03_collection_emptyAndPrimArray() {
		var emptyArr = P.read("<array/>", String[].class);
		assertEquals(0, emptyArr.length);
		var arr = P.read("<array><number>1</number><number>2</number><number>3</number></array>", int[].class);
		assertArrayEquals(new int[] { 1, 2, 3 }, arr);
	}

	// -----------------------------------------------------------------------------------------------------------------
	// g01 - readAnything: scalar types and the type-attribute / element-name dispatch
	// -----------------------------------------------------------------------------------------------------------------
	...
	@Test void g01_nilTrueReturnsNull() {
		...
		var o = P.read("<x _type='null'/>", Object.class);
		assertNull(o);
	}

	@Test void g02_typeNumber() {
		var n = P.read("<x _type='number'>42</x>", Object.class);
		assertEquals(42, n);
	}

	@Test void g03_typeBooleanStringArrayObject() {
		assertEquals(Boolean.TRUE, P.read("<x _type='boolean'>true</x>", Object.class));
		assertEquals("hi", P.read("<x _type='string'>hi</x>", Object.class));
		var l = (List) P.read("<x _type='array'><string>a</string></x>", Object.class);
		assertEquals(1, l.size());
		var m = (Map) P.read("<x _type='object'><a>1</a></x>", Object.class);
		assertEquals("1", m.get("a"));
	}

	@Test void g04_targetCharacterAndOptional() {
		assertEquals(Character.valueOf('z'), P.read("<x>z</x>", Character.class));
		var o = (Optional<Integer>) P.read("<x>5</x>", Optional.class, Integer.class);
	...
	@Test void h01_preserveRoot_genericMap() {
		...
		var m = p.read("<wrap><a>1</a></wrap>", JsonMap.class);
	...
	@Test void i01_attrFormat() {
		...
		var bean = P.read(x, AttrBean.class);
	...
	@Test void i02_collapsedFormat() {
		...
		var bean = P.read(x, CollapsedBean.class);
	...
	@Test void i03_textFormat() {
		...
		var bean = P.read(x, TextBean.class);
	...
	@Test void i04_xmlTextFormat() {
		...
		var bean = P.read(x, XmlTextBean.class);
		assertEquals("aval", bean.a);
	...
	@Test void i05_elementsFormat() {
		...
		var bean = P.read(x, ElementsBean.class);
		assertEquals("aval", bean.a);
		assertNotNull(bean.b);
	...
	@Test void i06_attrsFormat() {
		...
		var bean = P.read(x, AttrsBean.class);
		assertEquals("foo", bean.a);
		assertEquals(42, bean.b);
	}

	// -----------------------------------------------------------------------------------------------------------------
	// j01 - Comment handling inside bean parsing (event == COMMENT branch in readIntoBean)
	// -----------------------------------------------------------------------------------------------------------------
	...
	@Test void j01_commentInsideBean() {
		// Comments between bean property elements should be ignored (event == COMMENT branch in readIntoBean).
		var x = "<object><!-- skip --><f1>hi</f1><f2>5</f2></object>";
		var bean = P.read(x, SimpleBean.class);
	...
	@Test void j02_unknownPropertyOnBean() {
		...
		assertThrows(ParseException.class, () -> P.read(x, SimpleBean.class));
	}
	...
	// -----------------------------------------------------------------------------------------------------------------
	// k01 - Sub-document parsing: readIntoMap with attribute on map-element
	// -----------------------------------------------------------------------------------------------------------------

	@Test void k01_mapAttributeBecomesEntryAndNestedMap() {
		// Map element with an attribute → attribute becomes a key (readIntoMap attr loop).
		// Nested object property becomes nested Map.
		var x = "<object x='val'><inner><a>1</a></inner></object>";
		var m = (Map) P.read(x, Map.class);
	...
	// -----------------------------------------------------------------------------------------------------------------
	// m01 - readIntoBean error and content-property combinations
	// -----------------------------------------------------------------------------------------------------------------

	@Test void m01_readIntoMap_facadeEntryPoint() {
		// XmlParser.readIntoMap dispatches into XmlParserSession.doReadIntoMap.
		var dest = new LinkedHashMap<String,Integer>();
		var x = "<object><a>1</a><b>2</b></object>";
		P.readIntoMap(x, dest, String.class, Integer.class);
		assertEquals(1, dest.get("a"));
		assertEquals(2, dest.get("b"));
	}

	@Test void m02_readIntoCollection_facadeEntryPoint() {
		// XmlParser.readIntoCollection dispatches into XmlParserSession.doReadIntoCollection.
		var dest = new ArrayList<String>();
		var x = "<array><string>x</string><string>y</string></array>";
		P.readIntoCollection(x, dest, String.class);
	...
	@Test void n01_mixedFormat_collection() {
		...
		var bean = P.read(x, MixedBean.class);
		assertEquals("aval", bean.a);
		assertNotNull(bean.b);
	...
	@Test void o01_preserveRoot_typedMap_and_objectType() {
		...
		var p = XmlParser.create().preserveRootElement().build();
		var m = (Map) p.read("<root><a>1</a></root>", HashMap.class);
		assertTrue(m.containsKey("root"));
		var o = p.read("<wrap _type='object'><a>1</a></wrap>", Object.class);
		assertTrue(o instanceof Map);
		assertTrue(((Map) o).containsKey("wrap"));
	}

	// -----------------------------------------------------------------------------------------------------------------
	// p01 - readText / TEXT_PWS content format (readText() workhorse)
	// -----------------------------------------------------------------------------------------------------------------
	...
	@Test void p01_textPwsFormat_withInnerElement() {
		// TEXT_PWS preserves whitespace and readText() is invoked when an inner element is found
		...
		var bean = P.read(x, TextPwsBean.class);
		assertEquals("aval", bean.a);
		assertNotNull(bean.b);
		// At minimum the inner text or surrounding text appears in b - exact format depends on readText impl.
	...
	@Test void q01_objectTargetCharacterFromTypedString() {
		...
		// the isChar branch here by parsing into Character target where readAnything still routes
		// through the OBJECT/STRING branch via type-attribute.
		var c = P.read("<x _type='string'>z</x>", Character.class);
		assertEquals(Character.valueOf('z'), c);
	}

	// -----------------------------------------------------------------------------------------------------------------
	// r01 - readAnything: numeric on element-name dispatch (sType is Object, jsonType from element name)
	// -----------------------------------------------------------------------------------------------------------------
	...
	@Test void r01_elementNameDispatch_number() {
		...
		var m = (Map) P.read("<object><k _type='number'>42</k></object>", Map.class);
	...
	@Test void s01_readWithReporter_succeeds() {
		var parser = XmlParser.create().reporter(XmlConfigAnnotationTest.AB.class).build();
		var bean = parser.read("<object><name>Alice</name><age>30</age></object>", JsonMap.class);
		assertEquals("Alice", bean.get("name"));
	}

	@Test void s02_readWithResolver_succeeds() {
		var parser = XmlParser.create().resolver(XmlConfigAnnotationTest.AC.class).build();
		var bean = parser.read("<object><name>Bob</name></object>", JsonMap.class);
		assertEquals("Bob", bean.get("name"));
	}

	@Test void s03_readWithEventAllocator_succeeds() {
		var parser = XmlParser.create().eventAllocator(XmlConfigAnnotationTest.AA.class).build();
		var bean = parser.read("<object><x>test</x></object>", JsonMap.class);
