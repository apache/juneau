...
/**
 * Cross-pair round-trip coverage for {@link TemporalFormat} bound to the {@link Instant} subtype across
 * every supported serializer/parser pair.
 *
 * <p>
 * {@link TemporalFormat} is unique among the format enums in that it accepts an entire {@link Temporal}
 ...
 *
 * <p>
 * Many {@link TemporalFormat} values are intentionally lossy for {@link Instant} — date-only formats drop
 * the time portion (parsed back to midnight UTC), time-only formats drop the date portion (parsed back to
 * 1970-01-01 + time), and {@link TemporalFormat#ISO_YEAR} / {@link TemporalFormat#ISO_YEAR_MONTH} reduce
 * to a year (or year/month) only.  The {@code expectedAfter} helper canonicalizes through the format's
 * own {@code format / parse} cycle so the assertion reflects the lossy canonical form.
 */
class TemporalFormat_Instant_RoundTrip_Test extends TestBase {

	/**
	 * Builds a fully-configured {@link RoundTrip_Tester} parameterized on a {@link TemporalFormat} value.
	 *
	 * <p>
	 ...
	 * {@code serializer().temporalFormat(fmt)} and {@code parser().temporalFormat(fmt)} builder calls so the
	 ...
	 * {@link Instant} — date-only formats drop the time portion, time-only formats drop the date portion,
	 * and {@link TemporalFormat#ISO_YEAR} / {@link TemporalFormat#ISO_YEAR_MONTH} reduce to year / year-month.
	 * Canonicalize through the format's own {@code format/parse} cycle so the assertion reflects the lossy
	 * canonical form.
	 *
	 * <p>
	 * Mirrors the swap's call pattern exactly: {@code format(original, session.getTimeZoneId())} then
	 * {@code parse(s, Instant.class, session.getTimeZoneId())}.  With the default session (no explicit zone)
	 * both reduce to {@code zoneId == null} fallbacks.  For {@link Instant} both ends use the {@code Z}
	 * offset (per {@link TemporalFormat}'s Instant-specific dispatch), so round-trip is symmetric.
	 *
	 * <p>
	 ...
	 */
	private static Instant expectedAfter(Instant original, RoundTrip_Tester t, TemporalFormat fmt) {
		if (original == null)
			return null;
		if (t.isValidationOnly())
			return original;
		return fmt.parse(fmt.format(original, null), Instant.class, null);
	}

	//====================================================================================================
	// Bean with one Instant field — exercises MarshalledPropertyPostProcessor.applyContextFormats
	//====================================================================================================
	...
	@MethodSource("combos")
	void a01_instantProperty_basic(RoundTrip_Tester t, TemporalFormat fmt) throws Exception {
		var x = new A01Bean();
		x.i = Instant.parse("2024-06-15T12:00:00Z");

		x = t.roundTrip(x);
		assertEquals(expectedAfter(Instant.parse("2024-06-15T12:00:00Z"), t, fmt), x.i, "fmt=" + fmt);
	}

	//====================================================================================================
	// Bean with multiple Instant fields — multi-property cross-section
	//====================================================================================================
	...
	@ParameterizedTest(name = "{0} | {1}")
	@MethodSource("combos")
	void a02_instantProperty_multipleFields(RoundTrip_Tester t, TemporalFormat fmt) throws Exception {
		var x = new A02Bean();
		x.created = Instant.parse("2024-06-15T12:00:00Z");
		x.updated = Instant.parse("2025-01-01T00:00:00Z");
		x.epoch = Instant.EPOCH;

		x = t.roundTrip(x);
		assertEquals(expectedAfter(Instant.parse("2024-06-15T12:00:00Z"), t, fmt), x.created, "fmt=" + fmt);
		assertEquals(expectedAfter(Instant.parse("2025-01-01T00:00:00Z"), t, fmt), x.updated, "fmt=" + fmt);
		assertEquals(expectedAfter(Instant.EPOCH, t, fmt), x.epoch, "fmt=" + fmt);
	}

	//====================================================================================================
	// Bean with epoch + pre-epoch + sub-second Instant — boundary coverage
	//====================================================================================================

	public static class A03Bean {
		public Instant epoch;
		public Instant subSecond;
		public Instant midRange;
	}

	@ParameterizedTest(name = "{0} | {1}")
	@MethodSource("combos")
	void a03_instantProperty_boundaries(RoundTrip_Tester t, TemporalFormat fmt) throws Exception {
		var x = new A03Bean();
		x.epoch = Instant.EPOCH;
		// Whole-second value — sub-millisecond precision varies by format
		x.subSecond = Instant.parse("2024-06-15T12:00:00Z");
		x.midRange = Instant.parse("2009-02-13T23:31:30Z");

		x = t.roundTrip(x);
		assertEquals(expectedAfter(Instant.EPOCH, t, fmt), x.epoch, "fmt=" + fmt);
		assertEquals(expectedAfter(Instant.parse("2024-06-15T12:00:00Z"), t, fmt), x.subSecond, "fmt=" + fmt);
		assertEquals(expectedAfter(Instant.parse("2009-02-13T23:31:30Z"), t, fmt), x.midRange, "fmt=" + fmt);
	}

	//====================================================================================================
	// Bean with far-future Instant — large epoch-millis stress (MILLIS numeric promotion / overflow paths)
	//====================================================================================================

	public static class A04Bean {
		public Instant farFuture;
	}

	@ParameterizedTest(name = "{0} | {1}")
	@MethodSource("combos")
	void a04_instantProperty_largeValues(RoundTrip_Tester t, TemporalFormat fmt) throws Exception {
		var x = new A04Bean();
		// Year 2099 — large but still within all formatters' supported range.
		x.farFuture = Instant.parse("2099-12-31T23:59:59Z");

		x = t.roundTrip(x);
		assertEquals(expectedAfter(Instant.parse("2099-12-31T23:59:59Z"), t, fmt), x.farFuture, "fmt=" + fmt);
	}

	//====================================================================================================
	// Bean with null Instant field — sentinel-style null preservation
	//====================================================================================================

	@ParameterizedTest(name = "{0} | {1}")
	@MethodSource("combos")
	void a05_instantProperty_nullField(RoundTrip_Tester t, TemporalFormat fmt) throws Exception {
		var x = new A01Bean();

		x = t.roundTrip(x);
		assertNull(x.i, "fmt=" + fmt);
	}

	//====================================================================================================
	// Standalone top-level Instant — exercises per-format dispatch in *SerializerSession / *ParserSession
	//====================================================================================================

	@ParameterizedTest(name = "{0} | {1}")
	@MethodSource("combos")
	void a06_instantTopLevel(RoundTrip_Tester t, TemporalFormat fmt) throws Exception {
		// Validation-only testers (Json schema, Csv, Parquet) have no parser-side round-trip semantics for a
		// standalone Instant value — they assert that the serializer accepts it without throwing.
		if (t.isValidationOnly())
			return;
