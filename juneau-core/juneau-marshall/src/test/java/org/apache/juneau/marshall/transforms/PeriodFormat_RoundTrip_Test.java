...
class PeriodFormat_RoundTrip_Test extends TestBase {
	...
	private static final List<TesterBuilder> BUILDERS = List.of(
		...
		fmt -> RoundTrip_Tester.create(36, "Prototext - default | " + fmt)
			.serializer(PrototextSerializer.create().keepNullProperties().addBeanTypes().addRootType().periodFormat(fmt))
			.parser(PrototextParser.create().periodFormat(fmt))
			.build(),
	...
	void a06_periodTopLevel(RoundTrip_Tester t, PeriodFormat fmt) throws Exception {
		...
		try {
			...
			var x2 = p.read(out, Period.class);
