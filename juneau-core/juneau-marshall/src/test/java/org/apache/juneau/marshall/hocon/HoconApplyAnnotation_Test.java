...
class HoconApplyAnnotation_Test extends TestBase {
	...
	@Test void a03_empty_nonDefault() {
		var a = HoconApplyAnnotation.create(String.class).build();
		assertFalse(HoconApplyAnnotation.empty(a));
	}

	@Test void a04_create_and_build() {
		var a = HoconApplyAnnotation.create("MyClass").build();
	...
	@Test void b05_applier_emptyOn_returnsEarly() {
		...
		var applier = new HoconApplyAnnotation.Applier(vr);
		var b = MarshallingContext.DEFAULT.copy();
		var a = HoconApplyAnnotation.DEFAULT;
