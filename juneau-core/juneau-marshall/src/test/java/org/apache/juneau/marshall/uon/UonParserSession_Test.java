 ...
 *  - readAnything edge cases (empty/AMP for collections, primitives, strings)
 *  - readIntoMap / readIntoCollection / readIntoBeanMap state-machine error branches
 *  - readPString unmatched parens
 *  - readNull error
 *  - readBoolean error
 *  - validateEnd remainder error
 *  - doReadIntoMap / doReadIntoCollection wrappers
...
class UonParserSession_Test extends TestBase {
	...
	// -----------------------------------------------------------------------------------------------------------------
	// b01_httpPartParse - read(HttpPartType, schema, in, ClassMeta) shortcut paths
	// -----------------------------------------------------------------------------------------------------------------
	...
	private static <T> T httpParse(UonParser parser, String in, Class<T> type) throws Exception {
		...
		return session.read((HttpPartType)null, (HttpPartSchema)null, in, cm);
	}
	...
	// -----------------------------------------------------------------------------------------------------------------
	// c01_readAnything - empty/AMP-first branches
	// -----------------------------------------------------------------------------------------------------------------

	@Test void c01_readEmptyForCollection() throws Exception {
		// Empty input for collection - HttpPart shortcut returns null for null/empty,
		// but the readAnything path with isCollectionOrArray on empty input returns sType.newInstance().
		// For top-level public parse(...) on List from empty string, the result is null.
		var l = P.read("", List.class);
		assertNull(l);
	}

	@Test void c02_readEmptyForArray() throws Exception {
		var arr = P.getSession().read((HttpPartType)null, (HttpPartSchema)null, "", P.getSession().getClassMeta(String[].class));
		assertNotNull(arr);
		assertEquals(0, arr.length);
	}

	@Test void c03_readEmptyForMap() throws Exception {
		// Map with empty input - readAnything → returns null but readIntoMap may differ
		// Empty input → c==-1 → not collection/string/object/primitive → returns null
		var m = P.getSession().read((HttpPartType)null, (HttpPartSchema)null, "", P.getSession().getClassMeta(Map.class));
		assertNull(m);
	}

	@Test void c04_readEmptyForPrimitive() throws Exception {
		// Empty input for primitive returns the default for the primitive type.
		var i = P.getSession().read((HttpPartType)null, (HttpPartSchema)null, "", P.getSession().getClassMeta(int.class));
		assertEquals(Integer.valueOf(0), i);
		var b = P.getSession().read((HttpPartType)null, (HttpPartSchema)null, "", P.getSession().getClassMeta(boolean.class));
		assertEquals(Boolean.FALSE, b);
	}

	@Test void c05_readEmptyForString() throws Exception {
		var s = P.read("", String.class);
		assertEquals("", s);
	}

	// -----------------------------------------------------------------------------------------------------------------
	// d01_readAnything - error paths
	// -----------------------------------------------------------------------------------------------------------------

	@Test void d01_readBoolean_invalid() {
		assertThrows(ParseException.class, () -> P.read("notabool", Boolean.class));
	}

	@Test void d02_readBoolean_null() throws Exception {
		assertNull(P.read("null", Boolean.class));
	}

	@Test void d03_readNull_invalidSequence() {
		// 'n' followed by non-"ull" should throw.
		// "nx" would be parsed as a string at top-level; force Map context to take readAnything's c=='n' branch.
		// Actually the c == 'n' branch in readAnything is hit when sType is non-bean, non-string. Use HashMap
		// at second level via map literal to exercise.
		assertThrows(ParseException.class, () -> P.read("nope", Boolean.class));
	}

	@Test void d04_readString_unmatchedParens() {
		// 'xxx without matching ' — readPString should throw.
		assertThrows(ParseException.class, () -> P.read("'unmatched", String.class));
	}

	@Test void d05_readString_unmatchedParensInMap() {
		assertThrows(ParseException.class, () -> P.read("(a='unmatched)", Map.class));
	}

	// -----------------------------------------------------------------------------------------------------------------
	// e01_readIntoMap - state-machine errors
	// -----------------------------------------------------------------------------------------------------------------

	@Test void e01_map_missingOpenParen() {
		// readIntoMap requires '(' at start when not 'n'/AMP.
		assertThrows(ParseException.class, () -> P.read("xyz", Map.class));
	}
	...
	// -----------------------------------------------------------------------------------------------------------------
	// f01_readIntoCollection - state-machine errors and parens
	// -----------------------------------------------------------------------------------------------------------------

	@Test void f01_collection_missingAtSign() {
		// At non-top-level (i.e. via doReadIntoCollection / inner field), missing '@(' throws.
		// Actually the public parse(List.class) goes through doRead → readAnything which routes
		// arrays/lists differently (single-value wrap). This test exercises the inner state.
		assertThrows(ParseException.class, () -> P.read("(a=xyz)", List.class)); // routes via map-as-collection
	}
	...
	@Test void f03_collection_nullLiteral() throws Exception {
		assertNull(P.read("null", List.class));
	}
	...
	@Test void f04_collection_singleValueAsMap() throws Exception {
		...
		var l = (LinkedList<Map>) P.read("(x=1)", LinkedList.class, Map.class);
	...
	@Test void f05_collection_unclosedAfterEntry() {
		...
		assertThrows(ParseException.class, () -> P.read("@(1,2,", List.class));
	}
	...
	@Test void f06_collection_topLevelCommaList() throws Exception {
		...
		var l = ses.read((HttpPartType)null, (HttpPartSchema)null, "1,2,3", cm);
		assertEquals(3, l.size());
	}

	@Test void f07_collection_emptyArray() throws Exception {
		// At top-level, an empty input for an array yields an empty array.
		var arr = P.read("@()", String[].class);
		assertNotNull(arr);
		assertEquals(0, arr.length);
	}

	@Test void f08_collection_singleValueAsMapForArray() throws Exception {
		// Arrays go through similar map-as-array path in readAnything.
		var arr = P.read("(x=1)", Map[].class);
		assertEquals(1, arr.length);
	}

	// -----------------------------------------------------------------------------------------------------------------
	// g01_readIntoBeanMap - state-machine errors
	// -----------------------------------------------------------------------------------------------------------------
	...
	@Test void g06_bean_unknownProperty() throws Exception {
		...
		var b = p.read("(unknown=foo,f1=bar)", Bean.class);
	...
	@Test void g08_bean_nullLiteral() throws Exception {
		...
		assertNull(P.read("null", Bean.class));
	}

	@Test void g09_bean_missingOpenParen() {
		// "xyz" without '(' - readIntoBeanMap throws.
		assertThrows(ParseException.class, () -> P.read("xyz", Bean.class));
	}
	...
	@Test void h01_validateEnd_extraJunk() {
		...
		assertThrows(ParseException.class, () -> PVE.read("(a=1)xx", Map.class));
	}
	...
	// -----------------------------------------------------------------------------------------------------------------
	// i01_doReadIntoMap / doReadIntoCollection wrappers
	// -----------------------------------------------------------------------------------------------------------------
	...
	@Test void i04_doParseIntoCollection_validateEnd() {
		assertThrows(ParseException.class, () -> {
			var dest = new ArrayList<>();
			PVE.readIntoCollection("@(1,2)x", dest, Integer.class);
		});
	}

	// -----------------------------------------------------------------------------------------------------------------
	// j01_decoding / readAttrName encoded path
	// -----------------------------------------------------------------------------------------------------------------
	...
	@Test void j06_decoding_amp_terminatesParam() throws Exception {
		...
		var s = ses.read((HttpPartType)null, (HttpPartSchema)null, "hello%26", cm);
		assertNotNull(s);
	}
	...
	@Test void k01_escapedTilde() throws Exception {
		// "~~" in plain text → "~"
		assertEquals("~", P.read("~~", String.class));
	}
	...
	@Test void m02_typedCollection() throws Exception {
		var l = (LinkedList<Integer>) P.read("@(1,2,3)", LinkedList.class, Integer.class);
		assertEquals(3, l.size());
	}

	@Test void m03_arrayOfInts() throws Exception {
		var arr = P.read("@(1,2,3)", int[].class);
	...
	// -----------------------------------------------------------------------------------------------------------------
	// p01_readAnything - Date/Calendar/Temporal/Duration/Period type branches
	// -----------------------------------------------------------------------------------------------------------------

	@Test void p01_readDate() throws Exception {
		// Exercises the sType.isDate() branch in readAnything.
		var d = P.read("2024-01-15T00:00:00Z", Date.class);
		assertNotNull(d);
	}

	@Test void p02_readCalendar() throws Exception {
		// Exercises the sType.isCalendar() branch in readAnything.
		var c = P.read("2024-01-15T12:00:00Z", Calendar.class);
		assertNotNull(c);
	}

	@Test void p03_readTemporal() throws Exception {
		// Exercises the sType.isTemporal() branch in readAnything.
		var t = P.read("2024-01-15", LocalDate.class);
		assertNotNull(t);
	}

	@Test void p04_readDuration() throws Exception {
		// Exercises the sType.isDuration() branch in readAnything.
		var d = P.read("PT1H", Duration.class);
		assertNotNull(d);
		assertEquals(Duration.ofHours(1), d);
	}

	@Test void p05_readPeriod() throws Exception {
		// Exercises the sType.isPeriod() branch in readAnything.
		var p = P.read("P1Y", Period.class);
		assertNotNull(p);
		assertEquals(Period.ofYears(1), p);
	}

	@Test void p06_readDate_null() throws Exception {
		// Null value for Date type returns null.
		assertNull(P.read("null", Date.class));
	}

	@Test void p07_readCalendar_null() throws Exception {
		// Null value for Calendar type returns null.
		assertNull(P.read("null", Calendar.class));
	}

	@Test void p08_readTemporal_null() throws Exception {
		// Null value for LocalDate type returns null.
		assertNull(P.read("null", LocalDate.class));
	}

	// -----------------------------------------------------------------------------------------------------------------
	// q01_readAnything - ObjectSwap path
	// -----------------------------------------------------------------------------------------------------------------
	...
	@Test void q01_objectSwap_roundtrip() throws Exception {
		// Exercises the nn(swap) && nn(o) branch in readAnything.
		var parser = UonParser.create().swaps(Q01_WrappedSwap.class).build();
		var result = parser.read("hello", Q01_Wrapped.class);
		assertNotNull(result);
		assertEquals("hello", result.value);
	}

	// -----------------------------------------------------------------------------------------------------------------
	// r01_readAnything - BuilderSwap path (nn(builder) branch)
	// -----------------------------------------------------------------------------------------------------------------
	...
	@Test void r01_builderSwap() throws Exception {
		// Exercises the nn(builder) branch in readAnything via @Builder annotation.
		var b = P.read("(x=42)", R01_Bean.class);
		assertNotNull(b);
		assertEquals(42, b.x);
	}
	...
	@Test void r02_builderSwap_null() throws Exception {
		...
		var b = P.read("null", R01_Bean.class);
		assertNull(b);
	}

	// -----------------------------------------------------------------------------------------------------------------
	// s01_readAnything - canCreateNewBean null return (null literal in bean context)
	// -----------------------------------------------------------------------------------------------------------------

	@Test void s01_bean_nullReturnFromParseIntoBeanMap() throws Exception {
		// readIntoBeanMap returns null when the UON value is "null", so readAnything gets null back.
		var b = P.read("null", Bean.class);
		assertNull(b);
	}

	// -----------------------------------------------------------------------------------------------------------------
	// t01_readIntoCollection - S2 state closing ')' branch and args branch
	// -----------------------------------------------------------------------------------------------------------------

	@Test void t01_collection_S2_closingParen() throws Exception {
		// "@(,)" — after ',' takes S2 state; then ')' should trigger readAnything for that element.
	...
	@Test void t02_collection_whitespace_in_noParens_path() throws Exception {
		...
		var l = ses.read((HttpPartType)null, (HttpPartSchema)null, " 1,2", cm);
		assertNotNull(l);
	}
	...
	// -----------------------------------------------------------------------------------------------------------------
	// u01_readIntoMap - encoded mode null-attr-name path (S2 with currAttr==null)
	// -----------------------------------------------------------------------------------------------------------------

	@Test void u01_map_encoded_nullAttr() throws Exception {
		// In encoded mode, %00 decodes to a NUL char. The readAttrName in encoded mode returns the literal
		...
		var m = PE.read("(null=v)", Map.class);
	...
	@Test void u02_map_encoded_whitespace_S1() throws Exception {
		...
		var m = PE.read("( a=1)", Map.class);
		assertNotNull(m);
		assertEquals(1, m.get("a"));
	}

	// -----------------------------------------------------------------------------------------------------------------
	// v01_readIntoBeanMap - S3 empty-value branches and S1 whitespace
	// -----------------------------------------------------------------------------------------------------------------
	...
	@Test void v01_beanMap_S3_comma_emptyValue() throws Exception {
		...
		var b = P.read("(f1=,f2=0)", Bean.class);
		assertNotNull(b);
	...
	@Test void v03_beanMap_S4_comma_then_end() throws Exception {
		...
		var b = P.read("(f1=foo,f2=123)", Bean.class);
		assertNotNull(b);
		assertEquals("foo", b.f1);
	...
	// -----------------------------------------------------------------------------------------------------------------
	// w01_readAttrName - encoded path with AMP and EQ replacement in escape context
	// -----------------------------------------------------------------------------------------------------------------
	...
	// -----------------------------------------------------------------------------------------------------------------
	// x01_readString - isUrlParamValue with whitespace path
	// -----------------------------------------------------------------------------------------------------------------

	@Test void x01_readString_urlParamValue_trimmed() throws Exception {
		// isUrlParamValue=true: string is StringUtils.trim()ed at end.
		// Exercises the isUrlParamValue branch in readString.
		var ses = P.getSession();
		var cm = ses.getClassMeta(String.class);
		var s = ses.read((HttpPartType)null, (HttpPartSchema)null, "  hello  ", cm);
		// Leading/trailing whitespace is trimmed in URL param mode.
		assertNotNull(s);
	}

	@Test void x02_readString_nonUrlParam_whitespace() throws Exception {
		// At top-level, doRead uses isUrlParamValue=true so whitespace does NOT terminate the string.
		...
		var m = P.read("(a=hello world)", Map.class);
	...
	@Test void y01_getUonReader_alreadyUonReader() throws Exception {
		// Exercises the "r instanceof UonReader" branch in getUonReader by calling doRead
		...
		var s = PE.read("hello", String.class);
		assertEquals("hello", s);
	}

	// -----------------------------------------------------------------------------------------------------------------
	// z01_readIntoCollection - error branches (S1/S2 and S3 throws)
	// -----------------------------------------------------------------------------------------------------------------
	...
	@Test void aa01_collection_S2_closeParenWithElement() throws Exception {
		...
		// The S2 branch fires: unread ')' and call readAnything which returns empty string for Object type.
		var l = P.read("@(1,)", List.class);
		assertNotNull(l);
	...
	@Test void aa03_readArgs_isArgsType() throws Exception {
		// readArgs uses an ARGS ClassMeta which exercises the type.isArgs() true branch
		// in readIntoCollection (lines 610 and 620: type.getArg(argIndex++) instead of getElementType()).
	...
	@Test void aa04_readArgs_isArgsType_thenS2Close() throws Exception {
		// readArgs with "@(1,)" — ARGS type, S2 + ')' fires line 610 with type.isArgs()=true.
		var session = P.getSession();
		var args = session.readArgs("@(1,)", new java.lang.reflect.Type[]{int.class, String.class});
		assertNotNull(args);
	}

	@Test void aa05_readArgs_noParens_nonParensPath() throws Exception {
		// readArgs with "1,hello" — non-parens input (isUrlParamValue=true).
	...
	@Test void aa06_readArgs_nonParens_whitespace_S2() throws Exception {
	...
	// -----------------------------------------------------------------------------------------------------------------
	// ab01_readIntoMap - S2 null-attr with comma (triggers null-attr early-return path)
	// -----------------------------------------------------------------------------------------------------------------

	@Test void ab01_map_nullAttr_S2_eof() throws Exception {
		// In encoded mode, readAttrName returns null for the literal string "null" (line 905).
		// "(null" — 'null' attr followed by EOF → readAttrName returns null, currAttr=null, state=S2.
		// Then outer loop reads c=-1: S2 + c==-1 + currAttr==null → r.unread() + return null.
		var m = PE.read("(null", Map.class);
		assertNull(m);
	}

	@Test void ab02_map_S3_emptyValue_comma() throws Exception {
		// "(a=,b=2)" — S3 with ',' immediately after '=' sets empty value for a.
		var m = P.read("(a=,b=2)", Map.class);
		assertNotNull(m);
	...
	@Test void ab04_map_S2_nullAttr_closeParen() throws Exception {
		// In encoded mode, readAttrName with "null" followed by whitespace returns null (null currAttr).
		// "(null )" — null currAttr, whitespace consumed in outer S2 loop, then ')' triggers
		//            null-attr check: r.unread() + return null from readIntoMap.
		var m = PE.read("(null )", Map.class);
		assertNull(m);
	}

	// -----------------------------------------------------------------------------------------------------------------
	// ac01_readIntoBeanMap - S2 with comma (null-value bean property)
	// -----------------------------------------------------------------------------------------------------------------
	...
	@Test void ac02_beanMap_S3_unknownProp_emptyValue() {
		...
		assertDoesNotThrow(() -> P.read("(unknown=,f2=0)", Bean.class));
	}
	...
	@Test void ac03_beanMap_S3_typePropertyName_emptyValue() throws Exception {
		...
		var b = P.read("(_type=)", Bean.class);
		assertNotNull(b);
	}

	// -----------------------------------------------------------------------------------------------------------------
	// ad01_readAnything - canCreateNewInstanceFromString branch
	// -----------------------------------------------------------------------------------------------------------------
	...
	@Test void ad01_canCreateNewInstanceFromString() throws Exception {
		...
		// This exercises the sType.canCreateNewInstanceFromString() branch in readAnything.
		var o = P.read("hello", AD01_StringCtorType.class);
		assertNotNull(o);
	...
	@Test void ad02_canCreateNewInstanceFromString_null() throws Exception {
		...
		var o = P.read("null", AD01_StringCtorType.class);
		assertNull(o);
	}

	// -----------------------------------------------------------------------------------------------------------------
	// ae01_readObject - non-numeric non-null plain string in isObject() path
	// -----------------------------------------------------------------------------------------------------------------
	...
	@Test void ae01_object_plainString_nonNumeric() throws Exception {
		...
		var o = P.read("hello", Object.class);
		assertEquals("hello", o);
	}

	@Test void ae02_object_numericString() throws Exception {
		// A numeric string in Object type → o = readNumber(s, Number.class) (line 322).
		var o = P.read("42", Object.class);
		assertNotNull(o);
	}

	@Test void ae03_object_nullString_inObject() throws Exception {
		// "null" in Object type → o stays null (line 320 else-if false path).
		var o = P.read("null", Object.class);
		assertNull(o);
	}

	// -----------------------------------------------------------------------------------------------------------------
	// af01_readBoolean - "null" string (not null reference) — exercises s.equals("null") branch
	// -----------------------------------------------------------------------------------------------------------------

	@Test void af01_readBoolean_trueValue() throws Exception {
		// Exercises eqic(s, "true") branch.
		assertEquals(Boolean.TRUE, P.read("true", Boolean.class));
	}

	@Test void af02_readBoolean_falseValue() throws Exception {
		// Exercises eqic(s, "false") branch.
		assertEquals(Boolean.FALSE, P.read("false", Boolean.class));
	}

	// -----------------------------------------------------------------------------------------------------------------
	// ag01_readIntoCollection - !isInParens && !isUrlParamValue throws (line 591)
	// -----------------------------------------------------------------------------------------------------------------

	@Test void ag01_collection_notAtSign_notUrlParam_throws() {
		// readIntoCollection with isUrlParamValue=false and non-'@' input → throw line 591.
		// doReadIntoCollection uses isUrlParamValue=false.
		assertThrows(ParseException.class, () -> P.readIntoCollection("1,2,3", new ArrayList<>(), Integer.class));
	}

	// -----------------------------------------------------------------------------------------------------------------
	// ah01_readAnything - c == 'n' false branch → else-throw (line 407 false / 414)
	// -----------------------------------------------------------------------------------------------------------------
	...
	@Test void ah01_nonInstantiatable_notN_throws() {
		...
		assertThrows(ParseException.class, () -> P.read("foo", java.io.InputStream.class));
	}

	// -----------------------------------------------------------------------------------------------------------------
	// ai01_readAnything - collection parsed as non-@ object
	// -----------------------------------------------------------------------------------------------------------------
	...
	@Test void ai01_collection_emptyParens_asBean() throws Exception {
		...
		var l = P.read("()", new java.lang.reflect.ParameterizedType() {
	...
	// -----------------------------------------------------------------------------------------------------------------
	// aj01_readIntoMap - keyType null branch (line 674)
	// -----------------------------------------------------------------------------------------------------------------

	@Test void aj01_map_nullKeyType() throws Exception {
		// When parsing a raw Map (no key type), readAnything calls readIntoMap with
		...
		P.readIntoMap("(a=1,b=2)", dest, null, null);
		assertNotNull(dest.get("a"));
	}

	// -----------------------------------------------------------------------------------------------------------------
	// ak01_readString - whitespace terminator in non-urlParam mode (line 964)
	// -----------------------------------------------------------------------------------------------------------------

	@Test void ak01_readString_whitespace_terminates_nonUrlParam() throws Exception {
		// doReadIntoCollection uses isUrlParamValue=false, so within values, whitespace terminates.
		// "@(hello world)" — value "hello world": inside parens, readString with isUrlParamValue=false.
		// Whitespace terminates at "hello". Collection element is "hello".
		var dest = new ArrayList<>();
		P.readIntoCollection("@(hello world)", dest, String.class);
		assertNotNull(dest);
		assertEquals("hello", dest.get(0));
	}

	// -----------------------------------------------------------------------------------------------------------------
	// al01_readString - EQ control char replacement (line 962)
	// -----------------------------------------------------------------------------------------------------------------

	@Test void al01_readString_encodedEquals_replacement() throws Exception {
		// In decoded mode, %3D decodes to EQ control char (0x02).
		// readString sees EQ and calls r.replace('='), which exercises line 962.
		// "(a=hello%3Dworld)" with decoding mode → value "hello=world".
		var m = PE.read("(a=hello%3Dworld)", Map.class);
		assertNotNull(m);
		assertEquals("hello=world", m.get("a"));
	}

	@Test void al02_readString_encodedEquals_inCollection() throws Exception {
		// Encoded equals in a collection element exercises the EQ path with isUrlParamValue=false.
		var dest = new ArrayList<>();
		PE.readIntoCollection("@(hello%3Dworld)", dest, String.class);
		assertNotNull(dest);
		assertEquals("hello=world", dest.get(0));
	}

	// -----------------------------------------------------------------------------------------------------------------
	// am01_readIntoMap - AMP-first input terminates early (line 678 c==AMP)
	// -----------------------------------------------------------------------------------------------------------------

	@Test void am01_map_AMPfirst_returnsNull() throws Exception {
		// In decoding mode, bare '&' is decoded to AMP (0x01).
		// When readIntoMap reads AMP as the first char, it returns null (line 678 true-branch).
		// "&(a=1)" — '&' decoded to AMP, readIntoMap sees AMP immediately → return null.
		var m = PE.read("&(a=1)", Map.class);
		assertNull(m);
	}

	@Test void am02_collection_AMPfirst_returnsNull() throws Exception {
		// "&@(1,2)" — '&' decoded to AMP, readIntoCollection sees AMP → return null (line 577 true-branch).
		var l = PE.read("&@(1,2)", List.class);
		assertNull(l);
	}

	@Test void am03_map_AMP_midMap_terminates() throws Exception {
		// "(a=1&b=2)" with decoding — '&' terminates S4 of map. The 'b=2' is discarded.
		// Exercises the c==AMP branch in S4 of readIntoMap (line 740).
		var m = PE.read("(a=1&b=2)", Map.class);
		assertNotNull(m);
	...
	@Test void am04_beanMap_AMP_midMap_terminates() throws Exception {
		// "(f1=foo&f2=0)" with decoding — '&' terminates S4 of readIntoBeanMap.
		var b = PE.read("(f1=foo&f2=0)", Bean.class);
		assertNotNull(b);
		assertEquals("foo", b.f1);
	...
	@Test void am05_collection_AMP_midCollection_terminates() {
		...
		assertThrows(ParseException.class, () -> PE.read("@(a&b)", List.class));
	}

	@Test void am06_collection_AMPfirst_viaDoParseIntoCollection() throws Exception {
		// PE.readIntoCollection with "&..." — '&' decoded to AMP reaches readIntoCollection line 577.
		// AMP at the very first char → return null (line 577 AMP branch).
		var dest = new ArrayList<>();
		var result = PE.readIntoCollection("&@(1,2)", dest, String.class);
		assertNull(result);
	}
	...
	// -----------------------------------------------------------------------------------------------------------------
	// an01_readString - non-urlParam whitespace path (line 964) via readIntoCollection
	// -----------------------------------------------------------------------------------------------------------------

	@Test void an01_readString_nonUrlParam_whitespace_inParens() throws Exception {
		// Inside "@(hello world)", readString is called with isUrlParamValue=false.
		// Whitespace terminates "hello", leaving " world" in stream.
		// Exercises the whitespace branch at line 964.
		var dest = new ArrayList<>();
		P.readIntoCollection("@(hello world,bye)", dest, String.class);
		assertFalse(dest.isEmpty());
		assertEquals("hello", dest.get(0));
	}

	// -----------------------------------------------------------------------------------------------------------------
	// ao01_readAttrName - encoded EOF while-loop exit (lines 898, 907-910, 914)
	// -----------------------------------------------------------------------------------------------------------------
	...
	@Test void ao01_attrName_encoded_AMP_terminates() throws Exception {
		...
		// Then state S2, next read is '&' decoded again... actually readIntoMap outer handles it.
		var m = PE.read("(a&b=1)", Map.class);
	...
	// -----------------------------------------------------------------------------------------------------------------
	// ap01_readAnything - collection/array with _type key (lines 346, 363 true branches)
	// -----------------------------------------------------------------------------------------------------------------
	...
	@Test void ap01_collection_typeKey_cast() throws Exception {
		...
		Object result = P.read("(_type=foo)", (java.lang.reflect.Type) List.class);
		assertNotNull(result);
	}
	...
	@Test void ap02_array_typeKey_cast() throws Exception {
		...
		Object result = P.read("(_type=foo)", (java.lang.reflect.Type) String[].class);
		assertNotNull(result);
	}

	// -----------------------------------------------------------------------------------------------------------------
	// aq01 - readNull with non-"ull" sequence (line 761 false branch → throws ParseException)
	// -----------------------------------------------------------------------------------------------------------------

	@Test void aq01_readNull_nonUll_inBeanMap_throws() {
		// readIntoBeanMap calls readNull when first char is 'n'. "nfoo" starts with 'n'.
		// readString then reads remaining "foo" (stops at ','/'/'/EOF): "ull".equals("foo") is false.
		// → ParseException thrown (line 762 true branch).
		assertThrows(ParseException.class, () -> P.read("(nfoo)", Bean.class));
	}

	@Test void aq02_readNull_nonUll_inMap_throws() {
		// readIntoMap also calls readNull when first char is 'n'. "(nbar=1)" with Map target.
		// readAttrName reads "bar", "ull".equals("bar") is false → ParseException.
		assertThrows(ParseException.class, () -> P.read("nbar", Map.class));
	}
	...
	// -----------------------------------------------------------------------------------------------------------------
	// as01 - readString EQ replacement (line 962: c == EQ → r.replace('='))
	// -----------------------------------------------------------------------------------------------------------------

	@Test void as01_readString_encodedEquals_replaced() throws Exception {
		// In decoding mode, a literal '=' in a value is decoded to EQ char () by UonReader.
		// readString sees c == EQ (line 962) and replaces it with '=' in the marked buffer.
		// Input: "(a=b=c)" -- UonReader converts the second '=' to EQ -- readString restores it.
		var m = PE.read("(a=b=c)", Map.class);
		assertNotNull(m);
		assertEquals("b=c", m.get("a"));
	}

	// -----------------------------------------------------------------------------------------------------------------
	// at01 - readIntoCollection non-parens path (lines 641-655: isUrlParamValue=true, no '@' prefix)
	// -----------------------------------------------------------------------------------------------------------------

	@Test void at01_collection_urlParam_commaSeparated() throws Exception {
		// doRead calls readAnything(isUrlParamValue=true) which calls readIntoCollection.
		// "1,2,3" has no '@' so isInParens=false -> enters the non-parens while loop (line 641).
		var l = P.read("1,2,3", List.class);
		assertNotNull(l);
		assertEquals(3, l.size());
	}
	...
	// -----------------------------------------------------------------------------------------------------------------
	// au01 - readIntoCollection/readIntoMap EOF first char (line 577/678 c==-1 branch)
	// -----------------------------------------------------------------------------------------------------------------

	@Test void au01_readIntoCollection_emptyInput_returnsNull() throws Exception {
		// readIntoCollection with empty input: readSkipWs() hits EOF on first char, early-return yields null.
		var dest = new ArrayList<>();
		var result = P.readIntoCollection("", dest, String.class);
		assertNull(result);
	}
