...
class MsgPackTokenStreamCoverage_Test extends TestBase {
	...
	@Nested class R_reader extends TestBase {
		...
		@Test void r01_longFloatDoubleNumbers() throws Exception {
			...
			try (var r = MsgPackParser.DEFAULT.parseTokens(new byte[]{(byte) 0xCE, 0x00, 0x01, 0x00, 0x00})) {
				assertEquals(TokenType.VALUE_NUMBER, r.next());
				assertEquals(65536L, r.getNumber().longValue());
			}
			// 0xCA float32 = 1.0f (FLOAT branch).
			try (var r = MsgPackParser.DEFAULT.parseTokens(new byte[]{(byte) 0xCA, 0x3F, (byte) 0x80, 0x00, 0x00})) {
				assertEquals(TokenType.VALUE_NUMBER, r.next());
				assertEquals(1.0d, r.getNumber().doubleValue());
			}
			// 0xCB float64 = 2.5 (DOUBLE branch).
			try (var r = MsgPackParser.DEFAULT.parseTokens(new byte[]{(byte) 0xCB, 0x40, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00})) {
				assertEquals(TokenType.VALUE_NUMBER, r.next());
		...
		@Test void r02_extEmitsBinary() throws Exception {
			...
			try (var r = MsgPackParser.DEFAULT.parseTokens(new byte[]{(byte) 0xD4, 0x01, 0x7F})) {
		...
		@Test void r03_nonStringMapKeysCoerced() throws Exception {
			// {1:2} int key.
			try (var r = MsgPackParser.DEFAULT.parseTokens(new byte[]{(byte) 0x81, 0x01, 0x02})) {
				assertEquals(TokenType.START_OBJECT, r.next());
				assertEquals(TokenType.FIELD_NAME, r.next()); assertEquals("1", r.getFieldName());
			}
			// {65536:2} long key.
			try (var r = MsgPackParser.DEFAULT.parseTokens(new byte[]{(byte) 0x81, (byte) 0xCE, 0x00, 0x01, 0x00, 0x00, 0x02})) {
				assertEquals(TokenType.START_OBJECT, r.next());
				assertEquals(TokenType.FIELD_NAME, r.next()); assertEquals("65536", r.getFieldName());
			}
			// {true:2} boolean key.
			try (var r = MsgPackParser.DEFAULT.parseTokens(new byte[]{(byte) 0x81, (byte) 0xC3, 0x02})) {
				assertEquals(TokenType.START_OBJECT, r.next());
				assertEquals(TokenType.FIELD_NAME, r.next()); assertEquals("true", r.getFieldName());
			}
			// {1.0f:2} float key.
			try (var r = MsgPackParser.DEFAULT.parseTokens(new byte[]{(byte) 0x81, (byte) 0xCA, 0x3F, (byte) 0x80, 0x00, 0x00, 0x02})) {
				assertEquals(TokenType.START_OBJECT, r.next());
				assertEquals(TokenType.FIELD_NAME, r.next()); assertEquals("1.0", r.getFieldName());
			}
			// {2.5:2} double key.
			try (var r = MsgPackParser.DEFAULT.parseTokens(new byte[]{(byte) 0x81, (byte) 0xCB, 0x40, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02})) {
				assertEquals(TokenType.START_OBJECT, r.next());
				assertEquals(TokenType.FIELD_NAME, r.next()); assertEquals("2.5", r.getFieldName());
			}
			// {bin "x":2} binary key decodes as UTF-8.
			try (var r = MsgPackParser.DEFAULT.parseTokens(new byte[]{(byte) 0x81, (byte) 0xC4, 0x01, 'x', 0x02})) {
				assertEquals(TokenType.START_OBJECT, r.next());
		...
		@Test void r04_invalidMapKeyTypeThrows() throws Exception {
			...
			try (var r = MsgPackParser.DEFAULT.parseTokens(new byte[]{(byte) 0x81, (byte) 0x90, 0x02})) {
				assertEquals(TokenType.START_OBJECT, r.next());
		...
		@Test void r05_endOfStreamRepeatsAndCanReadFalse() throws Exception {
			try (var r = MsgPackParser.DEFAULT.parseTokens(new byte[]{0x01})) {
				assertEquals(TokenType.VALUE_NUMBER, r.next());
		...
		@Test void r06_truncatedContainerRethrowsIo() throws Exception {
			...
			try (var r = MsgPackParser.DEFAULT.parseTokens(new byte[]{(byte) 0x92, 0x01})) {
		...
		@Test void r09_currentTokenAndDepth() throws Exception {
			try (var r = MsgPackParser.DEFAULT.parseTokens(new byte[]{(byte) 0x91, (byte) 0x90})) {
		...
		@Test void r11_readNonValueStateThrows() throws Exception {
			...
			try (var r = MsgPackParser.DEFAULT.parseTokens(new byte[]{(byte) 0x91, 0x01})) {
				assertEquals(TokenType.START_ARRAY, r.next());
		...
		@Test void r15_getStringOnFieldNameAndScalarAccessors() throws Exception {
			...
			try (var r = MsgPackParser.DEFAULT.parseTokens(new byte[]{(byte) 0x81, (byte) 0xA1, 'a', 0x01})) {
				assertEquals(TokenType.START_OBJECT, r.next());
			...
			try (var r = MsgPackParser.DEFAULT.parseTokens(new byte[]{(byte) 0xC3})) {
		...
		@Test void r17_readParseExceptionWrapped() throws Exception {
			var bytes = MsgPackSerializer.DEFAULT.serialize("not-a-number");
		...
		@Test void r19_eofReadingElementInsideContainerRethrowsIo() throws Exception {
			...
			try (var r = MsgPackParser.DEFAULT.parseTokens(new byte[]{(byte) 0x91})) {
				assertEquals(TokenType.START_ARRAY, r.next());
				assertThrows(IOException.class, r::next);
			}
		}
		...
		@Test void r20_readBeanFromScalarSurfacesParseException() throws Exception {
			...
			var bytes = MsgPackSerializer.DEFAULT.serialize(1);
			try (var r = MsgPackParser.DEFAULT.parseTokens((byte[]) bytes)) {
