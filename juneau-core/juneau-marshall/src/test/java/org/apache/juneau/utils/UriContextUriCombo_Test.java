...
class UriContextUriCombo_Test extends TestBase {
	...
	void a01_testAbsoluteAuthority(Data d) {
		assertEquals(d.r.eAbsoluteAuthority, d.in.uriContext.getAbsoluteAuthority(), fs("%s: testAbsoluteAuthority() failed", d.label));
	}

	@ParameterizedTest
	@MethodSource("data")
	void a02_testAbsoluteContext(Data d) {
		assertEquals(d.r.eAbsoluteContext, d.in.uriContext.getAbsoluteContextRoot(), fs("%s: testAbsoluteContext() failed", d.label));
	}

	@ParameterizedTest
	@MethodSource("data")
	void a03_testAbsoluteResource(Data d) {
		assertEquals(d.r.eAbsoluteResource, d.in.uriContext.getAbsoluteServletPath(), fs("%s: testAbsoluteResource() failed", d.label));
	}

	@ParameterizedTest
	@MethodSource("data")
	void a04_testAbsolutePath(Data d) {
		assertEquals(d.r.eAbsolutePath, d.in.uriContext.getAbsolutePathInfo(), fs("%s: testAbsolutePath() failed", d.label));
	}

	@ParameterizedTest
	@MethodSource("data")
	void a05_testRootRelativeContext(Data d) {
		assertEquals(d.r.eRootRelativeContext, d.in.uriContext.getRootRelativeContextRoot(), fs("%s: testRootRelativeContext() failed", d.label));
	}

	@ParameterizedTest
	@MethodSource("data")
	void a06_testRootRelativeResource(Data d) {
		assertEquals(d.r.eRootRelativeResource, d.in.uriContext.getRootRelativeServletPath(), fs("%s: testRootRelativeResource() failed", d.label));
	}

	@ParameterizedTest
	@MethodSource("data")
	void a07_testRootRelativePath(Data d) {
		assertEquals(d.r.eRootRelativePath, d.in.uriContext.getRootRelativePathInfo(), fs("%s: testRootRelativePath() failed", d.label));
	}
}
