...
class HtmlLinkApplyAnnotation_Test extends TestBase {

	@Test void a01_empty_null() {
		assertTrue(HtmlLinkApplyAnnotation.empty(null));
	}

	@Test void a02_empty_default() {
		assertTrue(HtmlLinkApplyAnnotation.empty(HtmlLinkApplyAnnotation.DEFAULT));
	}

	@Test void a03_empty_nonDefault() {
		var a = HtmlLinkApplyAnnotation.create(String.class).build();
		assertFalse(HtmlLinkApplyAnnotation.empty(a));
	}

	@Test void a04_create_and_build() {
		var a = HtmlLinkApplyAnnotation.create("MyClass").build();
	...
	@Test void b05_applier_emptyOn_returnsEarly() {
		...
		var applier = new HtmlLinkApplyAnnotation.Applier(vr);
		var b = MarshallingContext.DEFAULT.copy();
		var a = HtmlLinkApplyAnnotation.DEFAULT;
