...
class HoconParserSession_Test extends TestBase {
	...
	// ============================================================
	// a01-a09: doRead top-level branches
	// ============================================================
	...
	void a01_nullInputReturnsNull() throws Exception {
		...
		var m = HoconParser.DEFAULT.read((Object) null, Map.class, String.class, Object.class);
		assertNull(m);
	}
	...
	void a02_whitespaceOnlyInputReturnsNull() throws Exception {
		...
		var m = HoconParser.DEFAULT.read("   \n\t  \n", Map.class, String.class, Object.class);
		assertNull(m);
	}

	@Test
	void a03_ioExceptionFromReader() {
		// Triggers the catch (IOException e) in doRead → wraps in ParseException.
		var bad = new Reader() {
		...
		assertThrows(Exception.class, () -> HoconParser.DEFAULT.read(bad, Map.class, String.class, Object.class));
	}
	...
	// ============================================================
	// c01-c10: readValue / readValueOrConcat branches
	// ============================================================
	...
	void d01_missingSeparatorAtRoot() {
		...
		assertThrows(Exception.class, () -> HoconParser.DEFAULT.read(hocon, Map.class, String.class, Object.class));
	}
	...
	void d03_unexpectedTokenAsValue() {
		// readValue default branch — value is RBRACE with no preceding open.
		var hocon = "a = }";
		assertThrows(Exception.class, () -> HoconParser.DEFAULT.read(hocon, Map.class, String.class, Object.class));
	}
	...
	// ============================================================
	// f01-f04: object merging — existing-is-object branch in readObject/readRootBraceless
	// ============================================================
	...
	void h01_byteArrayInListBase64Default() throws Exception {
		...
		var bean = HoconParser.DEFAULT.read(hocon, BeanWithByteList.class);
		assertNotNull(bean);
	...
	void h02_byteArrayInListHex() throws Exception {
		...
		var bean = p.read(hocon, BeanWithByteList.class);
		assertNotNull(bean);
		assertArrayEquals(new byte[] { 1, 2, 3 }, bean.data.get(0));
	}
	...
	@Test
	void i03_readEmptyToBean() throws Exception {
		// convertToBean(null,...) returns null branch.
		var bean = HoconParser.DEFAULT.read("", SimpleBean.class);
		assertNull(bean);
	}
	...
	void j02_typePromotionStringToBoolean() throws Exception {
		...
		var bean = HoconParser.DEFAULT.read("name = a\nage = 21\nactive = true", SimpleBean.class);
	...
	void j04_beanWithListOfBeans() throws Exception {
		...
		var bean = HoconParser.DEFAULT.read(hocon, BeanWithList.class);
		assertNotNull(bean);
	...
	void l01_nullKeyAtRoot() throws Exception {
		...
		// then returns EMPTY_PATH (line 173-174), and readRootBraceless breaks (line 119-120).
	...
	void l04_unterminatedObject() {
		...
		try {
			HoconParser.DEFAULT.read(hocon, Map.class, String.class, Object.class);
		} catch (Exception ignore) {
			// acceptable
		}
	}
	...
	void l07_swapPathOnBeanType() throws Exception {
		...
		var p = HoconParser.create().binaryFormat(BinaryFormat.HEX).build();
		var hocon = "data = \"010203\"";
		var bean = p.read(hocon, BeanWithBytes.class);
		assertNotNull(bean);
