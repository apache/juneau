...
class CommonParser_Test extends TestBase {
	...
	@Test void a01_fromSerializer() throws Exception {
		...
		var jl = (JsonList)p.read("[{\"attribute\":\"value\"},{\"attribute\":\"value\"}]", Object.class);
		...
		// JsonParser throws for empty/whitespace; Json5Parser returns null
		assertThrows(ParseException.class, ()->p.read("", Object.class));
		assertThrows(ParseException.class, ()->p.read("   ", Object.class));
		assertThrows(ParseException.class, ()->p.read("   \t", Object.class));

		jl = (JsonList)p.read("[{\"attribute\":\"value\"},{\"attribute\":\"value\"}]", Object.class);
		assertEquals("value", jl.getMap(0).getString("attribute"));
		assertEquals("value", jl.getMap(1).getString("attribute"));
		...
		b.list = tl;
		var json = JsonSerializer.create().addBeanTypes().addRootType().beanDictionary(A1.class).build().write(b);
		b = (A1)p.read(json, Object.class);
		assertEquals("value1", b.list.get(1).value);

		json = JsonSerializer.DEFAULT.write(b);
	...
	@Test void a02_correctHandlingOfUnknownProperties() throws Exception {
		...
		assertEquals(1, b.a);
		assertEquals(2, b.b);
		assertThrows(ParseException.class, ()->JsonParser.DEFAULT.read(in, B.class));
	}
	...
	@Test void a04_parserListeners() throws Exception {
		...
		assertEquals("unknownProperty, line 1, column 7", MyParserListener.events.get(0));
	}
