...
class BeanPropApplyAnnotation_Test extends TestBase {

	@Test void a01_empty_null() {
		assertTrue(BeanPropApplyAnnotation.empty(null));
	}

	@Test void a02_empty_default() {
		assertTrue(BeanPropApplyAnnotation.empty(BeanPropApplyAnnotation.DEFAULT));
	}

	@Test void a03_empty_nonDefault() {
		var a = BeanPropApplyAnnotation.create(String.class).build();
		assertFalse(BeanPropApplyAnnotation.empty(a));
	}

	@Test void a04_create_and_build() {
		var a = BeanPropApplyAnnotation.create("MyClass").build();
	...
	@Test void b05_applier_emptyOn_returnsEarly() {
		...
		var applier = new BeanPropApplyAnnotation.Applier(vr);
