...
class ReadWriteOnlyProperties_Test extends TestBase {
	...
	@Test void a02_beanpOnPrimitiveFields_parser() {
		var x = Json5.to("{f1:1,f2:2}", A.class);
		assertEquals(0, x.f1);
		assertEquals(2, x.f2);
	}
	...
	@Test void b02_beanAnnotationParser() {
		var x = Json5Parser.DEFAULT.copy().applyAnnotations(BcConfig.class).build().read("{f1:1,f2:2}", Bc.class);
		assertEquals(0, x.f1);
		assertEquals(2, x.f2);
	}
	...
	@Test void b02_beanAnnotationParser_usingConfig() throws Exception {
		var x = Json5.to("{f1:1,f2:2}", B.class);
		assertEquals(0, x.f1);
		assertEquals(2, x.f2);
	}
	...
	@Test void c02_beanAnnotationParser() throws Exception {
		var rp = Json5Parser.DEFAULT.copy()
			...
			.beanPropertiesWriteOnly(C.class.getName(), "f2")
			.build();
		var x = rp.read("{f1:1,f2:2}", C.class);
		assertEquals(0, x.f1);
		assertEquals(2, x.f2);
	...
	@Test void d02_beanAnnotation_bproAll_Parser() {
		var x = Json5.to("{f1:1,f2:2}", D.class);
		assertEquals(0, x.f1);
		assertEquals(0, x.f2);
	}
	...
	@Test void d04_beanAnnotation_bproAll_Parser_usingConfig() throws Exception {
		var x = Json5Parser.DEFAULT.copy().applyAnnotations(DcConfig.class).build().read("{f1:1,f2:2}", Dc.class);
		assertEquals(0, x.f1);
		assertEquals(0, x.f2);
	}
	...
	@Test void e02_beanAnnotation_bpwoAll_Parser() throws Exception {
		var x = Json5.to("{f1:1,f2:2}", E.class);
		assertEquals(1, x.f1);
		assertEquals(2, x.f2);
	}
	...
	@Test void e04_beanAnnotation_bpwoAll_Parser_usingConfig() throws Exception {
		var x = Json5Parser.DEFAULT.copy().applyAnnotations(EcConfig.class).build().read("{f1:1,f2:2}", Ec.class);
		assertEquals(1, x.f1);
		assertEquals(2, x.f2);
	}
}