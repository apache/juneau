...
class HjsonApplyAnnotation_Test extends TestBase {
	...
	@Test void a03_empty_nonDefault() {
		var a = HjsonApplyAnnotation.create(String.class).build();
		assertFalse(HjsonApplyAnnotation.empty(a));
	}

	@Test void a04_create_and_build() {
		var a = HjsonApplyAnnotation.create("MyClass").build();
	...
	@Test void b05_applier_emptyOn_returnsEarly() {
		...
		var applier = new HjsonApplyAnnotation.Applier(vr);
		var b = MarshallingContext.DEFAULT.copy();
		var a = HjsonApplyAnnotation.DEFAULT;
