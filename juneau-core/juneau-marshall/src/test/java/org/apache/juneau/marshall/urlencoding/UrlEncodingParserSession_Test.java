 ...
 *  <li>readAnything paths: collection/array/args, Optional, "not a bean", _value unwrap
 *  <li>readIntoBeanMap state-machine: empty value, unknown property, expanded-params
 *  <li>readIntoMap2 state-machine: array indices, repeated keys, null/empty values
 *  <li>doReadIntoMap with leading '?' and without
 * </ul>
 */
@SuppressWarnings({
	"rawtypes",
...
class UrlEncodingParserSession_Test extends TestBase {
	...
	//------------------------------------------------------------------------------------------------------------------
	// b01-b: Collections / arrays / numeric-indexed maps (readAnything sType.isCollection/isArray)
	//------------------------------------------------------------------------------------------------------------------

	@Test void b01_arrayInput() throws Exception {
		var v = P.read("?0=a&1=b&2=c", String[].class);
		assertArrayEquals(new String[]{"a","b","c"}, v);
	}

	@Test void b02_listInput() throws Exception {
		var v = P.read("?0=a&1=b&2=c", List.class, String.class);
		assertEquals(List.of("a","b","c"), v);
	}

	@Test void b03_intArrayInput() throws Exception {
		var v = P.read("?0=1&1=2&2=3", int[].class);
	...
	@Test void b04_outOfOrderIndices_sortedByTreeMap() throws Exception {
		...
		var v = P.read("?2=c&0=a&1=b", String[].class);
		assertArrayEquals(new String[]{"a","b","c"}, v);
	}

	@Test void b05_argsInput() throws Exception {
		var v = P.readArgs("?0=foo&1=42", new java.lang.reflect.Type[]{ String.class, Integer.class });
		assertEquals("foo", v[0]);
		assertEquals(Integer.valueOf(42), v[1]);
	}

	//------------------------------------------------------------------------------------------------------------------
	// c01-c: readAnything _value unwrap routing
	//------------------------------------------------------------------------------------------------------------------
	...
	@Test void c01_valueUnwrap_string() throws Exception {
		...
		assertEquals("hello", P.read("_value=hello", Object.class));
	}
	...
	@Test void c02_valueUnwrap_intoNonBeanType() throws Exception {
		...
		assertEquals(Integer.valueOf(42), P.read("_value=42", Integer.class));
	}

	@Test void c03_valueUnwrap_intoLong() throws Exception {
		assertEquals(Long.valueOf(123), P.read("_value=123", Long.class));
	}

	@Test void c04_valueUnwrap_intoBoolean() throws Exception {
		assertEquals(Boolean.TRUE, P.read("_value=true", Boolean.class));
	}

	@Test void c05_optional_present() throws Exception {
		// Optional path: readAnything recurses with element type.
		Optional<?> v = P.read("_value=foo", Optional.class, String.class);
	...
	@Test void c06_optional_empty_input() throws Exception {
		...
		Optional<?> v = P.read("_value=", Optional.class, String.class);
		assertTrue(v.isPresent());
		assertEquals("", v.get());
	}

	//------------------------------------------------------------------------------------------------------------------
	// d01-d: readIntoBeanMap state-machine paths
	//------------------------------------------------------------------------------------------------------------------
	...
	@Test void d03_attrOnly_separator_nullValue_thenAnother() throws Exception {
		...
		var b = P.read("?f1&f2=5", Bean1.class);
		assertNull(b.f1);
		assertEquals(5, b.f2);
	}
	...
	@Test void d06_unknownProperty_emptyValue() throws Exception {
		...
		var b = p.read("?unknown=&f1=hi", Bean1.class);
	...
	@Test void d07_unknownProperty_withValue() throws Exception {
		...
		var p = UrlEncodingParser.create().ignoreUnknownBeanProperties().build();
		var b = p.read("?unknown=stuff&f1=hi", Bean1.class);
		assertEquals("hi", b.f1);
	}

	//------------------------------------------------------------------------------------------------------------------
	// e: Expanded params path through readIntoBeanMap (shouldUseExpandedParams)
	//------------------------------------------------------------------------------------------------------------------
	...
	//------------------------------------------------------------------------------------------------------------------
	// f: readIntoMap2 paths via readIntoMap public API
	//------------------------------------------------------------------------------------------------------------------

	@Test void f01_doParseIntoMap_withQuestionMark() throws Exception {
		// doReadIntoMap: r.peekSkipWs() == '?' -> read.
		Map<String,String> m = new LinkedHashMap<>();
		P.readIntoMap("?a=1&b=2", m, String.class, String.class);
	...
	@Test void f03_doParseIntoMap_emptyInput() throws Exception {
		// Triggers c==-1 early-return in readIntoMap2.
		Map<String,String> m = new LinkedHashMap<>();
		P.readIntoMap("", m, String.class, String.class);
		assertTrue(m.isEmpty());
	}

	@Test void f04_repeatedKey_objectValue_promotesToList() throws Exception {
		// readIntoMap2: when valueType.isObject() and key seen twice, promote to JsonList.
		var m = (Map) P.read("?a=1&a=2&a=3", Map.class);
		Object v = m.get("a");
	...
	@Test void f05_repeatedKey_stringValue_overwrites() throws Exception {
		...
		Map<String,String> m = new LinkedHashMap<>();
		P.readIntoMap("?a=1&a=2", m, String.class, String.class);
		assertEquals("2", m.get("a"));
	}

	@Test void f06_emptyValue_S3_eof() throws Exception {
		// readIntoMap2: state==S3, c==-1 -> empty conversion, m.put.
		Map<String,String> m = new LinkedHashMap<>();
		P.readIntoMap("?a=", m, String.class, String.class);
		assertEquals("", m.get("a"));
	}

	@Test void f07_emptyValue_S3_separator() throws Exception {
		// readIntoMap2: state==S3, c=='<sep>' -> empty conversion, state=S1.
		Map<String,String> m = new LinkedHashMap<>();
		P.readIntoMap("?a=&b=v", m, String.class, String.class);
		assertEquals("", m.get("a"));
		assertEquals("v", m.get("b"));
	}

	@Test void f08_attrNoEquals_S2_eof() throws Exception {
		// readIntoMap2: state==S2, c==-1 with no '='.
		Map<String,Object> m = new LinkedHashMap<>();
		P.readIntoMap("?a", m, String.class, Object.class);
		assertTrue(m.containsKey("a"));
		assertNull(m.get("a"));
	}

	@Test void f09_attrNoEquals_S2_separator() throws Exception {
		// readIntoMap2: state==S2, c=='<sep>' with no '='.
		Map<String,Object> m = new LinkedHashMap<>();
		P.readIntoMap("?a&b=v", m, String.class, Object.class);
		assertTrue(m.containsKey("a"));
		assertNull(m.get("a"));
		assertEquals("v", m.get("b"));
	}
	...
	@Test void g01_percentEncodedUtf8_2byte() throws Exception {
		Map<String,String> m = new LinkedHashMap<>();
		P.readIntoMap("?k=%C2%A2", m, String.class, String.class);
		assertEquals("¢", m.get("k"));
	}

	@Test void g02_percentEncodedUtf8_3byte() throws Exception {
		Map<String,String> m = new LinkedHashMap<>();
		P.readIntoMap("?k=%E2%82%AC", m, String.class, String.class);
		assertEquals("€", m.get("k"));
	}

	@Test void g03_plusAsSpace() throws Exception {
		Map<String,String> m = new LinkedHashMap<>();
		P.readIntoMap("?k=hello+world", m, String.class, String.class);
	...
	@Test void g04_specialCharsEncoded() throws Exception {
		Map<String,String> m = new LinkedHashMap<>();
		P.readIntoMap("?k=%26%3D%3F", m, String.class, String.class);
		assertEquals("&=?", m.get("k"));
	}
	...
	@Test void h01_customMapType() throws Exception {
		Map<String,String> m = P.read("?a=1&b=2", TreeMap.class, String.class, String.class);
		assertEquals("1", m.get("a"));
		assertEquals("2", m.get("b"));
	}

	@Test void h02_nestedMapValue() throws Exception {
		var m = (Map) P.read("?a=(x=1,y=2)", Map.class);
	...
	@Test void i01_malformed_nonBeanClass() {
		...
		assertThrows(ParseException.class, () -> P.read("?x=1", Thread.class));
	}

	//------------------------------------------------------------------------------------------------------------------
	// j: Non-default outer (readAnything outer != null branch)
	//------------------------------------------------------------------------------------------------------------------
	...
	@Test void j01_nestedBean() throws Exception {
		// Nested bean parse path through readIntoBeanMap.
		var b = P.read("?inner=(val=hello)", OuterBean.class);
		assertNotNull(b.inner);
		assertEquals("hello", b.inner.val);
	}

	//------------------------------------------------------------------------------------------------------------------
	// k: Leading '?' branch in readAnything (line 166-170)
	//------------------------------------------------------------------------------------------------------------------

	@Test void k01_leadingQuestionMark_intoBean() throws Exception {
		var b = P.read("?f1=foo&f2=99", Bean1.class);
	...
	//------------------------------------------------------------------------------------------------------------------
	// m: Bean-type _type marker path (readAnything object branch)
	//------------------------------------------------------------------------------------------------------------------
	...
	@Test void m01_typeMarker() throws Exception {
		// readAnything: m.containsKey(getBeanTypePropertyName(eType)) -> cast(m, null, eType).
		// Use beanDictionary-aware parser.
		var p = UrlEncodingParser.create().beanDictionary(TypedBean.class).build();
		Object v = p.read("?_type=b1&x=hello", Object.class);
		assertTrue(v instanceof TypedBean, "got: " + cn(v));
		assertEquals("hello", ((TypedBean)v).x);
	}

	//------------------------------------------------------------------------------------------------------------------
	// n: Args parsing - covers readIntoMap2 type.isArgs() branches (lines 416-419)
	//------------------------------------------------------------------------------------------------------------------

	@Test void n01_args_threeTypes() throws Exception {
		Object[] v = P.readArgs("?0=foo&1=42&2=true",
	...
	//------------------------------------------------------------------------------------------------------------------
	// o: Null-attr (%00 attribute) -> readAttrName returns null -> readIntoBeanMap returns null
	//------------------------------------------------------------------------------------------------------------------

	@Test void o01_nullAttrName() throws Exception {
		// %00 as an attribute name causes readAttrName to return null and readIntoBeanMap to return null.
		// The wrapping readAnything() treats null as a null bean -> builder/canCreateNewBean branch returns null.
		var p = UrlEncodingParser.create().ignoreUnknownBeanProperties().build();
		Bean1 b = p.read("?%00=value", Bean1.class);
	...
	@Test void p01_customMapSubtype() throws Exception {
		// readAnything sType.isMap() with canCreateNewInstance true -> sType.newInstance().
		Map<String,String> m = P.read("?a=1", LinkedHashMap.class, String.class, String.class);
		assertEquals("1", m.get("a"));
	}

	//------------------------------------------------------------------------------------------------------------------
	// q: Object key value path -> JsonList promotion (readIntoMap2 with object value type, repeated keys)
	//------------------------------------------------------------------------------------------------------------------
	...
	@Test void q01_repeatedKey_promotesAlreadyList() throws Exception {
		...
		var m = (Map) P.read("?a=1&a=2&a=3&a=4", Map.class);
		Object v = m.get("a");
	...
	@Test void r01_collectionWithCtor() throws Exception {
		// LinkedList has a no-arg ctor; readAnything line 195 ternary uses sType.newInstance().
		List<String> v = P.read("?0=a&1=b", LinkedList.class, String.class);
		assertEquals(2, v.size());
	}

	//------------------------------------------------------------------------------------------------------------------
	// s: ParserPipe-driven explicit doReadIntoMap with leading '?'
	//------------------------------------------------------------------------------------------------------------------

	@Test void s01_typedMap_strings() throws Exception {
		Map<String,String> m = new TreeMap<>();
		P.readIntoMap("?b=2&a=1&c=3", m, String.class, String.class);
	...
	@Test void t01_optionalInteger() throws Exception {
		Optional<?> v = P.read("_value=42", Optional.class, Integer.class);
		assertTrue(v.isPresent());
	...
	@Test void u01_setterExceptionInExpandedParams() {
		...
		var p = UrlEncodingParser.create().expandedParams().build();
		assertThrows(Exception.class, () -> p.read("?f1=a", BadSetterBean.class));
	}
