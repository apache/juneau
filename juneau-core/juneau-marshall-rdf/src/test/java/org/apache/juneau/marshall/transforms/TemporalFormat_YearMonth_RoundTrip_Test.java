...
/**
 * Cross-pair round-trip coverage for {@link TemporalFormat} bound to the {@link YearMonth} subtype across
 ...
 *
 * <p>
 * Most {@link TemporalFormat} values are intentionally lossy for {@link YearMonth} — only
 * {@link TemporalFormat#ISO_YEAR_MONTH} (and {@link TemporalFormat#MILLIS} which round-trips through
 * epoch-millis at the first day of the month) preserve year+month cleanly.  The {@code expectedAfter}
 * helper canonicalizes through the format's own {@code format/parse} cycle so the assertion reflects the
...
class TemporalFormat_YearMonth_RoundTrip_Test extends TestBase {
	 ...
	 * {@link YearMonth} — only {@link TemporalFormat#ISO_YEAR_MONTH} (and {@link TemporalFormat#MILLIS}
	 * which round-trips through epoch-millis at day-1) preserve year+month cleanly; every other format
	 * coerces through {@link org.apache.juneau.marshall.swaps.DefaultingTemporalAccessor} (filling in day=1,
	 * time=00:00:00 on the way out and falling back to year=1970 / month=1 on the way back in for
	 * time-only formats).  Canonicalize through the format's own {@code format/parse} cycle.
	 *
	 * <p>
	 ...
	 */
	private static YearMonth expectedAfter(YearMonth original, RoundTrip_Tester t, TemporalFormat fmt) {
		if (original == null)
			return null;
		if (t.isValidationOnly())
			return original;
		return fmt.parse(fmt.format(original, null), YearMonth.class, null);
	}

	//====================================================================================================
	// Bean with one YearMonth field — exercises MarshalledPropertyPostProcessor.applyContextFormats
	//====================================================================================================
	...
	@MethodSource("combos")
	void a01_yearMonthProperty_basic(RoundTrip_Tester t, TemporalFormat fmt) throws Exception {
		var x = new A01Bean();
		x.ym = YearMonth.of(2024, 6);

		x = t.roundTrip(x);
		assertEquals(expectedAfter(YearMonth.of(2024, 6), t, fmt), x.ym, "fmt=" + fmt);
	}

	//====================================================================================================
	// Bean with multiple YearMonth fields — multi-property cross-section
	//====================================================================================================

	public static class A02Bean {
		public YearMonth created;
		public YearMonth updated;
		public YearMonth epoch;
	}

	@ParameterizedTest(name = "{0} | {1}")
	@MethodSource("combos")
	void a02_yearMonthProperty_multipleFields(RoundTrip_Tester t, TemporalFormat fmt) throws Exception {
		var x = new A02Bean();
		x.created = YearMonth.of(2024, 6);
		x.updated = YearMonth.of(2025, 1);
		x.epoch = YearMonth.of(1970, 1);

		x = t.roundTrip(x);
		assertEquals(expectedAfter(YearMonth.of(2024, 6), t, fmt), x.created, "fmt=" + fmt);
		assertEquals(expectedAfter(YearMonth.of(2025, 1), t, fmt), x.updated, "fmt=" + fmt);
		assertEquals(expectedAfter(YearMonth.of(1970, 1), t, fmt), x.epoch, "fmt=" + fmt);
	}

	//====================================================================================================
	// Bean with epoch + boundary-month YearMonth — boundary coverage
	//====================================================================================================

	public static class A03Bean {
		public YearMonth epoch;
		public YearMonth january;
		public YearMonth december;
	}

	@ParameterizedTest(name = "{0} | {1}")
	@MethodSource("combos")
	void a03_yearMonthProperty_boundaries(RoundTrip_Tester t, TemporalFormat fmt) throws Exception {
		var x = new A03Bean();
		x.epoch = YearMonth.of(1970, 1);
		x.january = YearMonth.of(2024, 1);
		x.december = YearMonth.of(2024, 12);

		x = t.roundTrip(x);
		assertEquals(expectedAfter(YearMonth.of(1970, 1), t, fmt), x.epoch, "fmt=" + fmt);
		assertEquals(expectedAfter(YearMonth.of(2024, 1), t, fmt), x.january, "fmt=" + fmt);
		assertEquals(expectedAfter(YearMonth.of(2024, 12), t, fmt), x.december, "fmt=" + fmt);
	}

	//====================================================================================================
	// Bean with far-future YearMonth — large epoch-millis stress (MILLIS numeric promotion paths)
	//====================================================================================================

	public static class A04Bean {
		public YearMonth farFuture;
	}

	@ParameterizedTest(name = "{0} | {1}")
	@MethodSource("combos")
	void a04_yearMonthProperty_largeValues(RoundTrip_Tester t, TemporalFormat fmt) throws Exception {
		var x = new A04Bean();
		// Year 2099 — large but still within all formatters' supported range.
		x.farFuture = YearMonth.of(2099, 12);

		x = t.roundTrip(x);
		assertEquals(expectedAfter(YearMonth.of(2099, 12), t, fmt), x.farFuture, "fmt=" + fmt);
	}

	//====================================================================================================
	// Bean with null YearMonth field — sentinel-style null preservation
	//====================================================================================================

	@ParameterizedTest(name = "{0} | {1}")
	@MethodSource("combos")
	void a05_yearMonthProperty_nullField(RoundTrip_Tester t, TemporalFormat fmt) throws Exception {
		var x = new A01Bean();

		x = t.roundTrip(x);
		assertNull(x.ym, "fmt=" + fmt);
	}

	//====================================================================================================
	// Standalone top-level YearMonth — exercises per-format dispatch in *SerializerSession / *ParserSession
	//====================================================================================================

	@ParameterizedTest(name = "{0} | {1}")
	@MethodSource("combos")
	void a06_yearMonthTopLevel(RoundTrip_Tester t, TemporalFormat fmt) throws Exception {
		// Validation-only testers (Json schema, Csv, Parquet) have no parser-side round-trip semantics for a
		// standalone YearMonth value — they assert that the serializer accepts it without throwing.
		if (t.isValidationOnly())
			return;
